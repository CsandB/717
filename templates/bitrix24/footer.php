...
if ($isCompositeMode && ! $isIndexPage) {
    $dynamicArea = FrameStatic::getCurrentDynamicArea();
    if ($dynamicArea !== null) {
...
<tr>
    ...
    <td class="bx-layout-inner-center">
        <div id="footer">
							<span id="copyright">
								<? if ($isBitrix24Cloud): ?>
                                    <a id="bitrix24-logo" target="_blank"
                                       class="bitrix24-logo-<?= (LANGUAGE_ID
                                           == "ua") ? LANGUAGE_ID
                                           : Loc::getDefaultLang(LANGUAGE_ID) ?>"
                                       href="<?= GetMessage("BITRIX24_URL") ?>"></a>
									<? include($_SERVER["DOCUMENT_ROOT"]
                                        .SITE_TEMPLATE_PATH
                                        ."/languages.php"); ?>
									<span class="bx-lang-btn <?= LANGUAGE_ID ?>"
                                          id="bx-lang-btn"
                                    ...
                                    <div style="display: none"
										...
										<table>
											<? for (
												...
												<? for ($j = 1; $j <= 3; $j++):
                                                    ?>
                                                    <td class="bx-lang-popup-item"
                                                        onclick="B24.changeLanguage('<?= key($b24Languages) ?>');">
														<span>
															<?
                                                            $lang
                                                                = array_shift($b24Languages);
                                                            echo $lang["NAME"]
                                                                .($lang["IS_BETA"]
                                                                    ? ", beta"
                                                                    : "");
                                ...
                                <? endif ?>
								<span class="bitrix24-copyright"><?= GetMessage("BITRIX24_COPYRIGHT2",
                                        array("#CURRENT_YEAR#" => date("Y"))) ?></span>
								<?
                                ...
                                if ($isBitrix24Cloud
                                    && $partnerID
                                        = COption::GetOptionString("bitrix24",
                                        "partner_id", "")
                                ) {
                                    if ($partnerID != "9409443") //sber
                                    {
                                        ...
                                        <a href="javascript:void(0)"
                                           onclick="showPartnerForm(<? echo CUtil::PhpToJSObject($arParamsPartner) ?>); return false;"
                                           class="footer-discuss-link"><?= GetMessage("BITRIX24_PARTNER_CONNECT") ?></a>
                                        <?
                                    }
                                } elseif (CModule::IncludeModule("bitrix24")) {
                                    $orderParams
                                        = CBitrix24::getPartnerOrderFormParams();
                                    ?>
                                    <a class="b24-web-form-popup-btn-57 footer-discuss-link"
                                       onclick="B24.showPartnerOrderForm(<?= CUtil::PhpToJSObject($orderParams) ?>);"><?= GetMessage("BITRIX24_PARTNER_ORDER") ?></a>
                                    <?
                                } else {
                                    ?>
                                    <a href="javascript:void(0)"
                                       onclick="BX.Helper.show();"
...
if (defined("BX24_HOST_NAME")):?>
    <script>
      ...
      var _ba = _ba || [];
      _ba.push(['aid', '1682f9867b9ef36eacf05e345db46f3c']);
...
$APPLICATION->IncludeComponent("bitrix:intranet.mail.check", "", array(), false,
    array("HIDE_ICONS" => "Y"));

$dynamicArea = new FrameStatic("otp-info");
$dynamicArea->setAssetMode(AssetMode::STANDARD);
$dynamicArea->startDynamicArea();
$APPLICATION->IncludeComponent("bitrix:intranet.otp.info", "", array(
    "PATH_TO_PROFILE_SECURITY" => $profileLink."/user/#user_id#/security/",
...
<script type="text/javascript">BX.onCustomEvent(window,
      'onScriptsLoaded');</script>
</body>
</html>
