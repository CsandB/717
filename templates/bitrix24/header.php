...
if (isset($_GET["RELOAD"]) && $_GET["RELOAD"] == "Y") {
    ...
    if (mb_strpos($_SERVER["REQUEST_URI"], "/historyget/") > 0) {
        return;
    } else {
        if (isset($_GET["IFRAME"]) && $_GET["IFRAME"] === "Y"
            ...
            $APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH."/interface.css",
                true);
            $APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH."/bitrix24.js",
                true);
...
Loc::loadMessages($_SERVER["DOCUMENT_ROOT"]."/bitrix/templates/"
    .SITE_TEMPLATE_ID."/header.php");
...
$isIndexPage = $APPLICATION->GetCurPage(true) === SITE_DIR."stream/index.php"
    ...
    || (defined("BITRIX24_INDEX_PAGE")
        && constant("BITRIX_INDEX_PAGE") === true);
...
<head>
    ...
    <? if ($isBitrix24Cloud): ?>
        ...
        <link rel="apple-touch-icon-precomposed"
              href="/images/iphone/57x57.png"/>
        <link rel="apple-touch-icon-precomposed" sizes="72x72"
              href="/images/iphone/72x72.png"/>
        <link rel="apple-touch-icon-precomposed" sizes="114x114"
              href="/images/iphone/114x114.png"/>
        <link rel="apple-touch-icon-precomposed" sizes="144x144"
              href="/images/iphone/144x144.png"/>
    <?endif;
    ...
    $bodyClass .= " bitrix24-".ThemePicker::getInstance()
            ->getCurrentBaseThemeId()."-theme";
...
$isExtranet = ModuleManager::isModuleInstalled("extranet")
...
<table class="bx-layout-table<?= $layoutMode ?>">
    <tr>
        <td class="bx-layout-header">
            <? if (( ! $isBitrix24Cloud || $USER->IsAdmin())
                && ! defined("SKIP_SHOW_PANEL")
            ): ?>
                <div id="panel">
            ...
            if ($isBitrix24Cloud) {
                ...
                if (
                    ...
                    || is_array(CUserOptions::GetOption('bitrix24',
                ...
                <div id="header-inner">
                    ...
                    if ( ! $isExtranet) {
                        if ( ! ModuleManager::isModuleInstalled("timeman")
                            ||
                            ! $APPLICATION->IncludeComponent('bitrix:timeman',
                                'bitrix24', array(), false,
                                array("HIDE_ICONS" => "Y"))
                        ) {
                            $APPLICATION->IncludeComponent('bitrix:planner',
                                'bitrix24', array(), false,
                                array("HIDE_ICONS" => "Y"));
                        }
                    } else {
                        CJSCore::Init("timer"); ?>
                        <div
                                class="timeman-container timeman-container-<?= LANGUAGE_ID ?><?= (IsAmPmMode()
                                    ? " am-pm-mode" : "") ?>"
                    ...
                    <script type="text/javascript" data-skip-moving="true">
                      (function() {
                        var isAmPmMode = <?=(IsAmPmMode() ? "true"
                            : "false") ?>;
                        var time = document.getElementById('timeman-timer');
                        var hours = new Date().getHours();
                        var minutes = new Date().getMinutes();
                        if (time) {
                          time.innerHTML = formatTime(hours, minutes, 0,
                              isAmPmMode);
                        ...
                        function formatTime(
                            hours, minutes, seconds, isAmPmMode) {
                          var ampm = '';
                          if (isAmPmMode) {

                            ampm = hours >= 12 ? 'PM' : 'AM';
                            ampm = '<span class="time-am-pm">' + ampm +
                                '</span>';
                            hours = hours % 12;
                          ...
                          return '<span class="time-hours">' + hours +
                              '</span>' +
                    ...
                    <div class="header-logo-block"><? include(__DIR__
                            ."/logo.php"); ?></div>
                    ...
                    <div class="header-search">
                        ...
                        $APPLICATION->IncludeComponent(
                            (ModuleManager::isModuleInstalled("search")
                                ? "bitrix:search.title"
                                : "bitrix:intranet.search.title"),
                            (
                            ModuleManager::isModuleInstalled("search")
                            && COption::GetOptionString("intranet",
                                "search_title_old", "") == "Y" ? ".default_old"
                                : ""
                            ),
                            array_merge(
                                array(
                                    ...
                                    "CONTAINER_ID"          => "search",
                                    "USE_LANGUAGE_GUESS"    => (LANGUAGE_ID
                                        == "ru") ? "Y" : "N",
                                ),
                                $searchParams),
                            false,
                    ...
                    <?
                    $profileLink = $isExtranet ? SITE_DIR."contacts/personal"
                        : SITE_DIR."company/personal";
                    $APPLICATION->IncludeComponent(
                        "bitrix:system.auth.form",
                        "",
                        array(
                            "PATH_TO_SONET_PROFILE"      => $profileLink
                                ."/user/#user_id#/",
                            "PATH_TO_SONET_PROFILE_EDIT" => $profileLink
                                ."/user/#user_id#/edit/",
                        ),
                        false
                    );
                ...
                <tr class="bx-layout-inner-top-row">
                    <td class="bx-layout-inner-left" id="layout-left-column">
                        <? $APPLICATION->IncludeComponent(
                            ...
                            "left_vertical",
                            array(
                                "ROOT_MENU_TYPE"        => file_exists($_SERVER["DOCUMENT_ROOT"]
                                    .SITE_DIR.".superleft.menu_ext.php")
                                    ? "superleft" : "top",
                        ...
                        if ($imBarExists) {
                            ...
                            $APPLICATION->IncludeComponent(
                                ...
                                array(
                                    ...
                                    "RECENT"                => "Y",
                                    'PATH_TO_SONET_EXTMAIL' => Option::get('intranet',
                                        'path_mail_client', SITE_DIR.'mail/'),
                                ),
                                false,
                                array("HIDE_ICONS" => "Y")
                            );
                    ...
                    <td class="bx-layout-inner-center" id="content-table">
                        <?
                        if ($isCompositeMode && ! $isIndexPage) {
                            $isDefaultTheme = ThemePicker::getInstance()
                                    ->getCurrentThemeId() === "default";
                            $bodyClass = $isDefaultTheme ? ""
                                : " no-background";
                            $dynamicArea = new FrameStatic("workarea");
                            $dynamicArea->setAssetMode(AssetMode::STANDARD);
                            $dynamicArea->setContainerId("content-table");
                            $dynamicArea->setStub('
							<table class="bx-layout-inner-inner-table composite-mode'
                                .$bodyClass.'">
								<colgroup>
                        ...
                        <table class="bx-layout-inner-inner-table <? $APPLICATION->ShowProperty("BodyClass"); ?>">
                            ...
                            <tr>
                                <td class="bx-layout-inner-inner-cont">
                                    ...
                                    <div id="workarea">
                                        <? if ($APPLICATION->GetProperty("HIDE_SIDEBAR",
                                                "N") != "Y"
                                        ):
                                            ?>
                                            <div id="sidebar"><?
                                                    ...
                                                    <div class="pagetitle-wrap <? $APPLICATION->ShowProperty("TitleClass"); ?>">
                                                        <div class="pagetitle-inner-container">
                                                            <div class="pagetitle-menu"
                                                                 id="pagetitle-menu"><? $APPLICATION->ShowViewContent("pagetitle") ?></div>
                                                            <div class="pagetitle"
                                                                 id="pagetitle"><? $APPLICATION->ShowTitle(false); ?></div>
                                                            <? $APPLICATION->ShowViewContent("inside_pagetitle") ?>
                                                        </div>
                                                    </div>
                                                <? endif ?>
                                                <? CPageOption::SetOptionString("main.interface",
                                                    "use_themes",
                                                    "N"); //For grids?>
