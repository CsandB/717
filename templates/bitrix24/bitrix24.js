...
(function() {
  ...
  var sliderMode = search.indexOf('IFRAME=') !== -1 ||
      search.indexOf('IFRAME%3D') !== -1;
  ...
  BX.addCustomEvent('onPopupWindowInit',
      function(uniquePopupId, bindElement, params) {
        ...
        } else if ((uniquePopupId == 'task-gantt-filter') ||
            (uniquePopupId == 'task-list-filter')) {
          params.lightShadow = true;
          params.className = '';
  ...
  BX.addCustomEvent('onPullEvent-bitrix24',
...
//connection status===
  ...
  BX.addCustomEvent('onImError', BX.delegate(function(error, sendErrorCode) {
    if (error == 'AUTHORIZE_ERROR' || error == 'SEND_ERROR' && sendErrorCode ==
        'AUTHORIZE_ERROR') {
      B24.connectionStatus('offline');
...
var B24 = {
  ...
  formateDate: function(time) {
    return BX.util.str_pad(time.getHours(), 2, '0', 'left') + ':' +
        BX.util.str_pad(time.getMinutes(), 2, '0', 'left');
  },
  ...
  changeLanguage: function(lang) {
    window.location.href = '/auth/?user_lang=' + lang + '&backurl=' +
        B24.getBackUrl();
  },

  getBackUrl: function() {
    var backUrl = window.location.pathname;
    var query = B24.getQueryString(
        ['logout', 'login', 'back_url_pub', 'user_lang']);
...
B24.toggleMenu = function(menuItem, messageShow, messageHide) {
  var menuBlock = BX.findChild(menuItem.parentNode, {tagName: 'ul'}, false,
      false);
  ...
  var toggleText = BX.findChild(menuItem, {className: 'menu-toggle-text'}, true,
      false);
  if (!toggleText)
    return;
  ...
  if (BX.hasClass(menuBlock, 'menu-items-close')) {
    ...
    BX.removeClass(BX.nextSibling(BX.nextSibling(menuItem)),
        'menu-items-close');
    ...
    (new BX.easing({
      ...
      complete: function() {
        if (!opening) {
          ...
          BX.addClass(BX.nextSibling(BX.nextSibling(menuItem)),
              'menu-items-close');
        }
...
B24.licenseInfoPopup = {
  show: function(popupId, title, content, showDemoButton) {
    if (BX.getClass('BX.Bitrix24.LicenseInfoPopup')) {
      BX.Bitrix24.LicenseInfoPopup.show(popupId, title, content,
          showDemoButton);
    }
  },
};
...
B24.Timemanager = {
  ...
  formatTime: function(ts, bSec) {
    return BX.util.str_pad(parseInt(ts / 3600), 2, '0', 'left') + ':' +
        BX.util.str_pad(parseInt(ts % 3600 / 60), 2, '0', 'left') +
        (!!bSec ? (':' + BX.util.str_pad(ts % 60, 2, '0', 'left')) : '');
  },

  formatWorkTime: function(h, m, s) {
    return '<span class="tm-popup-notice-time-hours"><span class="tm-popup-notice-time-number">' +
        h +
        '</span></span><span class="tm-popup-notice-time-minutes"><span class="tm-popup-notice-time-number">' +
        BX.util.str_pad(m, 2, '0', 'left') +
        '</span></span><span class="tm-popup-notice-time-seconds"><span class="tm-popup-notice-time-number">' +
        BX.util.str_pad(s, 2, '0', 'left') + '</span></span>';
  },
  ...
  formatCurrentTime: function(hours, minutes, seconds) {
    ...
    return '<span class="time-hours">' + hours + '</span>' +
        ...
        BX.util.str_pad(minutes, 2, '0', 'left') + '</span>' +
        mt;
  },

  init: function(reportJson) {
    BX.addCustomEvent('onTimeManDataRecieved',
        BX.proxy(this.onDataRecieved, this));
    BX.addCustomEvent('onTimeManNeedRebuild',
        BX.proxy(this.onDataRecieved, this));
    BX.addCustomEvent('onPlannerDataRecieved',
        BX.proxy(this.onPlannerDataRecieved, this));
    BX.addCustomEvent('onPlannerQueryResult',
        BX.proxy(this.onPlannerQueryResult, this));
    BX.addCustomEvent('onTaskTimerChange',
        BX.proxy(this.onTaskTimerChange, this));

    BX.timer.registerFormat('worktime_notice_timeman',
        BX.proxy(this.formatWorkTime, this));
    BX.timer.registerFormat('bitrix24_time',
        BX.proxy(this.formatCurrentTime, this));
  ...
  onTaskTimerChange: function(params) {
    if (params.action === 'refresh_daemon_event') {
      ...
      var s = '';
      s += this.formatTime(parseInt(params.data.TIMER.RUN_TIME || 0) +
          parseInt(params.data.TASK.TIME_SPENT_IN_LOGS || 0), true);

      if (!!params.data.TASK.TIME_ESTIMATE && params.data.TASK.TIME_ESTIMATE >
          0) {
  ...
  redraw_planner: function(data) {
    if (!!data.TASKS_ENABLED) {
      ...
      this.layout.tasks.style.display = data.TASKS_COUNT == 0
          ? 'none'
          : 'inline-block';
    }
    ...
    if (!!data.CALENDAR_ENABLED) {
      ...
      this.layout.event.style.display = data.EVENT_TIME == ''
          ? 'none'
          : 'inline-block';
    }
  ...
  redraw: function() {
    ...
    if (!this.timer)
      this.timer = BX.timer(
          {container: this.layout.timer, display: 'bitrix24_time'}); //BX.timer.clock(this.layout.timer);
    ...
    BX.removeClass(this.layout.block,
        'timeman-completed timeman-start timeman-paused timeman-expired');
  ...
  startAnimation: function() {
    ...
    this.startBlink();
    this.animation = setInterval(BX.proxy(this.startBlink, this),
...
B24.Bitrix24InviteDialog.Init = function(arParams) {
  ...
  BX.ready(BX.delegate(function() {
    B24.Bitrix24InviteDialog.popup = BX.PopupWindowManager.create(
        'B24InviteDialog', null, {
          autoHide: false,
          zIndex: 0,
          offsetLeft: 0,
          offsetTop: 0,
...
B24.Bitrix24InviteDialog.ShowForm = function(arParams) {
  ...
  B24.Bitrix24InviteDialog.popup.params.zIndex = (BX.WindowManager
...
B24.Bitrix24InviteDialog.loadForm = function() {
  B24.Bitrix24InviteDialog.popup.setContent(
      '<div style="width:500px;height:300px; background: url(/bitrix/templates/bitrix24/images/loader.gif) no-repeat center;"></div>');
  BX.ajax.post(
...
B24.connectionStatus = function(status) {
  ...
  var connectionPopup = document.querySelector(
  ...
  if (!connectionPopup) {
    ...
    this.b24ConnectionStatus = BX.create('div', {
      attrs: {
        className: 'bx24-connection-status ' +
            (this.b24ConnectionStatusState == 'online'
                ? 'bx24-connection-status-hide'
                : 'bx24-connection-status-show bx24-connection-status-' +
                this.b24ConnectionStatusState) +
            (isFloat ? ' bx24-connection-status-float' : ''),
        'data-role': 'b24-connection-status',
        'data-float': isFloat ? 'true' : 'false',
      },
      children: [
        BX.create('div', {
          props: {className: 'bx24-connection-status-wrap'}, children: [
            this.b24ConnectionStatusText = BX.create('span', {
              props: {className: 'bx24-connection-status-text'},
              html: b24ConnectionStatusStateText,
            }),
            BX.create('span', {
              props: {className: 'bx24-connection-status-text-reload'},
              children: [
                BX.create('span', {
                  props: {className: 'bx24-connection-status-text-reload-title'},
                  html: BX.message('BITRIX24_CS_RELOAD'),
                }),
                BX.create('span', {
                  props: {className: 'bx24-connection-status-text-reload-hotkey'},
                  html: (BX.browser.IsMac() ? '&#8984;+R' : 'Ctrl+R'),
                }),
              ],
              events: {
  ...
  this.b24ConnectionStatus.className = 'bx24-connection-status bx24-connection-status-show ' +
      statusClass + ' ' +
      (this.b24ConnectionStatus.getAttribute('data-float') == 'true'
          ? 'bx24-connection-status-float'
          : '');
  this.b24ConnectionStatusText.innerHTML = b24ConnectionStatusStateText;

  if (!connectionPopup) {
    var nextNode = BX.findChild(document.body,
        {className: 'bx-layout-inner-table'}, true, false);
...
B24.showPartnerOrderForm = function(params) {
  ...
  BX.PopupWindowManager.create('B24PartnerOrderForm', null, {
    ...
    content:
        '<script data-b24-form="inline/' + params.id + '/' + params.sec +
      ...
      onPopupFirstShow: function() {
        (function(w, d, u) {
          ...
          h.parentNode.insertBefore(s, h);
            ...
            'https://cp.bitrix.ru/upload/crm/form/loader_' + params.id + '_' +
            params.sec + '.js');
      },
    },
  }).show();
