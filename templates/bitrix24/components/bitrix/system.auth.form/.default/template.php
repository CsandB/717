...
if ( ! $USER->IsAuthorized()) {
    ?>
    <div class="authorization-block"><a
                href="<?= (SITE_DIR."auth/?backurl=".$arResult["BACKURL"]) ?>"
                class="authorization-text"><?= GetMessage("AUTH_AUTH") ?></a>
    </div>
    <?
    return;
}
...
if (LANGUAGE_ID == "ru" || LANGUAGE_ID == "ua") {
    $videoSteps[] = array(
        ...
        "patterns"      => $USER->CanDoOperation("bitrix24_invite") ? array(
    ...
    $addVideo = array(
        ...
        "7"  => array(
            "crm_perms",
            array("~^".SITE_DIR."crm/configs/perms/~"),
            "/crm/configs/perms/",
        ),
        "8"  => array(
            "crm_lists",
            array("~^".SITE_DIR."crm/configs/status/~"),
            "/crm/lists/",
        ),
        "9"  => array(
            "crm_bp",
            array("~^".SITE_DIR."crm/configs/bp/~"),
            "/crm/configs/bp/",
        ),
        "10" => array("im", array(), "/im/"),
        "11" => array(
            "lists",
            array("~^".SITE_DIR."company/lists/~"),
            "/company/lists/",
        ),
    ...
    foreach ($addVideo as $number => $ids) {
        $videoSteps[] = array(
            ...
            "title_full"    => GetMessage("BITRIX24_HELP_VIDEO_TITLE_FULL_"
                .$number),
...
?>
        ...
        BX.PopupMenu.show('user-menu', bindElement, [
              {
                ...
                href: "<?=CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_SONET_PROFILE'],
              ...
              {
                text: "<?=GetMessageJS("AUTH_PROFILE_B24NET")?>",
                className: 'menu-popup-no-icon',
                href: 'https://www.bitrix24.net/',
              },
                <?endif;?>
                <?if (IsModuleInstalled("im")):?>
              {
                text: "<?=GetMessageJS("AUTH_CHANGE_NOTIFY")?>",
                className: 'menu-popup-no-icon',
                onclick: 'BXIM.openSettings({\'onlyPanel\':\'notify\'})',
              },
                <?endif?>
                <?if (! $bitrix24Included && $USER->isAdmin()):?>
              {
                text: "<?=GetMessageJS("AUTH_ADMIN_SECTION")?>",
                className: 'menu-popup-no-icon',
                href: '/bitrix/admin/',
              },
                <?endif?>
                ...
                ?>
              {
                text: "<?=GetMessageJS("B24_UPGRADE_LICENSE")?>",
                className: 'menu-popup-no-icon',
                onclick: "if (BX.getClass('B24.upgradeButtonRedirect')) B24.upgradeButtonRedirect(<?=CUtil::PhpToJSObject($arJsParams)?>)",
              },
                <?endif?>
                <?if (($bindingItems = Binding\Menu::getMenuItems('top_panel',
                'user_menu',
                ['inline' => true, 'context' => ['USER_ID' => $USER->GetID()]]))):?>
                <?= CUtil::phpToJSObject($bindingItems);?>,
                <?endif;?>
              {
                text: "<?=GetMessageJS("AUTH_LOGOUT")?>",
                className: 'menu-popup-no-icon',
                href: "/auth/?logout=yes&<?=bitrix_sessid_get()?>&backurl=" +
                    encodeURIComponent(B24.getBackUrl()),
              },
            ],
            {
              offsetTop: -9,
	...
	<span class="ui-icon ui-icon-common-user user-img">
        ...
        <span class="user-name"
              id="user-name"><?= $arResult["USER_NAME"] ?></span>
    </div>

<?
$imBarExists = CModule::IncludeModule("im")
...
if ($imBarExists) {
    ...
    ?>
    <div class="help-block" id="bx-help-block"
         title="<?= GetMessage("AUTH_HELP") ?>">
...
if (Loader::includeModule("imbot")) {
    if (
        ...
        $support_bot = Support24::getBotId();
    } else {
        if (
            method_exists('\\Bitrix\\ImBot\\Bot\\Support', 'isEnabled')
            && Support::isEnabled()
        ) {
            $support_bot = Support::getBotId();
        }
    }
}

$isAdmin = ($bitrix24Included && CBitrix24::IsPortalAdmin($USER->GetID())
    || ! $bitrix24Included && $USER->IsAdmin()) ? 1 : 0;
...
if ($imBotIncluded) {
    ...
    $supportPartnerName
        = $APPLICATION->ConvertCharsetArray(Partner24::getPartnerName(),
        SITE_CHARSET, 'utf-8');
    $helpUrlParameters['support_partner_name'] = $supportPartnerName;
}
$helpUrl = CHTTP::urlAddParams($helpUrl, $helpUrlParameters,
    ["encode" => true]);
...
$host = $bitrix24Included && defined("BX24_HOST_NAME") ? BX24_HOST_NAME
...
$notifyData = array(
    ...
    "user_id"              => $USER->GetID(),
    "user_email"           => $USER->GetEmail(),
    "tariff"               => COption::GetOptionString("main",
        "~controller_group_name", ""),
    "host"                 => $host,
    "key"                  => $bitrix24Included ? CBitrix24::RequestSign($host
        .$USER->GetID()) : md5($host.$USER->GetID().'BX_USER_CHECK'),
    "is_cloud"             => $bitrix24Included ? "1" : "0",
    "user_date_register"   => $arResult["USER_DATE_REGISTER"],
    "portal_date_register" => $bitrix24Included
        ? COption::GetOptionString("main", "~controller_date_create", "") : "",
    "partner_link"         => COption::GetOptionString("bitrix24", "partner_id",
        0) ? 'Y' : 'N',
    "counter_update_date"  => $arResult["COUNTER_UPDATE_DATE"],
);
if ($imBotIncluded) {
    $notifyData['support_partner_code']
        = $helpUrlParameters['support_partner_code'];
    $notifyData['support_partner_name']
        = $helpUrlParameters['support_partner_name'];
}
?>
    <script>
      BX.message({
      ...
      BX.Helper.init({
        ...
        needCheckNotify: '<?=($arResult["NEED_CHECK_HELP_NOTIFICATION"] == "Y"
            ? "Y" : "N")?>',
      ...
      if ($support_bot && $_REQUEST['support_chat']) {
          echo 'BX.addCustomEvent("onImInit", function(BXIM) {BXIM.openMessenger('
              .$support_bot.');});';
      }
      ?>
    </script>
...
if ($arResult["CAN_HAVE_HELP_NOTIFICATIONS"] === 'Y') {
    ...
    <script>
      BX.ready(function() {
        BX.Intranet.Helper.Notification.Kernel.initLoader(
            <?=Json::encode($jsNotificationParams);?>);
      });
    </script>
    <?
}
