...
$arParams["PATH_TO_SONET_PROFILE"] = (isset($arParams["PATH_TO_SONET_PROFILE"])
    ? $arParams["PATH_TO_SONET_PROFILE"]
    : SITE_DIR."company/personal/user/#user_id#/");
$arParams["PATH_TO_SONET_PROFILE_EDIT"]
    = (isset($arParams["PATH_TO_SONET_PROFILE_EDIT"])
    ? $arParams["PATH_TO_SONET_PROFILE_EDIT"]
    : SITE_DIR."company/personal/user/#user_id#/edit/");
$arParams["THUMBNAIL_SIZE"] = (isset($arParams["THUMBNAIL_SIZE"])
    ? intval($arParams["THUMBNAIL_SIZE"]) : 100);
...
if ($obCache->InitCache($ttl, $cache_id, $cache_dir)) {
    ...
    if ($GLOBALS["USER"]->IsAuthorized()) {
        ...
        $arResult["USER_DATE_REGISTER"]
            = MakeTimeStamp($arUser["DATE_REGISTER"]);
        $imageFile = false;
        ...
        if (defined("BX_COMP_MANAGED_CACHE")) {
            $CACHE_MANAGER->RegisterTag("USER_CARD_".intval($user_id
...
if (CModule::IncludeModule('bitrix24')
    ...
    $arResult["COUNTER_UPDATE_DATE"]
        = $helpNotify["counter_update_date"]; //time when user read notifications last time
    ...
    if (isset($helpNotify["lastCheckNotificationsTime"])) {
        $arResult['LAST_CHECK_NOTIFICATIONS_TIME']
            = $helpNotify["lastCheckNotificationsTime"];
    }
...
if (Loader::includeModule("ui")) {
    $arResult["HELPDESK_URL"] = Util::getHelpdeskUrl(true);
}
