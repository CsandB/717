    ...
    switch ($_POST["action"]) {
        case "setView":
        {
            if (isset($_POST["currentStepId"])) {
                ...
                $arViewedSteps = CUserOptions::GetOption("bitrix24",
                    "new_helper_views", array());
                if ( ! in_array($currentStepId, $arViewedSteps)) {
                    $arViewedSteps[] = $currentStepId;
                    CUserOptions::SetOption("bitrix24", "new_helper_views",
                        $arViewedSteps);
                }
            }
            break;
        ...
        case "setNotify":
        {
            $notify = CUserOptions::GetOption("bitrix24", "new_helper_notify",
                array());
            if (isset($_POST["time"]) && $_POST["time"] == "hour") {
                ...
                {
                    $notify["counter_update_date"]
                        = time(); // time when user read all current notifications
                }
            }
        ...
        case 'saveNotifications':
        {
            ...
            if (array_key_exists($userOptionNotificationsParamName, $_POST)) {
                $notifications
                    = trim($_POST[$userOptionNotificationsParamName]);
                if ($notifications <> '') {
            ...
            $userOptionLastCheckNotificationsParamName
            ...
            if ($needUpdate) {
                CUserOptions::SetOption("bitrix24", "new_helper_notify",
                    $notify);
            }
            break;
        }
