...
BX.B24SearchTitle = function(arParams) {
  ...
  this.arParams = {
    ...
    'FORMAT': (typeof arParams.FORMAT != 'undefined' && arParams.FORMAT ==
    'json' ? 'json' : 'html'),
    'CATEGORIES_ALL': (typeof arParams.CATEGORIES_ALL != 'undefined'
        ? arParams.CATEGORIES_ALL
        : []),
    'USER_URL': (typeof arParams.USER_URL != 'undefined'
        ? arParams.USER_URL
        : ''),
    'GROUP_URL': (typeof arParams.GROUP_URL != 'undefined'
        ? arParams.GROUP_URL
        : ''),
    'WAITER_TEXT': (typeof arParams.WAITER_TEXT != 'undefined'
        ? arParams.WAITER_TEXT
        : ''),
    'CURRENT_TS': parseInt(arParams.CURRENT_TS),
    'SEARCH_PAGE': (typeof arParams.SEARCH_PAGE != 'undefined'
  ...
  this.MakeResultFromClientDB = function(
      arSearchStringAlternatives, searchStringOriginal) {
    var result = null;
    ...
    for (key = 0; key < arSearchStringAlternatives.length; key++) {
      ...
      if (
        ...
        for (i = 0; i <
        _this.ITEMS.oDbUserSearchResult[searchString].length; i++) {
          ...
          for (j = 0; j < _this.arParams.CATEGORIES_ALL.length; j++) {
            if (
                typeof _this.arParams.CATEGORIES_ALL[j].CLIENTDB_PREFIX !=
                'undefined'
              ...
              if (prefix == 'U') {
                result.CATEGORIES[j].ITEMS.push({
                  ICON: (typeof _this.ITEMS.obClientDbData.users[entityCode].avatar !=
                  'undefined'
                      ? _this.ITEMS.obClientDbData.users[entityCode].avatar
                      : ''),
                  ITEM_ID: entityCode,
                  MODULE_ID: '',
                  NAME: _this.ITEMS.obClientDbData.users[entityCode].name,
                  PARAM1: '',
                  URL: _this.arParams.USER_URL.replace('#user_id#',
                      _this.ITEMS.obClientDbData.users[entityCode].entityId),
                  TYPE: 'users',
                });
              } else if (prefix == 'G') {
                if (
                    typeof _this.ITEMS.obClientDbData.sonetgroups[entityCode].site !=
                    'undefined'
                    &&
                    _this.ITEMS.obClientDbData.sonetgroups[entityCode].site ==
                    BX.message('SITE_ID')
                ) {
                  result.CATEGORIES[j].ITEMS.push({
                    ICON: (typeof _this.ITEMS.obClientDbData.sonetgroups[entityCode].avatar !=
                    'undefined'
                        ? _this.ITEMS.obClientDbData.sonetgroups[entityCode].avatar
                        : ''),
                    ITEM_ID: entityCode,
                    MODULE_ID: '',
                    NAME: _this.ITEMS.obClientDbData.sonetgroups[entityCode].name,
                    PARAM1: '',
                    URL: _this.arParams.GROUP_URL.replace('#group_id#',
                        _this.ITEMS.obClientDbData.sonetgroups[entityCode].entityId),
                    TYPE: 'sonetgroups',
                    IS_MEMBER: (typeof _this.ITEMS.obClientDbData.sonetgroups[entityCode].isMember !=
                    'undefined' &&
                    _this.ITEMS.obClientDbData.sonetgroups[entityCode].isMember ==
                    'Y' ? 1 : 0),
                  });
                }
              } else if (prefix == 'M') {
        ...
        ITEMS: [
          {
            ...
            URL: BX.util.add_url_param(_this.arParams.SEARCH_PAGE,
                {'q': searchStringOriginal}),
          },
        ],
      };
    }
                ...
                if (
                  ...
                  itemBlock = BX.create('a', {
                    ...
                    children: [
                      BX.create('span', {
                        attrs: {
                          style: (typeof currentItem.ICON != 'undefined' &&
                          currentItem.ICON.length > 0
                              ? 'background-image: url(\'' + currentItem.ICON +
                              '\')'
                              : ''),
                        },
                        props: {
                          className: 'title-search-item-img title-search-item-img-' +
                              currentItem.TYPE,
                        },
                      }),
                      BX.create('span', {
  ...
  this.ShowResult = function(result, showWaiter) {
    ...
    if (result != null) {
      if (typeof _this.arParams.FORMAT != 'undefined' &&
          _this.arParams.FORMAT == 'json') {
    ...
    _this.RESULT.style.display = _this.RESULT.innerHTML.length > 0
        ? 'block'
        : 'none';
  };
      ...
      if (
        ...
        if (_this.arParams.CATEGORIES_ALL[i].CODE == 'custom_menuitems') {
          ...
          for (j = 0; j < result.CATEGORIES[i].ITEMS.length; j++) {
            ajaxDbEntities[result.CATEGORIES[i].ITEMS[j].ITEM_ID] = _this.ConvertAjaxToClientDB(
                result.CATEGORIES[i].ITEMS[j], 'menuitems');
          }
          BX.onCustomEvent(_this, 'onFinderAjaxSuccess',
              [ajaxDbEntities, _this.ITEMS, 'menuitems']);
        } else if (_this.arParams.CATEGORIES_ALL[i].CODE ==
            'custom_sonetgroups') {
          ajaxDbEntities = {};
          for (j = 0; j < result.CATEGORIES[i].ITEMS.length; j++) {
            ajaxDbEntities[result.CATEGORIES[i].ITEMS[j].ITEM_ID] = _this.ConvertAjaxToClientDB(
                result.CATEGORIES[i].ITEMS[j], 'sonetgroups');
          }
          BX.onCustomEvent(_this, 'onFinderAjaxSuccess',
              [ajaxDbEntities, _this.ITEMS, 'sonetgroups']);
        } else if (_this.arParams.CATEGORIES_ALL[i].CODE == 'custom_users') {
          ajaxDbEntities = {};
          for (j = 0; j < result.CATEGORIES[i].ITEMS.length; j++) {
            ajaxDbEntities[result.CATEGORIES[i].ITEMS[j].ITEM_ID] = _this.ConvertAjaxToClientDB(
                result.CATEGORIES[i].ITEMS[j], 'users');
          }
          BX.onCustomEvent(_this, 'onFinderAjaxSuccess',
              [ajaxDbEntities, _this.ITEMS, 'users']);
        }
      }
    }
  };
  ...
  this.ConvertAjaxToClientDB = function(oEntity, entity) {
    ...
    if (entity == 'sonetgroups') {
      result = {
        ...
        isMember: (typeof oEntity.IS_MEMBER != 'undefined' && oEntity.IS_MEMBER
            ? 'Y'
            : 'N'),
      };
  ...
  this.onKeyPress = function(keyCode) {
    ...
    switch (keyCode) {
      ...
      case 40: // down key - navigate down on search results
        ...
        for (i = 0; i < cnt; i++) {
          if (
              !BX.findChild(tbl.rows[i], {'class': 'title-search-separator'},
                  true)
              &&
  ...
  this.onKeyUp = function(event) {
    ...
    if (text.length >= 1) {
      ...
      if (_this.cache[_this.cache_key] == null) {
        ...
        var result = _this.MakeResultFromClientDB(arSearchStringAlternatives,
            text);
          ...
          for (var code in obClientDbData[key]) {
            if (obClientDbData[key].hasOwnProperty(code)) {
              ...
              if (
                  ...
                  && _this.arParams.CURRENT_TS >
                  (parseInt(firstItem.timestamp) + delta)
              ) {
                bNeedToClear = true;
              }
              break;
        ...
        BX.ajax({
          ...
          onsuccess: BX.delegate(function(data) {
            if (typeof data.ALLENTITIES != 'undefined') {
              BX.onCustomEvent('onFinderAjaxLoadAll',
                  [data.ALLENTITIES, this.ITEMS, params.entity]);
            }
            params.callback();
          }, this),
