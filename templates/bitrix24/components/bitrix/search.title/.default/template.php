...
Extension::load('intranet.search_title');
...
$className = ! isModuleInstalled("timeman")
|| (CModule::IncludeModule("bitrix24")
    && SITE_ID == "ex") ? " timeman-simple" : "";
?>

<div class="header-search<?= $className ?> header-search-empty">
    <div class="header-search-inner">
        <form class="header-search-form" method="get" name="search-form"
              onsubmit="return false;" action="<? //=$arResult["FORM_ACTION"]?>"
              id="<?= $containerId ?>">
            <input
                    class="header-search-input" name="q" id="<?= $inputId ?>"
...
<script type="text/javascript">
  ...
  new BX.Intranet.SearchTitle({
    ...
    'USER_URL': '<?=CUtil::JSEscape(Option::get('socialnetwork', 'user_page',
            SITE_DIR.'company/personal/', SITE_ID).'user/#user_id#/')?>',
    'GROUP_URL': '<?=CUtil::JSEscape(Option::get('socialnetwork',
            'workgroups_page', SITE_DIR.'workgroups/', SITE_ID)
        .'group/#group_id#/')?>',
    ...
    'MORE_USERS_URL': '<?=SITE_DIR
    ."company/?apply_filter=Y&with_preset=Y&FIND="?>',
    'IS_CRM_INSTALLED': '<?=IsModuleInstalled("crm") ? "Y" : "N"?>'
    //'SEARCH_PAGE': '<?=CUtil::JSEscape(str_replace("#SITE_DIR#", SITE_DIR,
          $arParams["PAGE"]))?>'
  });
</script>
<? $frame->end(); ?>
