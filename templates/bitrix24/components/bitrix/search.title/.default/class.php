...
//		$searchString = str_replace('%', '', $searchString)."%";
        ...
        $userPageURLTemplate = Option::get('socialnetwork', 'user_page',
                SITE_DIR.'company/personal/', SITE_ID).'user/#user_id#/';
        ...
        if ( ! empty($searchString)) {
            $matchesPhones = [];
            $phoneParserManager = Parser::getInstance();
            preg_match_all('/'.$phoneParserManager->getValidNumberPattern()
                .'/i', $searchString, $matchesPhones);
            ...
            if (
                ...
                foreach ($matchesPhones[0] as $phone) {
                    $convertedPhone = Parser::getInstance()
                        ->parse($phone)
                        ->format(Format::E164);
                    $searchString = str_replace($phone, $convertedPhone,
                        $searchString);
                }
            }

            $findFilter = UserUtils::getAdminSearchFilter([
                'FIND' => $searchString,
            ]);
            if ( ! empty($findFilter)) {
        ...
        if (
            ...
            $myGroupUsersList
                = CExtranet::getMyGroupsUsersSimple(CExtranet::getExtranetSiteId());
        ...
        */
        ...
        while ($user = $res->fetch()) {
            ...
            $result[] = array(
                ...
                'URL'          => str_replace('#user_id#', $user['ID'],
                    $userPageURLTemplate),
                'MODULE_ID'    => '',
                'PARAM1'       => '',
                'ID'           => $user['ID'],
    ...
    final public static function getSonetGroups($searchString = false)
    {
        ...
        if ($extranetIncluded === null) {
            ...
            $extranetSiteId = ($extranetIncluded
                ? CExtranet::getExtranetSiteID() : false);
            $extranetUser = ($extranetIncluded ? ! CExtranet::isIntranetUser()
                : false);
        }

        $groupPageURLTemplate = Option::get('socialnetwork', 'workgroups_page',
                SITE_DIR.'workgroups/', SITE_ID).'group/#group_id#/';
        ...
        if (empty($searchString)) {
            $cacheTtl = 3153600;
            $cacheId = 'search_title_sonetgroups_'.md5(serialize($groupFilter)
                    .$extranetSiteId.$groupPageURLTemplate);
        ...
        if ($cacheResult === false) {
            ...
            foreach ($groupList as $group) {
                ...
                $result[] = array(
                    ...
                    'URL'         => str_replace('#group_id#', $group['ID'],
                        $groupPageURLTemplate),
                    'MODULE_ID'   => '',
                    'PARAM1'      => '',
                    'ITEM_ID'     => 'G'.$group['ID'],
                    'ICON'        => empty($image['src']) ? '' : $image['src'],
            ...
            if ($obCache) {
                if (defined("BX_COMP_MANAGED_CACHE")) {
                    ...
                    $CACHE_MANAGER->RegisterTag("sonet_user2group_U"
                        .$USER->getID());
    ...
    final public static function convertAjaxToClientDb($arEntity, $entityType)
    {
        ...
        if ($entityType == 'sonetgroups') {
            $result = array(
                ...
                'name'       => $arEntity["NAME"],
                'avatar'     => empty($arEntity['ICON']) ? ''
                    : $arEntity['ICON'],
                'desc'       => empty($arEntity['DESCRIPTION']) ? ''
                    : (TruncateText($arEntity['DESCRIPTION'], 100)),
                'isExtranet' => ($arEntity['IS_EXTRANET'] ? "Y" : "N"),
                'site'       => $arEntity['SITE'],
                'isMember'   => (isset($arEntity['IS_MEMBER'])
                && $arEntity['IS_MEMBER'] ? "Y" : "N"),
            );
    ...
    final public static function getMenuItems($searchString = false)
    {
        ...
        $arMenuResult = $APPLICATION->IncludeComponent(
            ...
            "left_vertical",
            array(
                "ROOT_MENU_TYPE"        => isModuleInstalled("bitrix24")
                    ? "superleft" : "top",
                "MENU_CACHE_TYPE"       => "Y",
        ...
        foreach ($arMenuResult as $menuItem) {
            ...
            if (
                empty($searchString)
                || mb_strpos(ToLower($menuItem['TEXT']), ToLower($searchString))
                !== false
            ) {
                $result[] = array(
                    'NAME'      => $menuItem['TEXT'],
                    'URL'       =>
                        isset($menuItem['PARAMS'])
                        && isset($menuItem['PARAMS']["real_link"])
                            ?
                            $menuItem['PARAMS']["real_link"]
                            :
                            $menuItem['LINK'],
                    'CHAIN'     => (! empty($menuItem['CHAIN'])
                    && is_array($menuItem['CHAIN']) ? $menuItem['CHAIN']
                        : array($menuItem['TEXT'])),
                    'MODULE_ID' => '',
                    'PARAM1'    => '',
    ...
    final public static function customSearch(
        $searchString,
        $arParams,
        &$arResult
    ) {
        ...
        if ($bExtranetSite === null) {
            $bExtranetSite = (Loader::includeModule('extranet')
            ...
            if (mb_strpos($categoryCode, 'custom_') === 0) {
                ...
                if (
                    ...
                    $arResult["CATEGORIES"][$i]["ITEMS"]
                        = CB24SearchTitle::getUsers($searchString);

                    if ($arResult["customResultEmpty"]
                    ...
                    foreach (
                        $arResult["CATEGORIES"][$i]["ITEMS"] as $key => $arItem
                    ) {
                        $clientDbItem
                            = CB24SearchTitle::convertAjaxToClientDb($arItem,
                            'users');
                        $arResult["CATEGORIES"][$i]["ITEMS"][$key]['CHECKSUM']
                            = $clientDbItem['checksum'];
                    }
                } elseif (
                    ...
                    $arResult["CATEGORIES"][$i]["ITEMS"]
                        = CB24SearchTitle::getMenuItems($searchString);
