    ...
    foreach ($res as $key => $tags) {
        ...
        if ( ! empty($tags)) {
            ...
            $result = array(
                ...
                "TAG_PATH"    => $APPLICATION->GetCurPageParam("tags="
                    .urlencode(implode(",", $url)), array("tags")),
                "TAG_WITHOUT" => $APPLICATION->GetCurPageParam((count($url_without)
                > 0 ? "tags=".urlencode(implode(",", $url_without)) : ""),
                    array("tags")),
            );
...
if (IsModuleInstalled('socialnetwork')) {
    global $MESS;
    include($_SERVER["DOCUMENT_ROOT"].$this->GetFolder()."/lang/".LANGUAGE_ID
        ."/result_modifier.php");
    ...
    $arParams["PATH_TO_SONET_MESSAGES_CHAT"]
        = trim($arParams["PATH_TO_SONET_MESSAGES_CHAT"]);
...
foreach ($arResult["SEARCH"] as $i => $arItem) {
    ...
    switch ($arItem["MODULE_ID"]) {
        ...
        case "iblock":
            if (mb_substr($arItem["ITEM_ID"], 0, 1) === "G") {
                ...
                if ( ! array_key_exists($arItem["PARAM2"], $arIBlocks)) {
                    $arIBlocks[$arItem["PARAM2"]]
                        = CIBlock::GetArrayByID($arItem["PARAM2"]);
                }
                ...
                if (mb_substr($arItem["ITEM_ID"], 0, 1) !== "S") {
                    ...
                    if ($arElement
                        && $arElement["PROPERTY_DOC_TYPE_ENUM_ID"] <> ''
                    ) {
                        $arEnum
                            = CIBlockPropertyEnum::GetByID($arElement["PROPERTY_DOC_TYPE_ENUM_ID"]);
                        if ($arEnum && $arEnum["XML_ID"]) {
                            if (file_exists($abs_path."iblock_doc_type_"
                                .mb_strtolower($arEnum["XML_ID"]).".gif")
                            ) {
                                $file = "iblock_doc_type_"
                                    .mb_strtolower($arEnum["XML_ID"]).".gif";
                            }
                        }
                    }
                    ...
                    if ( ! $file) {
                        $rsSection
                            = CIBlockElement::GetElementGroups($arItem["ITEM_ID"],
                            true);
                ...
                if ( ! $file && $SECTION_ID) {
                    ...
                    if ($arSection = $rsSection->Fetch()) {
                        if (mb_strlen($arSection["CODE"])
                            && file_exists($abs_path."iblock_section_"
                                .mb_strtolower($arSection["CODE"]).".gif")
                        ) {
                            $file = "iblock_section_"
                                .mb_strtolower($arSection["CODE"]).".gif";
                        } elseif (file_exists($abs_path."iblock_section_"
                            .mb_strtolower($arSection["ID"]).".gif")
                        ) {
                            $file = "iblock_section_"
                                .mb_strtolower($arSection["ID"]).".gif";
                        } elseif (mb_strlen($arSection["XML_ID"])
                            && file_exists($abs_path."iblock_section_"
                                .mb_strtolower($arSection["XML_ID"]).".gif")
                        ) {
                            $file = "iblock_section_"
                                .mb_strtolower($arSection["XML_ID"]).".gif";
                        }
                ...
                if ( ! $file
                    && preg_match("/\\.([a-z]+?)$/i", $arItem["TITLE"], $match)
                ) {
                    if (file_exists($abs_path."iblock_type_"
                        .mb_strtolower($arIBlocks[$arItem["PARAM2"]]["IBLOCK_TYPE_ID"])
                        ."_".$match[1].".gif")
                    ) {
                        $file = "iblock_type_"
                            .mb_strtolower($arIBlocks[$arItem["PARAM2"]]["IBLOCK_TYPE_ID"])
                ...
                if ( ! $file) {
                    if (mb_strlen($arIBlocks[$arItem["PARAM2"]]["CODE"])
                        && file_exists($abs_path."iblock_iblock_"
                            .mb_strtolower($arIBlocks[$arItem["PARAM2"]]["CODE"])
                            .".gif")
                    ) {
                        $file = "iblock_iblock_"
                            .mb_strtolower($arIBlocks[$arItem["PARAM2"]]["CODE"])
                        ...
                        .mb_strtolower($arIBlocks[$arItem["PARAM2"]]["ID"])
                        .".gif")
                    ) {
                        $file = "iblock_iblock_"
                            .mb_strtolower($arIBlocks[$arItem["PARAM2"]]["ID"])
                            .".gif";
                    } elseif (mb_strlen($arIBlocks[$arItem["PARAM2"]]["XML_ID"])
                        && file_exists($abs_path."iblock_iblock_"
                            .mb_strtolower($arIBlocks[$arItem["PARAM2"]]["XML_ID"])
                            .".gif")
                    ) {
                        $file = "iblock_iblock_"
                            .mb_strtolower($arIBlocks[$arItem["PARAM2"]]["XML_ID"])
        ...
        case "main":
            ...
            break;
        case "blog":
            if (mb_substr($arItem["ITEM_ID"], 0, 1) === "P"
                && file_exists($abs_path."blog_post.gif")
            ) {
                $file = "blog_post.gif";
            } elseif (mb_substr($arItem["ITEM_ID"], 0, 1) === "U"
                && file_exists($abs_path."blog_user.gif")
            ) {
                $file = "blog_user.gif";
            }
            break;
        case "forum":
            ...
            break;
        case "intranet":
            if (mb_substr($arItem["ITEM_ID"], 0, 1) === "U"
                && file_exists($abs_path."intranet_user.gif")
            ) {
...
if (CModule::IncludeModule('intranet')) {
    ...
    $structure_iblock_id = COption::GetOptionInt("intranet", "iblock_structure",
        0);
    if ($structure_iblock_id > 0) {
        $arIBlock = CIBlock::GetArrayByID($structure_iblock_id);
        if ($arIBlock) {
            $arResult["STRUCTURE_PAGE"]
                = CIBlock::ReplaceDetailURL($arIBlock["LIST_PAGE_URL"],
                $arIBlock, true);
        }
    }
    ...
    foreach ($arResult["SEARCH"] as $i => $arItem) {
        if ($arItem["MODULE_ID"] === "intranet"
            && mb_substr($arItem["ITEM_ID"], 0, 1) === "U"
        ) {
            $rsUser = CUser::GetList(($by = ''), ($ord = ''),
                array("ID_EQUAL_EXACT" => mb_substr($arItem["ITEM_ID"], 1),),
            ...
            if ($arUser) {
                if ($arUser['PERSONAL_PHOTO']) {
                    ...
                    $arImage
                        = CIntranetUtils::InitImage($arUser['PERSONAL_PHOTO'],
                        100);
                ...
                if (is_array($arUser['UF_DEPARTMENT'])
                    ...
                    $arNewDep = array_diff($arUser['UF_DEPARTMENT'],
                        $arDepCache);

                    if (count($arNewDep) > 0) {
                        $dbRes = CIBlockSection::GetList(array('SORT' => 'ASC'),
                            array('ID' => $arNewDep));
                ...
                $arUser['IS_ONLINE'] = $bSoNet
                    && CSocNetUser::IsOnLine($arUser['ID']);

                if ($arUser['PERSONAL_BIRTHDAY']) {
                    $arBirthDate = ParseDateTime($arUser['PERSONAL_BIRTHDAY'],
                        CSite::GetDateFormat('SHORT'));
                    $arUser['IS_BIRTHDAY'] = (intval($arBirthDate['MM'])
                            == date('n'))
                        && (intval($arBirthDate['DD']) == date('j'));
                }

                $arUser['IS_FEATURED']
                    = CIntranetUtils::IsUserHonoured($arUser['ID']);
                $arUser['IS_ABSENT']
                    = CIntranetUtils::IsUserAbsent($arUser['ID']);
    ...
    $arRes = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("USER", 0,
        LANGUAGE_ID);
    if ( ! empty($arRes)) {
        foreach ($arRes as $key => $val) {
            $arResult['USER_PROP'][$val["FIELD_NAME"]]
                = ($val["EDIT_FORM_LABEL"] <> '' ? $val["EDIT_FORM_LABEL"]
                : $val["FIELD_NAME"]);
        }
    }
}
