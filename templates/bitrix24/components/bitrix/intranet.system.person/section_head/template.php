			...
			<span class="department-manager-name-block">
				<div class="department-manager-name">
					<a href="<?= $arUser['DETAIL_URL'] ?>"
                       class="department-manager-name-link"><?= $arUser["FORMATTED_NAME"] ?></a><? if ($arResult['CAN_EDIT_USER']
                        || $arUser["ACTIVITY_STATUS"] == "active"
                    ): ?><span
                        onclick="user_action_menu<?= $user_action_menu_number ?>(this,<?= $arUser['ID']
                        .rand() ?>,<?= $arUser["ID"] ?>)"
                        class="employee-user-action"></span><? endif ?>
				</div>
				...
				<? if ($arUser["PERSONAL_MOBILE"]): ?>
                    <div class="department-manager-tel"><?= GetMessage("ISL_PERSONAL_PHONE") ?>
                    : <?= $arUser["PERSONAL_MOBILE"] ?></div><? endif ?>
            <? if ($arUser["UF_SKYPE"]): ?>
                <div class="department-manager-tel"><?= GetMessage("ISL_PERSONAL_SKYPE") ?>
                :
                <a href="callto:<?= $arUser["UF_SKYPE"] ?>"><?= $arUser["UF_SKYPE"] ?></a>
                </div><? endif ?>
            <? if ($arUser["EMAIL"]): ?>
                <div class="department-manager-tel"><?= GetMessage("ISL_PERSONAL_EMAIL") ?>
                :
                <a href="mailto:<?= $arUser["EMAIL"] ?>"><?= $arUser["EMAIL"] ?></a>
                </div><? endif ?>
			</span>
		</span>
</div>
