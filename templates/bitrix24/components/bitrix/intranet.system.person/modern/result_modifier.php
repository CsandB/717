...
if ( ! IsModuleInstalled("bitrix24")
    ...
    $arResult['CAN_EDIT_USER'] = $arResult['CAN_EDIT_USER']
        && CSocNetUser::IsCurrentUserModuleAdmin();
}

if (intval($arParams['USER']["PERSONAL_PHOTO_SOURCE"]) > 0) {
    $imageFile
        = CFile::GetFileArray($arParams['USER']["PERSONAL_PHOTO_SOURCE"]);
    if ($imageFile !== false) {
        ...
        $arParams['USER']["PERSONAL_PHOTO_SOURCE"]
            = $arParams['~USER']["PERSONAL_PHOTO_SOURCE"] = $arFileTmp["src"];
    } else {
        $arParams['USER']["PERSONAL_PHOTO_SOURCE"]
            = $arParams['~USER']["PERSONAL_PHOTO_SOURCE"] = "";
    }
...
if ( ! isset($arParams["USER"]["DETAIL_URL"])) {
    $detailURL = COption::GetOptionString('intranet', 'search_user_url',
        '/user/#ID#/');
    if ( ! $detailURL) {
    ...
    $arParams["USER"]["DETAIL_URL"] = str_replace(array('#ID#', '#USER_ID#'),
        $arParams["USER"]['ID'], $detailURL);
}
