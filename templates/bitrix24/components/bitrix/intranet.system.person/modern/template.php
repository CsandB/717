...
$name = CUser::FormatName(str_replace(array("#NOBR#", "#/NOBR#"), "",
    $arParams['NAME_TEMPLATE']), $arUser, $arResult["bUseLogin"], false);
...
<tr id="tr_<?= $arUser["ID"] ?>">
    <td class="employee-table-info">
        <div class="employee-info-block <? if ( ! in_array('PERSONAL_PHOTO',
            $arParams['USER_PROPERTY'])
        ): ?> no-photo<? endif ?><? if ( ! $arUser['IS_ONLINE']): ?> employee-state-offline<? endif ?>">
            <? if (in_array('PERSONAL_PHOTO', $arParams['USER_PROPERTY'])): ?>
                <div class="user-avatar user-default-avatar"
                     <? if ($arUser['PERSONAL_PHOTO_SOURCE']): ?>style="background: url('<?= $arUser['PERSONAL_PHOTO_SOURCE'] ?>') no-repeat center center; background-size: cover;"<? endif; ?>></div>
            <? endif ?>
            <div class="employee-name<?= ($arUser["EXTRANET"]
                ? ' employee-name-extranet' : '') ?>"><a
                        class="employee-name-link"
                        href="<?= $arUser['DETAIL_URL'] ?>"><?= $name ?></a><? if ($arResult['CAN_EDIT_USER']
                    || $arUser["ACTIVITY_STATUS"] != "inactive"
                ): ?><span class="employee-user-action"
                           onclick="user_action_menu<?= $user_action_menu_number ?>(this,<?= $arUser['ID']
                           .rand() ?>, <?= $arUser['ID'] ?>, '<?= ($arUser["EXTRANET"]
                               ? "1" : "0") ?>')"></span><? endif ?></div>
            <div class="employee-post"><?= $arUser['WORK_POSITION'] ?></div>
            <div class="employee-state">
                <?= ($arUser['IS_ONLINE'] ? GetMessage('INTR_ISP_IS_ONLINE')
                    : GetMessage('INTR_ISP_IS_OFFLINE')); ?><? if ($arUser['IS_ABSENT']): ?> (<?= GetMessage('INTR_ISP_IS_ABSENT'); ?>)<? endif ?>
            </div>
            <? if ($arParams["LIST_MODE"] == "all"
                && in_array($arUser["ACTIVITY_STATUS"],
                    array("fired", "extranet", "inactive"))
            ): ?>
                <span class="employee-dept-post employee-dept-<?= $arUser["ACTIVITY_STATUS"] ?>"><?= GetMessage("INTR_USER_"
    ...
    <? if ($arResult['CAN_EDIT_USER']): ?>
        <td>
            <? if ( ! in_array($arUser['EXTERNAL_AUTH_ID'],
                array('bot', 'imconnector'))
            ): ?>
                <div style="margin-right: 40px;">
                    <a href="<?= CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_USER_EDIT'],
                        array("user_id" => $arUser['ID'])) ?>"><?= GetMessage("INTR_ISP_EDIT_LINK") ?></a>
                </div>
            <? endif ?>
        </td>
    <? endif ?>
    ...
    <td class="employee-table-phone">
        ...
        foreach ($arUserData as $key => $value) {
            ...
            echo $arParams['USER_PROP'][$key] ? $arParams['USER_PROP'][$key]
                : GetMessage('ISL_'.$key); ?>:
            <? switch ($key) {
                case 'EMAIL':
                ...
                case 'PERSONAL_GENDER':
                    echo $value == 'F'
                        ? GetMessage('INTR_ISP_GENDER_F')
                        : ($value == 'M' ? GetMessage('INTR_ISP_GENDER_M')
                            : '');
                    break;
                ...
                case 'PERSONAL_BIRTHDAY':
                    echo FormatDateEx(
                        $value,
                        false,
                        $arParams['DATE_FORMAT'.(($arParams['SHOW_YEAR'] == 'N'
                            || $arParams['SHOW_YEAR'] == 'M'
                            && $arUser['PERSONAL_GENDER'] == 'F') ? '_NO_YEAR'
                            : '')]
                    );
                        ...
                        foreach ($value as $dept_id => $dept_name) {
                            ...
                            if (CModule::IncludeModule('extranet')
                                ...
                                if (trim($arParams["PATH_TO_CONPANY_DEPARTMENT"])
                                    ...
                                    echo '<a href="', CComponentEngine::MakePathFromTemplate($arParams["~PATH_TO_CONPANY_DEPARTMENT"],
                                        array("ID" => $dept_id)), '">', htmlspecialcharsbx($dept_name), '</a>';
                                } else {
                                    echo '<a href="', $arParams['STRUCTURE_PAGE']
                                        .'?set_filter_', $arParams['STRUCTURE_FILTER'], '=Y&', $arParams['STRUCTURE_FILTER'], '_UF_DEPARTMENT=', $dept_id, '">', htmlspecialcharsbx($dept_name), '</a>';
                                }
                            }
                ...
                default:
                    if (mb_substr($key, 0, 3) == 'UF_'
                        && is_array($arResult['USER_PROP'][$key])
                    ) {
    ...
    <td class="employee-table-dept">
        ...
        foreach ($arUser["UF_DEPARTMENT"] as $dep_id => $dep_name) {
            ...
            if (CModule::IncludeModule('extranet')
                ...
                if (trim($arParams["PATH_TO_CONPANY_DEPARTMENT"]) <> '') {
                    echo '<a href="', CComponentEngine::MakePathFromTemplate($arParams["~PATH_TO_CONPANY_DEPARTMENT"],
                        array("ID" => $dep_id)), '">', $dep_name, '</a>';
                } else {
                    echo '<a href="', $arParams['STRUCTURE_PAGE']
                        .'?set_filter_', $arParams['STRUCTURE_FILTER'], '=Y&', $arParams['STRUCTURE_FILTER'], '_UF_DEPARTMENT=', $dep_id, '">', $dep_name, '</a>';
                }
                echo "<br>";
            }
            if (array_key_exists($dep_id, $arUser["DEP_HEAD"])):?>
                <span class="employee-dept-post"><?= GetMessage("INTR_IS_HEAD") ?></span>
                <br/>
            <?endif;
        }
...
$userActionHref
    = CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_USER_EDIT'],
        array("user_id" => $arUser['ID']))."?ACTIVE=".$userActive;
if ($arUser["ACTIVITY_STATUS"] == "fired") {
...
?>
<script>
  function user_action_menu<?=$user_action_menu_number?> (
      button, number, user_id, is_extranet) {

    var popupUserMenuItems = [
        <?if ($arUser["ACTIVITY_STATUS"] == "active"
    || $arUser["ACTIVITY_STATUS"] == "extranet"):?>
        <?if (CBXFeatures::IsFeatureEnabled("Tasks")):?>
      {
        text: "<?=GetMessage("INTR_ISP_TASK")?>",
        onclick: function() {
          this.popupWindow.close();
          taskIFramePopup.add({RESPONSIBLE_ID: user_id});
        },
      },
        <?endif?>
        <?if ($arResult["CAN_MESSAGE"]):?>
      {
        text: "<?=GetMessage("INTR_ISP_PM")?>",
        onclick: function() {
          if (BX.IM) {
            BXIM.openMessenger(user_id);
            return false;
          } else {
            window.open('<?echo $url ?>', '',
                'status=no,scrollbars=yes,resizable=yes,width=700,height=550,top=' +
                Math.floor((screen.height - 550) / 2 - 14) + ',left=' +
                Math.floor((screen.width - 700) / 2 - 5));
            return false;
          }
        },
      },
    ...
    && ( ! IsModuleInstalled("bitrix24")
        ...
        && CModule::IncludeModule('bitrix24'))):?>
      {
        text: "<?=GetMessage("INTR_ISP_INVITE")?>", onclick: function() {
          var myBX = (window.BX ? window.BX : (window.top.BX
              ? window.top.BX
              : null));
          ...
          BX.ajax.post(
              ...
              BX.delegate(function(result) {
                    ...
                    var InviteAccessPopup = BX.PopupWindowManager.create(
                        'invite_access' + number, button, {
        ...
        <?endif;?>
        <?if ($arUser["ACTIVITY_STATUS"] != "inactive"
    && CBXFeatures::IsFeatureEnabled("WebMessenger")
    && $arUser["ID"] != $USER->GetID()):?>
      {
        text: "<?=GetMessage("INTR_ISP_MESSAGE_HISTORY")?>",
        onclick: function() {
          this.popupWindow.close();
          BXIM.openHistory(user_id);
        },
      },
        <?endif;?>
    ...
    && ! in_array($arUser['EXTERNAL_AUTH_ID'], array('bot', 'imconnector'))):?>
      {
            ...
            BX.ajax({
              ...
              onsuccess: function(json) {
                ...
                if (json.error) {
                  ...
                  var DeleteErrorPopup = BX.PopupWindowManager.create(
                      'delete_error' + number, button, {
        ...
        <?endif;?>
        <?if ($arResult['CAN_EDIT_USER'] || $arUser["ID"] == $USER->GetID()
    and ! in_array($arUser['EXTERNAL_AUTH_ID'], array('bot', 'imconnector'))):?>
      {
        text: "<?=GetMessage("INTR_ISP_EDIT_USER")?>",
        href: "<?=CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_USER_EDIT'],
    ...
    if (BXIM && BXIM.checkCallSupport()) {
      popupUserMenuItems.push({
        text: "<?=GetMessage("INTR_ISP_VIDEO_CALL")?>",
        onclick: function() {
          if (BXIM) {
            BXIM.callTo(user_id);
            return false;
