...
include_once($_SERVER['DOCUMENT_ROOT']
    .'/bitrix/components/bitrix/desktop/include.php');

$col = 0;
$arDiff = array_diff($arParams["GADGETS_FIXED"], $arResult["GADGETS_LIST"]);

if ( ! empty($arDiff)) {
    if (array_key_exists("DEFAULT_ID", $arParams)
        && trim($arParams["DEFAULT_ID"]) <> ''
    ) {
        $arUserOptionsDefault = CUserOptions::GetOption("intranet",
            "~gadgets_".$arParams["DEFAULT_ID"], false, 0);
    } else {
        $arUserOptionsDefault = false;
    }

    $arUserOptions = CUserOptions::GetOption("intranet",
        "~gadgets_".$arParams["ID"], $arUserOptionsDefault, 99999999);

    if (is_array($arUserOptions) && array_key_exists("GADGETS", $arUserOptions)
        ...
        foreach ($arUserOptions["GADGETS"] as $key => $arGadgetDefault) {
            ...
            if (in_array($gadget_id, $arDiff)) {
                ...
                if ( ! array_key_exists("CAN_BE_FIXED", $arGadgetDefaultAll)
                    || ! $arGadgetDefaultAll["CAN_BE_FIXED"]
                ) {
                    continue;
                }

                $arGadgetDefault = array_merge($arGadgetDefault,
                    $arGadgetDefaultAll);
                $arGadgetDefault["TITLE"]
                    = htmlspecialcharsbx($arGadgetDefault["NAME"]);
                ...
                foreach ($arParams as $id => $p) {
                    ...
                    if (mb_strpos($id, $pref) === 0
                        && ! isset($arGadgetParams[mb_substr($id,
                                mb_strlen($pref))])
                    ) {
                        $arGadgetParams[mb_substr($id, mb_strlen($pref))] = $p;
                    }
                ...
                $arGadgetDefault["SETTINGS"] = array_merge($arGadgetParams,
                    $arGadgetDefault["SETTINGS"]);
                $arGadgetDefault["GADGET_ID"] = $arGadgetDefault["ID"];
                $arGadgetDefault["CONTENT"]
                    = BXGadget::GetGadgetContent($arGadgetDefault, $arParams);
