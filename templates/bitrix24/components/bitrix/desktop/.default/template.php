...
if ( ! defined("BX_GADGET_DEFAULT")) {
    ...
    if ($arResult["PERMISSION"] > "R") {
        ?>
        <script type="text/javascript"
                src="/bitrix/components/bitrix/desktop/script.js?v=<?= filemtime($_SERVER['DOCUMENT_ROOT']
                    .'/bitrix/components/bitrix/desktop/script.js'); ?>"></script><?
    }
}
...
if ($arResult["PERMISSION"] > "R") {
    ...
    foreach ($arResult['ALL_GADGETS'] as $gd) {
        $allGD[] = array(
            'ID' => $gd["ID"],
            'TEXT' =>
                '<div style="text-align: left;">'.($gd['ICON1'] ? '<img src="'
                    .($gd['ICON']).'" align="left">' : '').
                '<b>'.(htmlspecialcharsbx($gd['NAME'])).'</b><br>'
                .(htmlspecialcharsbx($gd['DESCRIPTION'])).'</div>',
        );
    }
	...
	<div class="sidebar-buttons">
		<a href="" class="sidebar-button" onclick="getGadgetHolder(\''
        .AddSlashes($arResult["ID"]).'\').ShowAddGDMenu(this);return false;">
			<span class="sidebar-button-top"><span class="corner left"></span><span class="corner right"></span></span>
			<span class="sidebar-button-content"><span class="sidebar-button-content-inner"><i class="sidebar-button-create"></i><b>'
        .GetMessage("CMDESKTOP_TDEF_ADD").'</b></span></span>
    ...
    if ($arResult["PERMISSION"] > "W") {
        ...
        $gadgetButtons .= '<a href="" class="sidebar-button" onclick="getGadgetHolder(\''
            .AddSlashes($arResult["ID"]).'\').SetForAll('.$mode.') return false;">
			<span class="sidebar-button-top"><span class="corner left"></span><span class="corner right"></span></span>
			<span class="sidebar-button-content"><span class="sidebar-button-content-inner"><i class="sidebar-button-accept"></i><b>'
            .GetMessage("CMDESKTOP_TDEF_SET").'</b></span></span>
			<span class="sidebar-button-bottom"><span class="corner left"></span><span class="corner right"></span></span></a>';
    }

    $gadgetButtons .= '<a href="" class="sidebar-button" onclick="getGadgetHolder(\''
        .AddSlashes($arResult["ID"]).'\').ClearUserSettings(); return false;">
		<span class="sidebar-button-top"><span class="corner left"></span><span class="corner right"></span></span>
		<span class="sidebar-button-content"><span class="sidebar-button-content-inner"><i class="sidebar-button-delete"></i><b>'
        .GetMessage("CMDESKTOP_TDEF_CLEAR").'</b></span></span>
		<span class="sidebar-button-bottom"><span class="corner left"></span><span class="corner right"></span></span></a>';
...
if ($arResult["PERMISSION"] > "W") {
    $gadgetButtons = '<table id="tdesktop-actions" class="data-table-gadget" style="margin-bottom:0;"><tr><td><div class="gdparent">
		<div class="gdheader" style="cursor:move;" onmousedown="return getGadgetHolder(\''
        .$arResult["ID"]
        .'\').DragStart(\'desktop-actions\', event)"><div class="gdheader-title">'
        .GetMessage("CMDESKTOP_DESC_NAME").'</div></div>
    ...
    if ($USER->IsAdmin()
        && COption::GetOptionString("main", "wizard_clear_exec", "N", SITE_ID)
        != "Y"
        ...
        $gadgetButtons .= '
            ...
            .GetMessage("CMDESKTOP_DEMO_DATA_BLOCK_TITLE").'</div>
					<div class="sidebar-help-content">'
            .GetMessage("CMDESKTOP_DEMO_DATA_BLOCK_DESC", array(
                "#LINK_TO_WIZARD#" => "/bitrix/admin/wizard_install.php?lang="
                    .LANGUAGE_ID."&wizardSiteID=".SITE_ID
                    ."&wizardName=bitrix:portal_clear&".bitrix_sessid_get(),
            )).'</div>
				</div>
...
<table class="gadgetholder gadgetholder-<?= $arResult["ID"] ?>" cellspacing="0"
       cellpadding="0" width="100%" id="GDHolder_<?= $arResult["ID"] ?>">
    <tbody>
    <tr><?
        for ($i = 0; $i < $arResult["COLS"]; $i++) {
            if ($i == 0) {
                ?>
                <td class="gd-page-column<?= $i ?>" valign="top"
                    width="<?= $arResult["COLUMN_WIDTH"][$i] ?>" id="s0"><?
                if ($arResult["COLS"] == 1) {
                ...
                <td class="gd-page-column<?= $i ?>" valign="top"
                    width="<?= $arResult["COLUMN_WIDTH"][$i] ?>" id="s2"><?
                $this->SetViewTarget("sidebar", 100);
                ?><?= $gadgetButtons ?><?
                $this->EndViewTarget();
            } else {
            ...
            foreach ($arResult["GADGETS"][$i] as $arGadget) {
                ...
                if (
                    ...
                    && in_array($arGadget["GADGET_ID"],
                        $arParams["GADGETS_FIXED"])
                ...
                <table id="t<?= $arGadget["ID"] ?>"
                       class="data-table-gadget<?= ($arGadget["HIDE"] == "Y"
                           ? ' gdhided' : '') ?>">
                    <tr>
                        <td>
                            <div class="gdparent"><?
                                ...
                                <div class="gdheader" style="cursor:move;"
                                     onmousedown="return getGadgetHolder('<?= AddSlashes($arResult["ID"]) ?>').DragStart('<?= $arGadget["ID"] ?>', event)"
                                     onmouseover="this.className=this.className.replace(/\s*gdheader-hover\s*/,'') + ' gdheader-hover';"
                                     onmouseout="this.className=this.className.replace(/\s*gdheader-hover\s*/,'')">
                                    <div class="gdheader-actions"><?
                                        if ($bChangable) {
                                            ?><a
                                            class="gdsettings<?= ($arGadget["NOPARAMS"]
                                                ? ' gdnoparams' : '') ?>"
                                            href="javascript:void(0)"
                                            onclick="return getGadgetHolder('<?= AddSlashes($arResult["ID"]) ?>').ShowSettings('<?= $arGadget["ID"] ?>');"
                                            title="<?= GetMessage("CMDESKTOP_TDEF_SETTINGS") ?>"></a><?
                                        }
                                        ?><a class="gdhide"
                                             href="javascript:void(0)"
                                             onclick="return getGadgetHolder('<?= AddSlashes($arResult["ID"]) ?>').Hide('<?= $arGadget["ID"] ?>', this);"
                                             title="<?= GetMessage("CMDESKTOP_TDEF_HIDE") ?>"></a><?
                                        if ($bChangable) {
                                            ?><a class="gdremove"
                                                 href="javascript:void(0)"
                                                 onclick="return getGadgetHolder('<?= AddSlashes($arResult["ID"]) ?>').Delete('<?= $arGadget["ID"] ?>');"
                                    ...
                                    <div class="ghheader-underline"></div>
                                    <div class="gdoptions" style="display:none"
                                         id="dset<?= $arGadget["ID"] ?>"></div>
                                    <div class="gdcontent"
