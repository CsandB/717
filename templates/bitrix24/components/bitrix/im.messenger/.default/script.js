  ...
  function init() {
    ...
    BX.bind(window, 'scroll', function() {
      if (typeof (BXIM) == 'undefined' ||
          !BXIM.messenger.popupPopupMenu) return true;
      if (BX.util.in_array(BXIM.messenger.popupPopupMenu.uniquePopupId.replace(
          'bx-messenger-popup-', ''), ['createChat', 'contactList'])) {
        BXIM.messenger.popupPopupMenu.close();
      }
    });

    BX.bindDelegate(BX('bx-im-external-recent-list'), 'contextmenu',
        {className: 'bx-messenger-cl-item'}, function(e) {
          if (typeof (BXIM) == 'undefined') return false;
    ...
    BX.bindDelegate(BX('bx-im-external-recent-list'), 'click',
        {className: 'bx-messenger-cl-item'}, function(e) {
          if (typeof (BXIM) == 'undefined') return false;
    ...
    BX.addCustomEvent('onMessengerWindowBodyOverflow',
        function(messengerWindow, size) {
          var td = BX.findChildrenByClassName(BX('im-workarea-popup'),
              'bx-im-fullscreen-popup-td', true);
          for (var i = 0; i < td.length; i++) {
            var computedStyle = getComputedStyle(td[i], null);
            computedStyle = computedStyle ? parseInt(
    ...
    BX.addCustomEvent('onImUpdateCounterNotify', function(counter) {
      var notifyCounter = BX.findChildByClassName(BX('bx-im-bar-notify'),
          'bx-im-informer-num');
      if (!notifyCounter)
        return false;

      if (counter > 0) {
        notifyCounter.innerHTML = '<div class="bx-im-informer-num-digit">' +
    ...
    BX.addCustomEvent('onImUpdateCounterMessage', function(counter, type) {
      ...
      var notifyCounter = node &&
          BX.findChildByClassName(node, 'bx-im-informer-num');
      if (!notifyCounter)
        return false;
    ...
    BX.addCustomEvent('onMessengerWindowInit', function() {
      BX('im-workarea-backgound-selector-title').innerHTML = BX(
          'im-workarea-backgound-selector').options[BX(
          'im-workarea-backgound-selector').selectedIndex].text;
    });
