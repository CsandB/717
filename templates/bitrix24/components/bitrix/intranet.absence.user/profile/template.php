...
<div class="intranet-user-profile-absence">
    <div class="intranet-user-profile-absence-title"><?= Loc::getMessage("SONET_USER_ABSENCE") ?>
        :
    </div>
    <div class="intranet-user-profile-absence-value">
        ...
        foreach ($arResult['ENTRIES'] as $key => $arEntry) {
            ...
            <div class="intranet-user-profile-absence-value-item">
                <?= Loc::getMessage('INTR_IAU_TPL'.($bNow ? '_TO'
                        : '_FROM')) ?> <? echo FormatDate($DB->DateFormatToPHP(FORMAT_DATE),
                    MakeTimeStamp($arEntry['DATE_ACTIVE'.($bNow ? '_TO'
                        : '_FROM')])) ?>
