...
foreach ($arResult as $item) {
    $newItem = array(
        ...
        "COUNTER"   => isset($item["PARAMS"]["counter_num"])
        && intval($item["PARAMS"]["counter_num"])
            ? $item["PARAMS"]["counter_num"] : "",
        "IS_ACTIVE" => $item["SELECTED"],
        "IS_LOCKED" => "",
        "CLASS"     => $item["PARAMS"]["class"],
    );

    if (isset($item["PARAMS"]["action"])
        && is_array($item["PARAMS"]["action"])
    ) {
        if (isset($item["PARAMS"]["action"]["ID"])
            && $item["PARAMS"]["action"]["ID"] == "CREATE"
        ) {
...
$topMenuSectionDir
    = $APPLICATION->GetPageProperty("topMenuSectionDir"); //hack for complex component (/company/personal/ pages)
