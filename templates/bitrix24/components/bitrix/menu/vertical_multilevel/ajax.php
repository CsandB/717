    ...
    switch ($_POST["action"]) {
        ...
        case "set_app_rights":
            if (CModule::IncludeModule("bitrix24")
                ...
                $resRights = CBitrix24AppRights::SetAppRights($app_id,
                    $_POST["rights"]);
            }
            break;
        case "add_favorite":
            if (
                ...
                || isset($userOption["menu-favorites"]["show"])
                && ! in_array($menuItemID,
                    $userOption["menu-favorites"]["show"])
            ) {
                ...
                $favoriteItemsUserAdded = CUserOptions::GetOption("bitrix24",
                    $optionNameUserFavoriteItems);
                if (is_array($favoriteItemsUserAdded)
                    && ! in_array($menuItemID, $favoriteItemsUserAdded)
                    || ! is_array($favoriteItemsUserAdded)
                ) {
                    $favoriteItemsUserAdded[] = $menuItemID;
                }
                CUserOptions::SetOption("bitrix24",
                    $optionNameUserFavoriteItems, $favoriteItemsUserAdded);
            }
            break;
        case "delete_favorite":
            $arStatus = array("show", "hide");
            foreach ($arStatus as $status) {
                if (isset($userOption["menu-favorites"][$status])
                    && in_array($menuItemID,
                        $userOption["menu-favorites"][$status])
                ) {
                    $key = array_search($menuItemID,
                        $userOption["menu-favorites"][$status]);
                    ...
                    if (empty($userOption)) {
                        ...
                        CUserOptions::SetOption("bitrix24", $optionName,
                            $userOption);
                    }
                    break;
                }
        ...
        case "add_favorite_admin":
            if (
                ...
                $adminOption = COption::GetOptionString("bitrix24",
                    "admin_menu_items");
                if ($adminOption) {
        ...
        case "hide":
            if (
                ...
                || isset($userOption[$titleItemID]["hide"])
                && ! in_array($menuItemID, $userOption[$titleItemID]["hide"])
            ) {
                $userOption[$titleItemID]["hide"][] = $menuItemID;
                if ($titleItemID == "menu-favorites"
                    && ! isset($userOption[$titleItemID]["show"])
                    && isset($_POST["all_show_items"])
                ) {
                    $userOption[$titleItemID]["show"]
                        = $_POST["all_show_items"];
                }
                if (isset($userOption[$titleItemID]["show"])
                    && in_array($menuItemID, $userOption[$titleItemID]["show"])
                ) {
                    $key = array_search($menuItemID,
                        $userOption[$titleItemID]["show"]);
        ...
        case "sort_items":
            if (isset($_POST["all_title_items"])) {
                if ( ! isset($userOption[$titleItemID]["show"])) {
                    ...
                    if ( ! empty($lostItems)) {
                        $userOption[$titleItemID]["show"]
                            = array_merge($userOption[$titleItemID]["show"],
                            $lostItems);
                    }
                } else {
                    $userOption[$titleItemID]["show"]
        ...
        case "sort_sections":
            if (isset($_POST["all_sections"])
                && is_array($_POST["all_sections"])
            ) {
                CUserOptions::SetOption("bitrix24", $optionSectionsName,
                    $_POST["all_sections"]);
            } else {
                $error = "Y";
            }
