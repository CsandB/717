    ...
    function CheckMenuAdminItems(&$adminFavoriteOption, $arMenuItemsId)
    {
        if (is_array($adminFavoriteOption) && ! empty($adminFavoriteOption)) {
            ...
            if ($isChanged
                ...
                COption::SetOptionString("bitrix24", "admin_menu_items",
                    serialize($adminFavoriteOption), false, SITE_ID);
            }
        }
    }
}
if ( ! function_exists("CheckMenuUserItems")) {
    function CheckMenuUserItems(&$userMenuOption, $arMenuItemsId)
    {
        if (is_array($userMenuOption) && ! empty($userMenuOption)) {
            ...
            foreach ($userMenuOption as $title => $arIDs) {
                ...
                if (is_array($arIDs["hide"])) {
                    foreach ($arIDs["hide"] as $key => $id) {
                        if ( ! in_array($id, $arMenuItemsId)
                            || is_array($arIDs["show"])
                            && in_array($id, $arIDs["show"])
                        ) {
                            $key = array_search($id,
                                $userMenuOption[$title]["hide"]);
            ...
            if ($isChanged) {
                CUserOptions::SetOption("bitrix24", "user_menu_items_".SITE_ID,
                    $userMenuOption);
            }
        }
    }
...
if ( ! function_exists("CheckFavouriteItemsUserAdded")) {
    function CheckFavouriteItemsUserAdded(
        &$favoriteItemsUserAdded,
        $userMenuOption,
        $arMenuItemsId
    ) {
        if (is_array($favoriteItemsUserAdded)) {
            ...
            foreach ($favoriteItemsUserAdded as $key => $itemID) {
                if ( ! in_array($itemID, $arMenuItemsId)
                    ...
                    && ( ! is_array($userMenuOption["menu-favorites"]["show"])
                        || ! in_array($itemID,
                            $userMenuOption["menu-favorites"]["show"]))
                    && ( ! is_array($userMenuOption["menu-favorites"]["hide"])
                        || ! in_array($itemID,
                            $userMenuOption["menu-favorites"]["hide"]))
                ) {
            ...
            if ($isChanged) {
                CUserOptions::SetOption("bitrix24",
                    "user_added_favorite_items_".SITE_ID,
                    $favoriteItemsUserAdded);
            }
        }
    }
...
if ( ! function_exists("CheckMenuUserSections")) {
    function CheckMenuUserSections(&$userMenuSectionsOption, $arTitles)
    {
        if (is_array($userMenuSectionsOption)) {
            ...
            $userMenuSectionsOption
                = array_unique(array_merge($userMenuSectionsOption, $arTitles));

            if ($isChanged) {
                CUserOptions::SetOption("bitrix24",
                    "user_menu_sections_".SITE_ID, $userMenuSectionsOption);
            }
        }
    }
}
...
//sections order
$userMenuSectionsOption = CUserOptions::GetOption("bitrix24",
    "user_menu_sections_".SITE_ID);
...
$favoriteItemsUserAdded = CUserOptions::GetOption("bitrix24",
    "user_added_favorite_items_".SITE_ID);
...
//id to item
    if ($arItem["DEPTH_LEVEL"] == 2 && ! $arItem["PARAMS"]["menu_item_id"]) {
        $arItem["PARAMS"]["menu_item_id"] = ($arItem["PARAMS"]["name"]
            == "live_feed") ? "menu_live_feed" : crc32($arItem["LINK"]);
    } elseif ($arItem["DEPTH_LEVEL"] == 1
        && ! $arItem["PARAMS"]["menu_item_id"]
    ) {
        if (isset($arItem["PARAMS"]["class"])) {
            $arItem["PARAMS"]["menu_item_id"]
                = ( ! in_array($arItem["PARAMS"]["class"], $allTitleItemsID))
                ? $arItem["PARAMS"]["class"]
                : $arItem["PARAMS"]["class"]."_".crc32($arItem["LINK"]);
...
foreach ($arResultItems as $index => $arItem) {
    if ($arItem["DEPTH_LEVEL"] == 1) {
        ...
        $arResultItemsNew[$currentTitle][$arItem["PARAMS"]["menu_item_id"]]
            = $arItem;
...
if (is_array($userMenuSectionsOption) && ! empty($userMenuSectionsOption)) {
    CheckMenuUserSections($userMenuSectionsOption,
        array_keys($arResultTitleItemsNew));
...
CheckMenuAdminItems($adminFavoriteOption,
    $arMenuItemsId);//check admin items  for the existence
CheckMenuUserItems($userMenuOption,
    $arMenuItemsId);  //check user items  for the existence
CheckFavouriteItemsUserAdded($favoriteItemsUserAdded, $userMenuOption,
    $arMenuItemsId);
...
if (is_array($adminFavoriteOption) && ! empty($adminFavoriteOption)) {
    foreach ($arResultItems as $index => $arItem) {
        if ($arItem["DEPTH_LEVEL"] == 1) {
            ...
            if (in_array($arItem["PARAMS"]["menu_item_id"],
                $adminFavoriteOption)
            ) {
                $arItem["PARAMS"]["can_delete_from_favourite"]
                    = "A";//can edit just admin
                $arResultItemsNew[$currentTitle][$arItem["PARAMS"]["menu_item_id"]]
                    = $arItem;
                $arFlatItemsList[$arItem["PARAMS"]["menu_item_id"]] = $arItem;
                $arFavouriteDefault[] = $arItem["PARAMS"]["menu_item_id"];
...
//--add admin items to default favorite, status
$arResult["FAVOURITE_DEFAULT_ITEMS"]
    = $arFavouriteDefault;   //can't delete from favorite
...
if ($userMenuOption) {
    ...
    foreach ($userMenuOption as $title => $arIDs) {
        if ($title == "menu-favorites") {
            if (is_array($arIDs["show"])) {
                foreach ($arIDs["show"] as $key => $id) {
                    if ( ! in_array($id, $arResult["FAVOURITE_DEFAULT_ITEMS"])
                        && ! in_array($id, $favoriteItemsUserAdded)
                        && ! in_array($id, $adminFavoriteOption)
                    ) {
    ...
    if ($isChanged) {
        CUserOptions::SetOption("bitrix24", $optionNameUserMenu,
            $userMenuOption);
    }
}
...
foreach ($arResultTitleItemsNew as $title => $arTitle) {
    $titleItemExists = ($title == "menu-favorites") ? true
        : false;  //not hidden
    if (in_array($title, $userMenuOptionTitleIDs))  //block is sorted
    {
        if (is_array($userMenuOption[$title]["show"]))   //show items from option
        {
            foreach ($userMenuOption[$title]["show"] as $itemID) {
                $arResultSortItems[$title]["show"][$itemID]
                    = isset($arResultItemsNew[$title][$itemID])
                    ? $arResultItemsNew[$title][$itemID]
                    : $arFlatItemsList[$itemID];
        ...
        if (is_array($userMenuOption[$title]["hide"]))     //hide items from option
        {
            foreach ($userMenuOption[$title]["hide"] as $itemID) {
                $arResultSortItems[$title]["hide"][$itemID]
                    = isset($arResultItemsNew[$title][$itemID])
                    ? $arResultItemsNew[$title][$itemID]
                    : $arFlatItemsList[$itemID];
                unset($arResultItemsNew[$title][$itemID]);
        ...
        if (is_array($arResultItemsNew[$title])
            && ! empty($arResultItemsNew[$title])
        )   //other items, not from option
        {
