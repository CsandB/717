...
BX.Bitrix24.MenuClass = (function() {
  var MenuClass = function(params) {
    ...
    if (params.arHiddenItemsSelected) {
      for (var key in params.arHiddenItemsSelected) {
        this.showHideMoreItems(
            BX('more_btn_' + params.arHiddenItemsSelected[key]),
            params.arHiddenItemsSelected[key]);
      }
    }
    ...
    for (var i = 0, l = this.arTitles.length; i < l; i++) {
      ...
      BX.bind(item, 'click',
          BX.proxy(this.showHideMenuSection2, {element: item, self: this}));
      BX.bind(item.lastChild, 'click', BX.proxy(this.showHideMenuSection,
          {element: item.lastChild, self: this}));
    }
  };
  ...
  MenuClass.prototype.showHideMenuSection = function(event) {
    ...
    B24.toggleMenu(this.element.parentNode, BX.message('menu_show'),
        BX.message('menu_hide'));
  };
  ...
  MenuClass.prototype.showHideMenuSection2 = function() {
    if (!this.self.isEditMode()) {
      ...
      B24.toggleMenu(this.element, BX.message('menu_show'),
          BX.message('menu_hide'));
    }
  };
  ...
  MenuClass.prototype.applyEditMode = function() {
    ...
    var allTitleBlocks = BX.findChildren(BX('bx_b24_menu'),
        {className: 'menu-items-block'}, true);
    ...
    if (!isEditMode) {
      ...
      var allActiveItems = BX.findChildren(BX('bx_b24_menu'),
          {className: 'menu-item-active'}, true);
      ...
      BX.removeClass(BX('menu_favorites_settings'),
    ...
    for (var j = 0; j < this.arTitles.length; j++) {
      if (this.arTitles[j] != 'menu-favorites') {
        BX(this.arTitles[j]).onbxdragstart = BX.proxy(this.sectionDragStart,
            this);
        BX(this.arTitles[j]).onbxdrag = BX.proxy(this.sectionDragMove, this);
        BX(this.arTitles[j]).onbxdragstop = BX.proxy(this.sectionDragStop,
            this);
        BX(this.arTitles[j]).onbxdraghover = BX.proxy(this.sectionDragHover,
            this);
      ...
      if (!isEditMode) {
        jsDD.Enable();
        var liObj = BX.findChildren(BX('ul_' + this.arTitles[j]),
            {tagName: 'li'}, true);
        for (var i = 0; i < liObj.length; i++) {
          if (liObj[i].id == 'separator_' + this.arTitles[j])
            break;

          if (((this.isBitrix24 && this.arTitles[j] == 'menu-favorites') ||
              this.arTitles[j] == 'menu-groups') && liObj[i].id == 'empty_li_' +
              this.arTitles[j])
            continue;

          if ((this.isBitrix24 && liObj[i].id == 'menu_live_feed') ||
              liObj[i].id == 'menu_all_groups') {
      ...
      var liObj = BX.findChildren(BX('hidden_items_ul_' + this.arTitles[j]),
          {tagName: 'li'}, true);
      if (liObj.length > 0)
        BX('separator_' + this.arTitles[j]).style.display = (isEditMode)
            ? 'none'
            : 'block';
      else
  ...
  MenuClass.prototype.showHideMoreItems = function(element, titleItemId) {
    BX.toggleClass(BX('hidden_items_li_' + titleItemId),
    ...
    BX.firstChild(element).innerHTML = (BX.firstChild(element).innerHTML ==
  ...
  MenuClass.prototype.openMenuPopup = function(bindElement, menuItemId) {
    ...
    var can_delete_from_favorite = BX(menuItemId).
        getAttribute('data-can-delete-from-favorite');
    var title_item = BX(menuItemId).getAttribute('data-title-item');

    //add to favorite
    if (!itemIsFavourite && can_delete_from_favorite == 'Y')
      menuItems.push({
        text: BX.message('add_to_favorite'),
        className: 'menu-popup-no-icon',
        onclick: function() {
          this.popupWindow.close();
          self.addFavouriteItem(menuItemId, title_item, 'N');
    ...
    if (itemIsFavourite && can_delete_from_favorite == 'Y')
      menuItems.push({
        text: BX.message('delete_from_favorite'),
        className: 'menu-popup-no-icon',
        onclick: function() {
          this.popupWindow.close();
          self.deleteFavouriteItem(menuItemId, title_item, 'N');
          BX.PopupMenu.destroy('popup_' + menuItemId);
        },
      });
    //hide item
    if (BX(menuItemId).getAttribute('data-status') ==
        'show' /*&& !(itemIsFavourite && can_delete_from_favorite == "Y")*/)
      menuItems.push({
        text: BX.message('hide_item'),
        className: 'menu-popup-no-icon',
        onclick: function() {
          this.popupWindow.close();
          self.hideItem(menuItemId, title_item);
          BX.PopupMenu.destroy('popup_' + menuItemId);
        },
      });
    //show item
    if (BX(menuItemId).getAttribute('data-status') ==
        'hide'/* && !(itemIsFavourite && can_delete_from_favorite == "Y")*/)
      menuItems.push({
        text: BX.message('show_item'),
        className: 'menu-popup-no-icon',
        onclick: function() {
          this.popupWindow.close();
          self.showItem(menuItemId, title_item);
          BX.PopupMenu.destroy('popup_' + menuItemId);
        },
      });
    ...
    if (this.isAdmin) {
      ...
      if (!itemIsFavourite)
        menuItems.push({
          text: BX.message('add_to_favorite_all'),
          className: 'menu-popup-no-icon',
          onclick: function() {
            self.addFavouriteItem(menuItemId, title_item, 'Y');
            BX.PopupMenu.destroy('popup_' + menuItemId);
          },
        });
      ...
      if (itemIsFavourite && can_delete_from_favorite == 'A')
        menuItems.push({
          text: BX.message('delete_from_favorite_all'),
          className: 'menu-popup-no-icon',
          onclick: function() {
            self.deleteFavouriteItem(menuItemId, title_item, 'Y');
            BX.PopupMenu.destroy('popup_' + menuItemId);
          },
        });
      ...
      if (BX(menuItemId).getAttribute('data-app-id'))
        menuItems.push({
          text: BX.message('set_rights'),
          className: 'menu-popup-no-icon',
          onclick: function() {
            this.popupWindow.close();
            self.setRights(menuItemId);
            BX.PopupMenu.destroy('popup_' + menuItemId);
          },
        });
    }

    var MenuPopup = BX.PopupMenu.show('popup_' + menuItemId, bindElement,
        menuItems,
        {
          offsetTop: 0,
          offsetLeft: 12,
  ...
  MenuClass.prototype.setRights = function(menuItemId) {
    BX.rest.Marketplace.setRights(BX(menuItemId).getAttribute('data-app-id'),
        this.siteid);
  };

  MenuClass.prototype.addFavouriteItem = function(
      menuItemId, titleItem, forAll) {
    BX.ajax({
      method: 'POST',
      dataType: 'json',
      url: this.ajaxPath,
      ...
      onsuccess: BX.proxy(function(json) {
        if (json.error) {
          ...
          BX.removeClass(BX.firstChild(BX(menuItemId)),
              'menu-favorites-btn-active');
          var cloneObj = BX.clone(BX(menuItemId));
          BX(menuItemId).id = 'hidden_' + menuItemId;
          BX('hidden_' + menuItemId).style.display = 'none';
          BX('ul_menu-favorites').
              insertBefore(cloneObj, BX('separator_menu-favorites'));
          ...
          var otherItems = BX.findChildren(BX('ul_' + titleItem),
              {tagName: 'li'}, true);
          ...
          var otherHiddenItems = BX.findChildren(
              BX('hidden_items_ul_' + titleItem), {tagName: 'li'}, true);
  ...
  MenuClass.prototype.deleteFavouriteItem = function(
      menuItemId, oldTitleItem, forAll) {
    BX.ajax({
      method: 'POST',
      dataType: 'json',
      url: this.ajaxPath,
      ...
      onsuccess: BX.proxy(function(json) {
        if (json.error) {
          ...
          var curOtherHiddenItems = BX.findChildren(
              BX('hidden_items_ul_' + cur_title_item), {tagName: 'li'}, true);
          var otherHiddenExist = false;
          ...
          //favorite block
          var otherHiddenItems = BX.findChildren(
              BX('hidden_items_ul_' + oldTitleItem), {tagName: 'li'}, true);
      ...
      onsuccess: BX.proxy(function(json) {
        if (json.error) {
          ...
          if (BX(menuItemId).getAttribute('data-counter-id')) {
            this.hiddenCounters.push(
                BX(menuItemId).getAttribute('data-counter-id'));
            var curSumCounters = 0;
            for (var i = 0; i < this.hiddenCounters.length; i++) {
              curSumCounters += +(this.allCounters[this.hiddenCounters[i]]);
            }

            BX('menu-hidden-counter').innerHTML = curSumCounters > 50
                ? '50+'
                : curSumCounters;
            if (curSumCounters > 0)
    ...
    BX.ajax({
      ...
      onsuccess: BX.proxy(function(json) {
        if (json.error) {
          ...
          BX(menuItemId).setAttribute('data-status', 'show');
          BX('ul_' + titleItem).
              insertBefore(BX(menuItemId), BX('separator_' + titleItem));
          BX(menuItemId).onbxdragstart = BX.proxy(this.menuItemDragStart, this);
          BX(menuItemId).onbxdrag = BX.proxy(this.menuItemDragMove, this);
          BX(menuItemId).onbxdragstop = BX.proxy(this.menuItemDragStop, this);
          BX(menuItemId).onbxdraghover = BX.proxy(this.menuItemDragHover, this);
  ...
  MenuClass.prototype.menuItemDragStart = function() {
    ...
    this.bxblank = this.bxparent.insertBefore(
        BX.create('DIV', {style: {height: '0px'}}), dragElement);
    ...
    for (var i = 0; i < liObj.length; i++) {
      if (liObj[i].id == 'separator_' +
          dragElement.getAttribute('data-title-item'))
        break;
      if (liObj[i].id == 'empty_li_' +
          dragElement.getAttribute('data-title-item'))
        continue;
  ...
  MenuClass.prototype.menuItemDragMove = function(x, y) {
    ...
    if (this.isBitrix24 &&
        (dragElement.getAttribute('data-title-item') == 'menu-favorites' ||
            dragElement.getAttribute('data-title-item') == 'menu-groups') && y <
        this.objHeight)
      y = this.objHeight;
    else if (y < 0)
      y = 0;
  ...
  MenuClass.prototype.menuItemDragStop = function() {
    ...
    if (this.bxblank1 && this.bxblank1.parentNode == this.bxparent) {
      ...
      var arTitleItems = [];
      var liObj = BX.findChildren(dragElement.parentNode, {tagName: 'li'},
          true);
      for (var i = 0; i < liObj.length; i++) {
        if (liObj[i].id == 'empty_li_' +
            dragElement.getAttribute('data-title-item'))
  ...
  MenuClass.prototype.sectionDragStart = function() {
    ...
    this.bxSectBlank = this.bxSectParent.insertBefore(
        BX.create('DIV', {style: {height: '0px'}}), dragElement.parentNode);
    this.bxSectBlank1 = BX.create('DIV',
        {style: {height: this.objSectHeight + 'px'}}); //empty div
  ...
  MenuClass.prototype.sectionDragStop = function() {
    ...
    if (this.bxSectBlank1 && this.bxSectBlank1.parentNode ==
        this.bxSectParent) {
      this.bxSectParent.replaceChild(dragElement.parentNode, this.bxSectBlank1);

      var arSectionItems = [];
      var sectionsObj = BX.findChildren(dragElement.parentNode.parentNode,
          {className: 'menu-items-title'}, true);
      for (var i = 0; i < sectionsObj.length; i++) {
        arSectionItems.push(sectionsObj[i].id);
      }
      this.sortSections(arSectionItems,
          dragElement.getAttribute('data-title-item'));
    } else {
