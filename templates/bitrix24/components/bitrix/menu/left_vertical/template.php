...
<div class="menu-items-block" id="menu-items-block">
    <div class="menu-items-header"><?
        include($_SERVER["DOCUMENT_ROOT"]
            ."/bitrix/templates/bitrix24/logo.php");
        ?>
    ...
    <div class="menu-items-body"><?
        ...
        <ul class="menu-items"><?
            ?>
            <li class="menu-items-empty-li" id="left-menu-empty-item"
                style="height: 3px;"></li><?
            foreach (array("show", "hide") as $status) {
                ...
                if (isset($arResult["ITEMS"][$status])
                    && is_array($arResult["ITEMS"][$status])
                ) {
                    foreach ($arResult["ITEMS"][$status] as $item) {
                        ...
                        $counterId = "";
                        $counter = 0;
                        if (array_key_exists("counter_id", $item["PARAMS"])
                            && $item["PARAMS"]["counter_id"] <> ''
                        ) {
                            $counterId = $item["PARAMS"]["counter_id"]
                            == "live-feed" ? "**"
                                : $item["PARAMS"]["counter_id"];
                            $counter = array_key_exists($counterId,
                                $arResult["COUNTERS"])
                                ? $arResult["COUNTERS"][$counterId] : 0;
                        }

                        if ($counterId) {
                            $arAllItemsCounters[$counterId] = $isCompositeMode
                                ? 0 : $counter;
                            if ($status == "hide") {
                        ...
                        $curLink = isset($item["PARAMS"]["real_link"])
                            ? $item["PARAMS"]["real_link"] : $item["LINK"];
                        if (preg_match("~^".SITE_DIR."index\\.php~i",
                            $curLink)
                        ) {
                            $curLink = SITE_DIR;
                        } elseif (isset($item["PARAMS"]["onclick"])
                            && ! empty($item["PARAMS"]["onclick"])
                        ) {
                            $curLink = "";
                        }
                        ...
                        if ($isCompositeMode === false && $counter > 0
                            && $counterId <> ''
                        ) {
                        ...
                        <? endif ?>
                            data-new-page="<?= (isset($item["OPEN_IN_NEW_PAGE"])
                                && $item["OPEN_IN_NEW_PAGE"]) ? "Y" : "N" ?>"
                            class="<?= $itemClass ?>"
                        ><?
                        ?><span
                        ...
                        if (isset($item["PARAMS"]["sub_link"])):
                            ?><a
                            href="<?= htmlspecialcharsbx($item["PARAMS"]["sub_link"]) ?>"
                        ...
                        href="<?= (isset($item["PARAMS"]["onclick"]))
                            ? "javascript:void(0)" : $curLink ?>"
                        <? if (isset($item["OPEN_IN_NEW_PAGE"])
                            && $item["OPEN_IN_NEW_PAGE"]
                        ):?>
                            target="_blank"
                        <?endif ?>
                        ...
                        <span class="menu-item-icon-box"><span
                                    class="menu-item-icon"></span></span><?
                        ?><span
                        class="menu-item-link-text <? echo isset($item["PARAMS"]["is_beta"])
                            ? ' menu-item-link-beta' : '' ?>"
                        data-role="item-text"><?
                        echo $item["TEXT"];
                        ?></span><?
                        ...
                        $editBtnHideClass = "";
                        if ($item["PARAMS"]["menu_item_id"]
                            === "menu_all_groups"
                            && count($arResult["GROUPS"])
                        ):
                            $groupPopupExists = true;
                            $editBtnHideClass = " menu-fav-editable-btn-hide";
                            ?><span class="menu-item-show-link"
                                    id="menu-all-groups-link"><?= Loc::getMessage("MENU_SHOW") ?></span><?
                        endif
                        ?><span
                ...
                if ($status === "hide") {
                    ?>
                    <li class="menu-items-hidden-empty-li"
                        id="left-menu-hidden-empty-item"></li><?
                    ?></ul><?
                    ?></li><?
                }
            }
        ...
        <div class="menu-favorites-more-btn<? if (empty($arResult["ITEMS"]["hide"])): ?> menu-favorites-more-btn-hidden<? endif ?>">
            ...
            <span id="menu-hidden-counter"
                  class="menu-item-index menu-item-index-more<?
                  if ($isCompositeMode
                      || $sumHiddenCounters <= 0
                  ):?> menu-hidden-counter<? endif ?>"><?
                ?><?= ($isCompositeMode ? ""
        ...
        <div class="menu-extra-btn-box">
            ...
            if (CModule::IncludeModule("bitrix24")
                ...
                <div class="menu-invite-employees"
                    ...
                    <span class="menu-invite-icon-box"><span
                                class="menu-invite-icon"></span></span>
...
$arJSParams = array(
    ...
    "isCollapsedMode"         => CUserOptions::GetOption("intranet",
            "left_menu_collapsed") === "Y",
    "showPresetPopup"         => $arResult["SHOW_PRESET_POPUP"] ? "Y" : "N",
    "showImportConfiguration" => $arResult["SHOW_IMPORT_CONFIGURATION"] ? "Y"
        : "N",
    "urlImportConfiguration"  => $arResult["URL_IMPORT_CONFIGURATION"],
    "isPublicConverted"       => $arResult["IS_PUBLIC_CONVERTED"] ? "Y" : "N",
    "isCustomPresetAvailable" => $arResult["IS_CUSTOM_PRESET_AVAILABLE"] ? "Y"
        : "N",
...
if (count($arResult["MAP_ITEMS"])) {
    ...
    $link = isset($item["PARAMS"]["real_link"]) ? $item["PARAMS"]["real_link"]
        : $item["LINK"];
...
if ($groupPopupExists):
    ...
    <?
    $filter = CUserOptions::GetOption("intranet",
        "left_menu_group_filter_".SITE_ID, "all");
    ?>
    <div class="group-panel-content group-panel-content-<?= $filter ?>"
         data-filter="<?= $filter ?>" id="group-panel-content">
		...
		<span class="group-panel-header-filters">
			...
			<span
					...
					<span class="group-panel-header-filter-counter"
                          id="group-panel-header-filter-counter"></span>
			</span>
		</span>
        </div>
        <div class="group-panel-items" id="group-panel-items"><?
            foreach ($arResult["GROUPS"] as $index => $group):
                ...
                $className .= $group["EXTRANET"] ? " group-panel-item-extranet"
                    : " group-panel-item-intranet";
                $className .= $group["FAVORITE"] ? " group-panel-item-favorite"
                    : "";
                ?><a href="<?= SITE_DIR ?>workgroups/group/<?= $group["ID"] ?>/"
                     class="<?= $className ?>" data-id="<?= $group["ID"] ?>"><?
                ?><span
            ...
            endforeach
            ?></div>
        <div class="sitemap-close-link group-panel-close-link"
             id="group-panel-close-link"></div>
    </div>
<? endif ?>
...
$APPLICATION->includeComponent("bitrix:spotlight", "", [
        ...
        "JS_OPTIONS" => [
            ...
            "lightMode"       => ThemePicker::getInstance()
                    ->getCurrentBaseThemeId() !== "dark",
