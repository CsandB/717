...
$getGroups = function ($siteId, $limit, $ids = array()) use (
    $userId,
    $extranetSiteId
) {
...
$getFavorites = function ($siteId, $limit, $ids = array()) use (
    $userId,
    $extranetSiteId
) {
    $groups = WorkgroupTable::getList(array(
...
$extranetGroups = isModuleInstalled("extranet") ? $getGroups($extranetSiteId,
    150) : array();
$intranetGroups = $getGroups(SITE_ID, 150, array_keys($extranetGroups));
$favoriteExtranetGroups = isModuleInstalled("extranet")
    ? $getFavorites($extranetSiteId, 100) : array();
$favoriteIntranetGroups = $getFavorites(SITE_ID, 100,
    array_keys($favoriteExtranetGroups));

$groups = array_replace($extranetGroups, $intranetGroups,
    $favoriteExtranetGroups, $favoriteIntranetGroups);
