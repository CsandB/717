...
<div class="left-menu-popup-wrapper" id="left-menu-preset-popup"
     style="display: none">
    <form method="POST" name="left-menu-preset-form">
        ...
        <div class="left-menu-popup-card-container">
            <? if (CModule::IncludeModule("crm")
                ...
                <label class="left-menu-popup-card-item js-left-menu-preset-item<? if ($arResult["CURRENT_PRESET_ID"]
                    == "crm"
                ): ?> left-menu-popup-selected<? endif ?>" for="presetTypeCrm">
                    ...
                    <input type="radio" name="presetType" value="crm"
                           id="presetTypeCrm"
                           <? if ($arResult["CURRENT_PRESET_ID"]
                           == "crm"): ?>checked<? endif ?>style="display: none">
                </label>
            <? endif ?>

            <?
            if (Loader::includeModule("socialnetwork")) {
                $arUserActiveFeatures
                    = CSocNetFeatures::GetActiveFeatures(SONET_ENTITY_USER,
                    $GLOBALS["USER"]->GetID());
                $arSocNetFeaturesSettings
                    = CSocNetAllowed::GetAllowedFeatures();

                $isFeatureAllowed = array_key_exists("tasks",
                        $arSocNetFeaturesSettings)
                    && array_key_exists("allowed",
                        $arSocNetFeaturesSettings["tasks"])
                    && in_array(SONET_ENTITY_USER,
                        $arSocNetFeaturesSettings["tasks"]["allowed"])
                ...
                if ($isFeatureAllowed):
                    ...
                    <label class="left-menu-popup-card-item js-left-menu-preset-item<? if ($arResult["CURRENT_PRESET_ID"]
                        == "tasks"
                    ):?> left-menu-popup-selected<?endif ?>"
                           for="presetTypeTasks">
                        ...
                        <input type="radio" name="presetType" value="tasks"
                               id="presetTypeTasks"
                               <? if ($arResult["CURRENT_PRESET_ID"]
                               == "tasks"): ?>checked<?endif ?>
                               style="display: none">
            ...
            <label class="left-menu-popup-card-item js-left-menu-preset-item<? if ($arResult["CURRENT_PRESET_ID"]
                == "social"
            ): ?> left-menu-popup-selected<? endif ?>" for="presetTypeSocial">
                ...
                <input type="radio" name="presetType" value="social"
                       id="presetTypeSocial"
                       <? if ($arResult["CURRENT_PRESET_ID"]
                       == "social"): ?>checked<? endif ?> style="display: none">
            </label>
            ...
            <? if (
                ...
                && (
                    in_array(CBitrix24::getPortalZone(),
                        array("ru", "ua", "kz", "by"))
                ...
                <label class="left-menu-popup-card-item js-left-menu-preset-item <? if ($arResult["CURRENT_PRESET_ID"]
                    ...
                    <input type="radio" name="presetType" value="sites"
                           id="presetTypeSites"
                           <? if ($arResult["CURRENT_PRESET_ID"]
                           == "sites"): ?>checked<? endif ?>
                           style="display: none">
