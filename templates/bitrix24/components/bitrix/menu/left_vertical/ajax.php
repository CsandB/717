...
$setGroupToFavorites = function ($groupId, $value = "Y") {
    if (intval($groupId) && $GLOBALS["USER"]->getId()
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["action"] <> ''
    ...
    $isAdmin = $GLOBALS["USER"]->isAdmin()
        || (Loader::includeModule("bitrix24")
            && CBitrix24::IsPortalAdmin($GLOBALS["USER"]->getID()));
    ...
    switch ($_POST["action"]) {
        case "add_standard_item":
            ...
            if (isset($_POST["itemData"]["counterId"])
                && $_POST["itemData"]["counterId"]
            ) {
                $newItem["COUNTER_ID"] = $_POST["itemData"]["counterId"];
            }

            if (isset($_POST["itemData"]["subLink"])
                && is_array($_POST["itemData"]["subLink"])
            ) {
                $newItem["SUB_LINK"] = $_POST["itemData"]["subLink"]["URL"];
            }

            $standardItems = CUserOptions::GetOption("intranet",
                "left_menu_standard_items_".$siteID);

            if (is_array($standardItems) && ! empty($standardItems)) {
                foreach ($standardItems as $item) {
                    if ($item["LINK"] == $newItem["LINK"]) {
                        $error
                            = GetMessage("LEFT_MENU_SELF_ITEM_DUBLICATE_ERROR");
                        break 2;
                    }
                }
            ...
            CUserOptions::SetOption("intranet",
                "left_menu_standard_items_".$siteID, $standardItems);

            if (preg_match("~^/workgroups/group/([0-9]+)/$~i", $itemLink,
                $match)
            ) {
        ...
        case "delete_standard_item":
            ...
            if (is_array($standardItems)) {
                ...
                if ( ! empty($standardItems)) {
                    ...
                    CUserOptions::DeleteOption("intranet",
                        "left_menu_standard_items_".$siteID);
                }
        ...
        case "add_self_item":
            ...
            if (isset($_POST["itemData"]["text"])) {
                $itemText = trim($_POST["itemData"]["text"]);
                $itemText = Emoji::encode($itemText);
            }
            if (empty($itemText)) {
            ...
            $newItem = array(
                ...
                "LINK"     => $itemLink,
                "ID"       => $itemID,
                "NEW_PAGE" => isset($_POST["itemData"]["openInNewPage"])
                && $_POST["itemData"]["openInNewPage"] == "Y" ? "Y" : "N",
            );
            $selfItems = CUserOptions::GetOption("intranet",
                "left_menu_self_items_".$siteID);
            ...
            CUserOptions::SetOption("intranet", "left_menu_self_items_".$siteID,
                $selfItems);
        ...
        case "update_self_item":
            ...
            if (isset($_POST["itemData"]["text"])) {
                ...
                $itemData["TEXT"] = Emoji::encode($itemData["TEXT"]);
            }
            ...
            if (is_array($selfItems) && ! empty($selfItems)) {
                foreach ($selfItems as $key => $item) {
                    if ($item["ID"] == $_POST["itemData"]["id"]) {
                        ...
                        CUserOptions::SetOption("intranet",
                            "left_menu_self_items_".$siteID, $selfItems);
                        break;
                    }
                }
            }
        ...
        */
        case "add_item_to_all":
            ...
            if (isset($_POST["itemInfo"]) && is_array($_POST["itemInfo"])) {
                ...
                if (isset($_POST["itemInfo"]["openInNewPage"])
                    && $_POST["itemInfo"]["openInNewPage"] == "Y"
                ) {
                    $itemData["NEW_PAGE"] = "Y";
                }
                ...
                $adminOption = COption::GetOptionString("intranet",
                    "left_menu_items_to_all_".$siteID);
                ...
                COption::SetOptionString("intranet",
                    "left_menu_items_to_all_".$siteID, serialize($adminOption),
                    false, $siteID);
            }
                ...
                foreach ($adminOption as $key => $item) {
                    if ($item["ID"] == $_POST["menu_item_id"]) {
                        ...
                        if (empty($adminOption)) {
                            ...
                            COption::SetOptionString("intranet",
                                "left_menu_items_to_all_".$siteID,
                                serialize($adminOption), false, $siteID);
                        }
        ...
        case "delete_custom_item_from_all":
            ...
            $customItems = COption::GetOptionString("intranet",
                "left_menu_custom_preset_items", "", $siteID);
                ...
                foreach ($customItems as $key => $item) {
                    if ($item["ID"] == $_POST["menu_item_id"]) {
                        ...
                        if (empty($customItems)) {
                            ...
                            COption::SetOptionString("intranet",
                                "left_menu_custom_preset_items",
                                serialize($customItems), false, $siteID);
                        }
            ...
            $customItemsSort = COption::GetOptionString("intranet",
                "left_menu_custom_preset_sort", "", $siteID);
            if ( ! empty($customItemsSort)) {
                ...
                COption::SetOptionString("intranet",
                    "left_menu_custom_preset_sort", serialize($customItemsSort),
                    false, $siteID);
            }
        ...
        case "add_favorite_admin":
            if ($isAdmin) {
                $adminOption = COption::GetOptionString("intranet",
                    "admin_menu_items", "", $siteID);
                if ($adminOption) {
                    $adminOption = unserialize($adminOption);
                } else {
                    $adminOption = array();
                }
                if (is_array($adminOption)
                    && ! in_array($menuItemID, $adminOption)
                ) {
                    $adminOption[] = $menuItemID;
                    $adminOption = serialize($adminOption);
                    COption::SetOptionString("intranet", "admin_menu_items",
                        $adminOption, false, $siteID);
                }
            } else {
                $error = "Y";
            }
        ...
        case "save_items_sort":
            if (isset($_POST["items"])) {
                foreach (array("show", "hide") as $status) {
                    if (isset($_POST["items"][$status])
                        && is_array($_POST["items"][$status])
                    ) {
        ...
        case "set_default_menu":
            ...
            if (COption::GetOptionString("intranet", "left_menu_preset", "",
                    $siteID) == "custom"
            ) {
                CUserOptions::DeleteOptionsByName("intranet",
                    "left_menu_preset_".$siteID);
            }
            break;
        ...
        case "set_preset":
            if ( ! isset($_POST["preset"])
                || ! in_array($_POST["preset"],
                    array("social", "crm", "tasks", "sites"))
                || ! isset($_POST["mode"])
            ) {
                $error = GetMessage("LEFT_MENU_PRESET_ERROR");
            } else {
                if ($_POST["mode"] == "global" && $isAdmin) {
                    COption::SetOptionString("intranet", "left_menu_preset",
                        $_POST["preset"], false, $siteID);
                } else {
                    CUserOptions::SetOption("intranet",
                        "left_menu_preset_".$siteID, $_POST["preset"]);
                ...
                switch ($_POST["preset"]) {
                    case "tasks":
                        $firstPageUrl = $_POST["siteDir"]
                            ."company/personal/user/".($_POST["mode"]
                            == "global" ? "#USER_ID#" : $USER->GetID())
                            ."/tasks/";
                        break;
                ...
                if ($firstPageUrl) {
                    if ($_POST["mode"] == "global" && $isAdmin) {
                        COption::SetOptionString("intranet",
                            "left_menu_first_page", $firstPageUrl, false,
                            $siteID);
                    } else {
                        CUserOptions::SetOption("intranet",
                            "left_menu_first_page_".$siteID, $firstPageUrl);
                    }

                    $arJsonData["url"] = str_replace("#USER_ID#",
                        $USER->GetID(), $firstPageUrl);
                }
            ...
            $showPresetPopup = COption::GetOptionString("intranet",
                    "show_menu_preset_popup", "N") == "Y";
            if ($showPresetPopup) {
                COption::SetOptionString("intranet", "show_menu_preset_popup",
                    "N");
            }
        ...
        case "save_custom_preset":
            ...
            if (isset($_POST["userApply"])
                && $_POST["userApply"] == "currentUser"
            ) {
                CUserOptions::DeleteOptionsByName("intranet",
                    "left_menu_sorted_items_".$siteID);
                CUserOptions::DeleteOptionsByName("intranet",
                    "left_menu_preset_".$siteID);
            }
            if (isset($_POST["itemsSort"])) {
                COption::SetOptionString("intranet",
                    ...
                    serialize($_POST["itemsSort"]), false, $siteID);
            }
            if (isset($_POST["customItems"])) {
                COption::SetOptionString("intranet",
                    "left_menu_custom_preset_items",
                    serialize($_POST["customItems"]), false, $siteID);
            }
            ...
            if (isset($_POST["firstItemLink"])) {
                ...
                if (preg_match("~company/personal/user/\d+/tasks/$~i",
                    $firstPageUrl, $match)
                ) {
                    $firstPageUrl = $_POST["siteDir"]
                ...
                COption::SetOptionString("intranet", "left_menu_first_page",
                    $firstPageUrl, false, $siteID);
            }
        ...
        case "set_group_filter":
            if (isset($_POST["filter"])
                && in_array($_POST["filter"],
                    array("all", "extranet", "favorites"))
            ) {
                CUserOptions::SetOption("intranet",
                    "left_menu_group_filter_".$siteID, $_POST["filter"]);
            }
            break;
        ...
        case "set_first_page":
            ...
            CUserOptions::SetOption("intranet", "left_menu_first_page_".$siteID,
                $_POST["firstPageUrl"]);
            break;
    }
