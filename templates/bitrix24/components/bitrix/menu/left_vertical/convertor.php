...
$oldItemsAdded = CUserOptions::GetOption("bitrix24",
...
if ( ! IsModuleInstalled("bitrix24")
    && COption::GetOptionString("intranet", "intranet_public_converted", "")
    == "Y"
) {
    $oldLinksMapping = array(
        crc32(SITE_DIR."company/absence.php")     => "menu_absence",
        //crc32(SITE_DIR."timeman/"),
        crc32(SITE_DIR."company/work_report.php") => "menu_work_report",
        //crc32(SITE_DIR."timeman/work_report.php"),
        crc32(SITE_DIR."company/timeman.php")     => "menu_timeman",
        //crc32(SITE_DIR."timeman/timeman.php"),
        crc32(SITE_DIR."services/meeting/")       => "menu_meeting",
        //crc32(SITE_DIR."timeman/meeting/"),
        crc32(SITE_DIR."about/calendar.php")      => "menu_company_calendar",
        //crc32(SITE_DIR."calendar/"),
        crc32(SITE_DIR."services/processes/")     => "menu_processes",
        //crc32(SITE_DIR."bizproc/processes/"),
...
if (CModule::IncludeModule("socialnetwork")) {
    ...
    while ($group = $groups->GetNext()) {
        $link = str_replace("#subject_id#", $group["GROUP_ID"],
            $strGroupSubjectLinkTemplate);
...
$skipId = array(
    ...
    "menu_marketplace",
    "menu_employee",
    "menu_telephony_balance",
    "menu_bizproc",
    "menu_license",
    "menu_configs",
    "menu_openlines_lines",
);

$convertStandardItems = array();
$convertSortedItems = array();
if (is_array($oldItemsSort) && ! empty($oldItemsSort)) {
    if (isset($oldItemsSort["menu-favorites"])
        && is_array($oldItemsSort["menu-favorites"])
        && ! empty($oldItemsSort["menu-favorites"])
    ) {
        foreach ($oldItemsSort["menu-favorites"] as $status => $items) {
            ...
            foreach ($items as $id) {
                ...
                if ( ! is_array($oldItemsAdded)
                    || ! in_array($id, $oldItemsAdded)
                ) {
                    continue;
                }

                /*check for new page links*/
                if ( ! empty($oldLinksMapping)
                    && array_key_exists($id, $oldLinksMapping)
                ) {
                    $id = $oldLinksMapping[$id];
                }

                if ( ! in_array($id, $defaultItemsId)
                    && array_key_exists($id, $mapIds)
                    && ! in_array($id, $skipId)
                ) {
        ...
        if ( ! empty($convertStandardItems)) {
            CUserOptions::SetOption("intranet",
                "left_menu_standard_items_".SITE_ID, $convertStandardItems);
        }
...
if (COption::GetOptionString("intranet", "left_menu_admin_converted", "N")
    !== "Y"
) {
        ...
        if (is_array($oldItemsAdmin) && ! empty($oldItemsAdmin)) {
            ...
            if ( ! empty($convertAdminItems)) {
                COption::SetOptionString("intranet",
                    ...
                    serialize($convertAdminItems), false, SITE_ID);
            }
        }
    }

    COption::SetOptionString("intranet", "left_menu_admin_converted", "Y",
        false, SITE_ID);
}
