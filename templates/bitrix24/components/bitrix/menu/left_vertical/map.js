...
BX.Bitrix24.SlidingPanel.prototype = {
  ...
  open: function() {
    ...
    this.header.addEventListener('click', BX.proxy(this.onHeaderClick, this),
        true);
  ...
  close: function(immediately) {
    ...
    this.header.removeEventListener('click', BX.proxy(this.onHeaderClick, this),
        true);
  ...
  adjustPosition: function() {
    ...
    var top = scrollTop > menuItemsPosition.top
        ? scrollTop
        : menuItemsPosition.top;
...
BX.Bitrix24.GroupPanel = function(options) {
  ...
  this.ajaxPath = BX.type.isNotEmptyString(options.ajaxPath)
      ? options.ajaxPath
      : null;
  this.siteId = BX.type.isNotEmptyString(options.siteId)
      ? options.siteId
      : BX.message('SITE_ID');
  ...
  var intranetGroups = this.items.getElementsByClassName(
      'group-panel-item-intranet');
  var extranetGroups = this.items.getElementsByClassName(
  ...
  this.filters = [].slice.call(
      this.content.getElementsByClassName('group-panel-header-filter'));
...
BX.Bitrix24.GroupPanel.prototype = Object.create(
    BX.Bitrix24.SlidingPanel.prototype);
...
BX.Bitrix24.GroupPanel.prototype.onFilterClick = function(event) {
  var filterElement = BX.type.isDomNode(BX.proxy_context)
      ? BX.proxy_context
      : null;

  var currentFilter = this.content.dataset.filter
      ? this.content.dataset.filter
      : 'all';
  var newFilter = filterElement.dataset.filter
...
BX.Bitrix24.GroupPanel.prototype.onItemsClick = function(event) {
  ...
  var action = BX.hasClass(item, 'group-panel-item-favorite')
