...
BX.Bitrix24.LeftMenu = {
  init: function(params) {
    ...
    this.spotlightId = BX.type.isNotEmptyString(params.spotlightId)
        ? params.spotlightId
        : null;
    ...
    this.menuMoreButton = this.menuContainer.querySelector(
        '.menu-favorites-more-btn');
    this.menuItemsBlock = this.menuContainer.querySelector('.menu-items');

    this.inviteEmployeesBox = this.menuContainer.querySelector(
        '.menu-invite-employees');
    this.licenseBox = this.menuContainer.querySelector(
        '.menu-license-all-container');

    this.settingsBox = this.menuContainer.querySelector('.menu-settings-btn');
    this.settingsIconBox = this.settingsBox.querySelector(
        '.menu-settings-icon-box');
    this.settingsBtnText = this.settingsBox.querySelector(
        '.menu-settings-btn-text');
    this.settingsSaveBtn = this.menuContainer.querySelector(
    ...
    if (this.adminPanel) {
      ...
      BX.addCustomEvent('onTopPanelCollapse',
          this.handleAdminPanelCollapse.bind(this));
    ...
    BX.bind(this.menuContainer, 'dblclick',
        this.handleMenuDoubleClick.bind(this));
    BX.bind(this.menuContainer, 'mouseenter',
        this.handleMenuMouseEnter.bind(this));
    BX.bind(this.menuContainer, 'mouseleave',
        this.handleMenuMouseLeave.bind(this));
    BX.bind(this.menuContainer, 'transitionend',
        this.handleSlidingTransitionEnd.bind(this));
    ...
    this.headerSettings = this.header.querySelector(
    ...
    if (this.headerSettings) {
      BX.bind(this.headerLogoBlock, 'mouseenter',
          this.handleHeaderLogoMouserEnter.bind(this));
      BX.bind(this.headerLogoBlock, 'mouseleave',
          this.handleHeaderLogoMouserLeave.bind(this));
      BX.bind(this.menuHeader, 'mouseenter',
          this.handleHeaderLogoMouserEnter.bind(this));
      BX.bind(this.menuHeader, 'mouseleave',
          this.handleHeaderLogoMouserLeave.bind(this));
    }

    BX.bind(this.menuHeaderBurger, 'click', this.handleBurgerClick.bind(this));
    BX.bind(this.menuHeaderTitle, 'click',
        this.handleBurgerClick.bind(this, true));
    ...
    BX.bind(this.upButton, 'mouseenter',
        this.handleUpButtonMouseEnter.bind(this));
    BX.bind(this.upButton, 'mouseleave',
        this.handleUpButtonMouseLeave.bind(this));

    BX.bind(BX('left-menu-hidden-separator'), 'click',
        BX.proxy(this.showHideMoreItems, this));
    BX.bind(this.menuMoreButton, 'click',
        BX.proxy(this.showHideMoreItems, this));
    ...
    if (this.inviteEmployeesBox) {
      BX.bind(this.inviteEmployeesBox, 'mouseenter',
    ...
    BX.addCustomEvent('BX.Bitrix24.GroupPanel:onOpen',
        this.handleGroupPanelOpen.bind(this));
    BX.addCustomEvent('BX.Bitrix24.GroupPanel:onClose',
        this.handleGroupPanelClose.bind(this));

    BX.addCustomEvent('BX.Main.InterfaceButtons:onFirstItemChange',
        BX.proxy(function(firstPageLink, firstNode) {
          this.onTopMenuFirstItemChange(firstPageLink, firstNode);
        }, this));

    BX.addCustomEvent('BX.Main.InterfaceButtons:onHideLastVisibleItem',
        BX.proxy(function(bindElement) {
          this.showMessage(bindElement,
              BX.message('MENU_TOP_ITEM_LAST_HIDDEN'));
        }, this));

    BX.addCustomEvent('BX.Main.InterfaceButtons:onBeforeCreateEditMenu',
        function(contextMenu, dataItem, topMenu) {
          var isItemInLeftMenu = BX.type.isDomNode(
              BX('bx_left_menu_' + dataItem.DATA_ID));
          contextMenu.addMenuItem({
            text: BX.message(isItemInLeftMenu
                ? 'MENU_DELETE_FROM_LEFT_MENU'
            ...
            onclick: function(event, item) {
              ...
              itemInfo.link = BX.util.htmlspecialcharsback(
                  link.pathname + link.search);

              //IE11 omits slash in the pathname
              itemInfo.link = itemInfo.link[0] !== '/'
                  ? '/' + itemInfo.link
                  : itemInfo.link;
    ...
    BX.addCustomEvent('BX.Main.InterfaceButtons:onBeforeResetMenu',
        function(promises) {
    ...
    this.menuSelectedNode = BX.findChild(this.menuContainer,
        {className: 'menu-item-active'}, true, false);
    ...
    if (leftMenuSelectedUrl === currentPath || leftMenuSelectedUrl ===
        currentFullPath) {
  ...
  areMoreItemsShowed: function() {
    return BX.hasClass(BX('left-menu-hidden-items-block'),
        'menu-item-favorites-more-open') ? true : false;
  },
  ...
  animateShowingHiddenItems: function() {
    ...
    function animation(opening, hiddenBlock, maxHeight) {
      ...
      (new BX.easing({
        ...
        complete: function() {
          BX.toggleClass(BX('left-menu-hidden-items-block'),
  ...
  openMenuPopup: function(bindElement, menuItemId) {
    ...
    //set main page
    if (!this.isExtranet && itemType !== 'self' &&
        BX.previousSibling(itemNode).id != 'left-menu-empty-item' &&
        this.isPublicConverted) {
      contextMenuItems.push({
    ...
    contextMenuItems.push({
      text: this.isEditMode() ? BX.message('MENU_EDIT_READY_FULL') : BX.message(
          'MENU_SETTINGS_MODE'),
      className: 'menu-popup-no-icon',
      onclick: BX.proxy(function() {
        BX.PopupMenu.destroy('popup_' + menuItemId);
  ...
  showSettingsPopup: function(event) {
    ...
    if (this.isCurrentPageInLeftMenu && itemType === 'default') {
      itemPageToLeftMenu = {
        text: BX.message(this.isCurrentPageInLeftMenu
            ? 'MENU_DELETE_PAGE_FROM_LEFT_MENU'
            : 'MENU_ADD_PAGE_TO_LEFT_MENU'),
    ...
    var menuItems = [
      ...
      {
        text: this.isCollapsedMode ? BX.message('MENU_EXPAND') : BX.message(
            'MENU_COLLAPSE'),
        className: 'menu-popup-no-icon',
        onclick: BX.proxy(function() {
          BX.proxy_context.popupWindow.close();
  ...
  onTopMenuFirstItemChange: function(firstPageLink, firstNode) {
    ...
    var leftMenuNode = this.menuItemsBlock.querySelector(
        '[data-top-menu-id=\'' + topMenuId + '\']');
    if (BX.type.isDomNode(leftMenuNode)) {
      ...
      var leftMenuLink = BX.findChild(leftMenuNode,
          {tagName: 'a', className: 'menu-item-link'}, true, false);
      if (leftMenuLink) {
    ...
    if (BX.type.isDomNode(leftMenuNode) && BX.previousSibling(leftMenuNode) ===
        BX('left-menu-empty-item')) {
      var ajaxData = {
        sessid: BX.bitrix_sessid(),
        site_id: this.siteId,
  ...
  showSelfItemPopup: function(bindElement, itemInfo) {
    ...
    BX.PopupWindowManager.create('menu-self-item-popup', bindElement, {
      ...
      closeByEsc: true,
      titleBar: isEditMode ? BX.message('MENU_EDIT_SELF_PAGE') : BX.message(
          'MENU_ADD_SELF_PAGE'),
      content: popupContent,
      buttons: [
        (button = new BX.PopupWindowButton({
          text: isEditMode ? BX.message('MENU_SAVE_BUTTON') : BX.message(
              'MENU_ADD_BUTTON'),
  ...
  addItemToAll: function(menuItemId) {
    ...
    var itemLink = itemNode.getAttribute('data-link'),
        ...
        openInNewPage = BX.type.isDomNode(itemLinkNode) &&
            itemLinkNode.hasAttribute('target') &&
            itemLinkNode.getAttribute('target') === '_blank';
    ...
    BX.ajax({
      ...
      onsuccess: BX.proxy(function(json) {
        if (json.error) {
          ...
          this.showMessage(itemNode,
              BX.message('MENU_ITEM_WAS_DELETED_FROM_ALL'));
        }
      }, this),
    });
  },
  ...
  updateCounters: function(counters, send) {
    ...
    for (var id in counters) {
      ...
      var counter = BX(
          id === '**' ? 'menu-counter-live-feed' : 'menu-counter-' +
              id.toLowerCase(), true);
      if (counter) {
        if (id === '**') {
          ...
          valueToShow = this.livefeedCounter.value -
              this.livefeedCounter.decrementStack;
        } else {
        ...
        if (valueToShow > 0) {
          ...
          BX.addClass(counter.parentNode.parentNode.parentNode,
              'menu-item-with-index');
        } else {
          BX.removeClass(counter.parentNode.parentNode.parentNode,
              'menu-item-with-index');
    ...
    if (BX.type.isDomNode(BX('menu-hidden-counter'))) {
      ...
      BX('menu-hidden-counter').innerHTML = sumHiddenCounters > 99
          ? '99+'
          : sumHiddenCounters;
    }
  },
  ...
  decrementCounter: function(node, iDecrement) {
    ...
    if (node.id == 'menu-counter-live-feed') {
      ...
      var counterValue = this.livefeedCounter.value -
          this.livefeedCounter.decrementStack;

      if (counterValue > 0) {
        node.innerHTML = counterValue;
      } else {
        BX.removeClass(node.parentNode.parentNode.parentNode,
            'menu-item-with-index');
      }
    }
  },
  ...
  recountHiddenCounters: function() {
    ...
    var hiddenItems = BX.findChildren(BX('left-menu-hidden-items-list'),
        {className: 'menu-item-block'}, true);
    this.hiddenCounters = [];
    ...
    BX('menu-hidden-counter').innerHTML = curSumCounters > 99
        ? '99+'
        : curSumCounters;
  },
  ...
  checkMoreButton: function(status) {
    ...
    var hiddenItems = BX('left-menu-hidden-items-list').
        getElementsByClassName('menu-item-block');
  ...
  showItem: function(menuItemId) {
    ...
    if (BX.type.isDomNode(this.menuItemsBlock)) {
      ...
      this.menuItemsBlock.insertBefore(itemNode,
          BX('left-menu-hidden-items-block'));
    }
  ...
  addStandardItem: function(itemInfo, startX, startY) {
    if (typeof itemInfo !== 'object') {
      ...
      if (this.isCurrentPageStandard &&
          BX.type.isDomNode(this.topMenuSelectedNode)) {
        ...
        itemInfo = {
          ...
          link: BX.type.isNotEmptyString(this.currentPagePath)
              ? this.currentPagePath
              : this.topItemSelectedObj.URL,
      ...
      onsuccess: BX.proxy(function(json) {
        if (json.hasOwnProperty('error')) {
          ...
          if (json.hasOwnProperty('itemId')) {
            ...
            BX.onCustomEvent('BX.Bitrix24.LeftMenuClass:onMenuItemAdded',
                [itemInfo, this]);

            this.animateTopItemToLeft(itemInfo, startX, startY);

            this.showMessage(BX('uiToolbarStar'),
                BX.message('MENU_ITEM_WAS_ADDED_TO_LEFT'));
            BX('uiToolbarStar').title = BX.message(
                'MENU_DELETE_PAGE_FROM_LEFT_MENU');
      ...
      onsuccess: BX.proxy(function(json) {
        if (json.hasOwnProperty('error')) {
          ...
          if (json.hasOwnProperty('itemId')) {
            ...
            this.showMessage(BX('uiToolbarStar'),
                BX.message('MENU_ITEM_WAS_DELETED_FROM_LEFT'));
            BX('uiToolbarStar').title = BX.message(
                'MENU_ADD_PAGE_TO_LEFT_MENU');
  ...
  generateItemHtml: function(itemParams) {
    ...
    var newItemNode = BX.create('li', {
      attrs: {
        className: 'menu-item-block menu-item-no-icon-state '
            + (isCounterExisted && itemParams.counterValue
                ? ' menu-item-with-index'
                : ''),
    ...
    if (BX.type.isDomNode(this.menuItemsBlock)) {
      this.menuItemsBlock.insertBefore(newItemNode,
          BX('left-menu-hidden-items-block'));
    }
      ...
      if (BX.type.isDomNode(linkNode)) {
        ...
        if (itemParams.hasOwnProperty('openInNewPage')) {
          linkNode.setAttribute('target',
              itemParams.openInNewPage == 'Y' ? '_blank' : '');
          itemNode.setAttribute('data-new-page',
              itemParams.openInNewPage == 'Y' ? 'Y' : 'N');
        }
      }
    }
  ...
  setMainPage: function(itemId) {
    ...
    if (BX.type.isDomNode(this.menuItemsBlock)) {
      ...
      (new BX.easing({
        ...
        complete: BX.proxy(function() {
          this.menuItemsBlock.insertBefore(itemNode,
              BX('left-menu-empty-item').nextSibling);
  ...
  showConfirmWindow: function(options) {
    options = options || {};
    var id = BX.type.isNotEmptyString(options.id)
        ? options.id
    ...
    var popup = BX.PopupWindowManager.create(id, null, {
      content: '<div class="left-menu-confirm-popup">' +
          ...
          '</div>',
      titleBar: BX.type.isNotEmptyString(options.titleBar)
          ? options.titleBar
          : false,
      closeByEsc: true,
      closeIcon: true,
  ...
  menuItemDragStart: function() {
    ...
    this.itemDomBlank = dragElement.parentNode.insertBefore(
        BX.create('div', {style: {height: '0px'}}), dragElement); //remember original item place
    this.itemMoveBlank = BX.create('div',
        {style: {height: this.itemHeight + 'px'}}); //empty div
  ...
  menuItemDragHover: function(dest, x, y) {
    ...
    if (dest == dragElement) {
      this.itemDomBlank.parentNode.insertBefore(this.itemMoveBlank,
          this.itemDomBlank);
    } else if (dragElement.getAttribute('data-type') == 'self' && dest.id ===
        'left-menu-empty-item') {
			 // self-item cannot be moved on the first place
    } else {
      if (BX.findParent(dest, {className: 'menu-items'}))  //li is hovered
      {
        if (BX.nextSibling(dest))
          dest.parentNode.insertBefore(this.itemMoveBlank,
              BX.nextSibling(dest));
        else
  ...
  menuItemDragStop: function() {
    ...
    var firstItem = BX.findChild(this.menuContainer,
        {className: 'menu-item-block'}, true, false);
    if (BX.type.isDomNode(firstItem) && firstItem.getAttribute('data-type') ==
        'self') {
      this.showMessage(firstItem, BX.message('MENU_SELF_ITEM_FIRST_ERROR'),
          'right');
      this.menuItemsBlock.replaceChild(dragElement, this.itemDomBlank);
    } else if (this.itemMoveBlank &&
        BX.findParent(this.itemMoveBlank, {className: 'menu-items'})) {
      this.itemMoveBlank.parentNode.replaceChild(dragElement,
          this.itemMoveBlank);

      if (dragElement.parentNode.id == 'left-menu-hidden-items-list') {
        if (dragElement.getAttribute('data-status') == 'show' &&
            dragElement.getAttribute('data-counter-id')) {
          this.recountHiddenCounters();
        }

        dragElement.setAttribute('data-status', 'hide');
      } else {
        if (dragElement.getAttribute('data-status') == 'hide' &&
            dragElement.getAttribute('data-counter-id')) {
          this.recountHiddenCounters();
        }
      ...
      if (BX.type.isDomNode(prevItem) && prevItem.id ==
          'left-menu-empty-item' && !this.isExtranet) {
        this.showMessage(dragElement, BX.message('MENU_ITEM_MAIN_PAGE'),
            'right');
        analyticsLabel = '?analyticsLabel=mainPage&itemId=' +
  ...
  adjustAdminPanel: function() {
    ...
    if (rect.bottom > 0) {
      this.menuContainer.style.top = Math.max(rect.bottom,
          this.fixedAdminPanelHeight) + 'px';
    } else {
      this.menuContainer.style.top = Math.max(0, this.fixedAdminPanelHeight) +
          'px';
    }
  ...
  handleDocumentScroll: function() {
    ...
    if (window.pageXOffset > 0) {
      ...
      this.upButton.style.left = -window.pageXOffset +
          (this.isCollapsed() ? 0 : 172) + 'px';
    } else {
  ...
  switchToSlidingMode: function(enable, immediately) {
    if (enable === false) {
      ...
      if (BX.hasClass(this.mainTable, 'menu-sliding-mode')) {
        ...
        BX.removeClass(this.mainTable,
            'menu-sliding-mode menu-sliding-opening-mode');
      }
    } else if (this.isCollapsedMode &&
        !BX.hasClass(this.mainTable, 'menu-sliding-mode')) {
  ...
  handleSlidingTransitionEnd: function(event) {
    if (event.target === this.menuContainer) {
      BX.removeClass(this.mainTable,
          'menu-sliding-opening-mode menu-sliding-closing-mode');
    }
  },
  ...
  toggle: function(flag, fn) {
    ...
    if ((flag === true && isOpen) || (flag === false && !isOpen) ||
        BX.hasClass(this.mainTable, 'menu-animation-mode')) {
      return;
    }
    ...
    var logoImageContainer = this.menuHeader.querySelector(
        '.logo-image-container');
    if (logoImageContainer) {
      var logoWidth = this.header.querySelector(
          '.logo-image-container').offsetWidth;
      if (logoWidth > 0) {
    ...
    BX.addClass(
        this.mainTable,
        'menu-animation-mode ' +
        (isOpen ? 'menu-animation-closing-mode' : 'menu-animation-opening-mode'),
    );

    var menuLinks = [].slice.call(
        leftColumn.querySelectorAll('.menu-item-link'));
    ...
    var menuEmployeesText = leftColumn.querySelector(
        '.menu-invite-employees-text');
    var menuEmployeesIcon = leftColumn.querySelector('.menu-invite-icon-box');

    var licenseContainer = leftColumn.querySelector(
        '.menu-license-all-container');
    var licenseBtn = leftColumn.querySelector('.menu-license-all-default');
    var licenseCollapsedBtn = leftColumn.querySelector(
        '.menu-license-all-collapsed');
    ...
    (new BX.easing({
      ...
      step: function(state) {
        ...
        if (pageHeader) {
          pageHeader.style.maxWidth = 'calc(100vw - ' + state.sidebarWidth +
              'px - ' + imBarWidth + 'px)';
        }
        ...
        if (isOpen) {
          //Closing Mode
          if (menuSitemapIcon) {
            menuSitemapIcon.style.transform = 'translateX(' +
                state.translateIcon + 'px)';
            menuSitemapIcon.style.opacity = state.opacityRevert / 100;
          }

          if (menuSitemapText) {
            menuSitemapText.style.transform = 'translateX(' +
                state.translateText + 'px)';
            menuSitemapText.style.opacity = state.opacity / 100;
          }

          if (menuEmployeesIcon) {
            menuEmployeesIcon.style.transform = 'translateX(' +
                state.translateIcon + 'px)';
            menuEmployeesIcon.style.opacity = state.opacityRevert / 100;
          }

          if (menuEmployeesText) {
            menuEmployeesText.style.transform = 'translateX(' +
                state.translateText + 'px)';
            menuEmployeesText.style.opacity = state.opacity / 100;
          }

          this.settingsIconBox.style.transform = 'translateX(' +
              state.translateIcon + 'px)';
          this.settingsIconBox.style.opacity = state.opacityRevert / 100;

          this.settingsBtnText.style.transform = 'translateX(' +
              state.translateText + 'px)';
          this.settingsBtnText.style.opacity = state.opacity / 100;

          menuMoreBtn.style.transform = 'translateX(' + state.translateIcon +
              'px)';
          menuMoreBtn.style.opacity = state.opacityRevert / 100;

          menuMoreBtnDefault.style.transform = 'translateX(' +
              state.translateMoreBtn + 'px)';
          menuMoreBtnDefault.style.opacity = state.opacity / 100;

          if (menuMoreCounter) {
            menuMoreCounter.style.transform = 'translateX(' +
                state.translateIcon + 'px)';
            menuMoreCounter.style.opacity = state.opacityRevert / 100;
          }

          if (licenseContainer) {
            licenseBtn.style.transform = 'translateX(' +
                state.translateLicenseBtn + 'px)';
            licenseBtn.style.opacity = state.opacity / 100;

            licenseCollapsedBtn.style.transform = 'translateX(' +
                state.translateIcon + 'px)';
          ...
          menuLinks.forEach(function(item) {
            ...
            menuLinkText.style.transform = 'translateX(' + state.translateText +
                'px)';
            menuLinkText.style.opacity = state.opacity / 100;

            menuIcon.style.transform = 'translateX(' + state.translateIcon +
                'px)';
            menuIcon.style.opacity = state.opacityRevert / 100;

            if (menuCounter) {
              menuCounter.style.transform = 'translateX(' +
                  state.translateIcon + 'px)';
      ...
      for (var j = 0, l = itemLinks.length; j < l; j++) {
        ...
        if (isItemSelected) {
          ...
          if (
              ...
              (newLength === curSelectedLen && itemPriority >
                  curSelectedPriority)
          ) {
            curSelectedItem = i;
  ...
  isItemSelected: function(url, currentUrl) {
    ...
    if (currentUrl.indexOf(url) === 0 || currentUrlWithIndex.indexOf(url) ===
        0) {
      return true;
    }
    ...
    for (var varName in params) {
      ...
      var globalValues = typeof (globals[varName]) !== 'undefined'
          ? globals[varName]
          : [];
  ...
  getUrlWithIndex: function(url) {
    ...
    var questionPos = url.indexOf('?');
    var queryString = questionPos >= 0
        ? '?' + url.substring(questionPos + 1)
        : '';
  ...
  checkCurrentPageInTopMenu: function() {
    ...
    if (BX.Main && BX.Main.interfaceButtonsManager) {
      ...
      if (menuIds[0]) {
        ...
        if (typeof this.topItemSelectedObj === 'object' &&
            this.topItemSelectedObj) {
          ...
          //IE11 omits slash in the pathname
          var path = link.pathname[0] !== '/'
              ? '/' + link.pathname
              : link.pathname;

          this.topItemSelectedObj.URL = BX.util.htmlspecialcharsback(
              path + link.search);
          this.topItemSelectedObj.TEXT = BX.util.htmlspecialcharsback(
              this.topItemSelectedObj.TEXT);
  ...
  getStructureForHelper: function() {
    ...
    if (BX.type.isDomNode(this.menuContainer)) {
      ...
      for (var type in types) {
        var curItems = this.menuContainer.querySelectorAll(
            '[data-status=\'' + types[type] + '\']');
        if (curItems) {
          var curItemsId = [];
  ...
  initPagetitleStar: function() {
    ...
    starContNode.title = BX.message(
        BX.hasClass(starContNode, 'ui-toolbar-star-active')
            ? 'MENU_DELETE_PAGE_FROM_LEFT_MENU'
            : 'MENU_ADD_PAGE_TO_LEFT_MENU');

    //default page
    if (typeof currentPageInMenu == 'object' &&
        currentPageInMenu.getAttribute('data-type') === 'default') {
      starContNode.title = BX.message('MENU_STAR_TITLE_DEFAULT_PAGE');
      BX.bind(starContNode, 'click', BX.proxy(function() {
        this.showMessage(BX.proxy_context,
            BX.message('MENU_STAR_TITLE_DEFAULT_PAGE_DELETE_ERROR'));
      }, this));
  ...
  initPreset: function() {
    ...
    this.presetItems = container.getElementsByClassName(
        'js-left-menu-preset-item');
      ...
      BX.PopupWindowManager.create('menu-preset-popup', null, {
        ...
        buttons: [
          (button = new BX.PopupWindowButton({
            ...
            events: {
              click: BX.proxy(function() {
                if (BX.hasClass(button.buttonNode,
              ...
              BX.ajax({
                ...
                onsuccess: BX.proxy(function() {
                  ...
                  BX.PopupWindowManager.create(
                      'menu-custom-preset-success-popup', null, {
                        closeIcon: true,
                        contentColor: 'white',
                        titleBar: BX.message('MENU_CUSTOM_PRESET_POPUP_TITLE'),
