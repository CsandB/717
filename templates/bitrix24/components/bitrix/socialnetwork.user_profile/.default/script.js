...
BX.Socialnetwork.User.Profile = (function() {
  ...
  Profile.prototype.init = function() {
    if (this.showOtpPopup) {
      ...
      BX.PopupWindowManager.create('securityOtpSuccessPopup', null, {
        ...
        closeByEsc: false,
        content: '<div style="width:450px;min-height:100px; padding:15px;font-size:14px;">' +
            BX.message('SONET_OTP_SUCCESS_POPUP_TEXT') +
            (this.otpRecoveryCodes ? BX.message(
                'SONET_OTP_SUCCESS_POPUP_TEXT_RES_CODE') : '') +
            '<div style="background-color: #fdfaea;padding: 10px;border-color: #e5e0c4 #f1edd7 #f9f6e4;border-style: solid;border-width: 1px;border-radius: 2px;">' +
            BX.message('SONET_OTP_SUCCESS_POPUP_TEXT2') + '</div></div>',
        buttons: buttons,
      }).show();
    }
    ...
    BX.ajax({
      ...
      onsuccess: function(json) {
        if (json.result == 'success') {
          ...
          var DeleteErrorPopup = BX.PopupWindowManager.create('delete_error',
              this, {
  ...
  Profile.prototype.showExtranet2IntranetForm = function(userId, email) {
    ...
    window.Bitrix24Extranet2IntranetForm = BX.PopupWindowManager.create(
        'BXExtranet2Intranet', null, {
          autoHide: false,
          zIndex: 0,
          offsetLeft: 0,
          offsetTop: 0,
  ...
  Profile.prototype.reinvite = function(userId, isExtranet, bindObj) {
    ...
    var reinvite = 'reinvite_user_id_' +
        (isExtranet == 'Y' ? 'extranet_' : '') + userId;
    ...
    BX.ajax.post(
        ...
        BX.delegate(function(result) {
          var InviteAccessPopup = BX.PopupWindowManager.create('invite_access',
              bindObj, {
  ...
  Profile.prototype.showHint = function(id, bind, text) {
    ...
    this.popupHint[id] = new BX.PopupWindow('user-profile-email-hint', bind, {
      ...
      content: BX.create('div',
          {attrs: {style: 'padding-right: 5px; width: 250px;'}, html: text}),
    });
