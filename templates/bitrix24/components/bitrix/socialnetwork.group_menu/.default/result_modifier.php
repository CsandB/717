...
$urlGeneralSpecific
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_GROUP_GENERAL"],
...
if (
    ...
    if (
        ...
        foreach ($menuItems as $menuItem) {
            ...
            $firstMenuItem = preg_match('/^'.$arResult["menuId"].'_(.*)$/i',
                $menuItem, $matches);
            if ( ! empty($matches)) {
                if (
...
if ($this->__component->__parent && $this->__component->__parent->arResult
    && array_key_exists("PATH_TO_GROUP_CARD",
        $this->__component->__parent->arResult)
) {
    $arParams["PATH_TO_GROUP_CARD"]
        = $this->__component->__parent->arResult["PATH_TO_GROUP_CARD"];
}
if ($this->__component->__parent && $this->__component->__parent->arResult
    && array_key_exists("PATH_TO_MESSAGE_TO_GROUP",
        $this->__component->__parent->arResult)
) {
    $arParams["PATH_TO_MESSAGE_TO_GROUP"]
        = $this->__component->__parent->arResult["PATH_TO_MESSAGE_TO_GROUP"];
}
if ($this->__component->__parent && $this->__component->__parent->arResult
    && array_key_exists("PATH_TO_GROUP_FEATURES",
        $this->__component->__parent->arResult)
) {
    $arParams["PATH_TO_GROUP_FEATURES"]
        = $this->__component->__parent->arResult["PATH_TO_GROUP_FEATURES"];
}
if ($this->__component->__parent && $this->__component->__parent->arResult
    && array_key_exists("PATH_TO_GROUP_DELETE",
        $this->__component->__parent->arResult)
) {
    $arParams["PATH_TO_GROUP_DELETE"]
        = $this->__component->__parent->arResult["PATH_TO_GROUP_DELETE"];
}
if ($this->__component->__parent && $this->__component->__parent->arResult
    && array_key_exists("PATH_TO_GROUP_REQUESTS_OUT",
        $this->__component->__parent->arResult)
) {
    $arParams["PATH_TO_GROUP_REQUESTS_OUT"]
        = $this->__component->__parent->arResult["PATH_TO_GROUP_REQUESTS_OUT"];
}
if ($this->__component->__parent && $this->__component->__parent->arResult
    && array_key_exists("PATH_TO_USER_REQUEST_GROUP",
        $this->__component->__parent->arResult)
) {
    $arParams["PATH_TO_USER_REQUEST_GROUP"]
        = $this->__component->__parent->arResult["PATH_TO_USER_REQUEST_GROUP"];
}
if ($this->__component->__parent && $this->__component->__parent->arResult
    && array_key_exists("PATH_TO_USER_LEAVE_GROUP",
        $this->__component->__parent->arResult)
) {
    $arParams["PATH_TO_USER_LEAVE_GROUP"]
        = $this->__component->__parent->arResult["PATH_TO_USER_LEAVE_GROUP"];
}
if ($this->__component->__parent && $this->__component->__parent->arResult
    && array_key_exists("PATH_TO_GROUP_SUBSCRIBE",
        $this->__component->__parent->arResult)
) {
    $arParams["PATH_TO_GROUP_SUBSCRIBE"]
        = $this->__component->__parent->arResult["PATH_TO_GROUP_SUBSCRIBE"];
}

if ($this->__component->__parent && $this->__component->__parent->arParams
    && array_key_exists("GROUP_USE_BAN", $this->__component->__parent->arParams)
) {
    $arParams["GROUP_USE_BAN"]
        = $this->__component->__parent->arParams["GROUP_USE_BAN"];
}
$arParams["GROUP_USE_BAN"] = $arParams["GROUP_USE_BAN"] != "N" ? "Y" : "N";

if (intval($arResult["Group"]["IMAGE_ID"]) <= 0) {
    $arResult["Group"]["IMAGE_ID"] = COption::GetOptionInt("socialnetwork",
        "default_group_picture", false, SITE_ID);
}
...
$arResult["Urls"]["Card"]
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_GROUP_CARD"],
    array("group_id" => $arResult["Group"]["ID"]));
$arResult["Urls"]["MessageToGroup"]
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_MESSAGE_TO_GROUP"],
    array("group_id" => $arResult["Group"]["ID"]));
$arResult["Urls"]["Features"]
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_GROUP_FEATURES"],
    array("group_id" => $arResult["Group"]["ID"]));
$arResult["Urls"]["Delete"]
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_GROUP_DELETE"],
    array("group_id" => $arResult["Group"]["ID"]));
$arResult["Urls"]["GroupRequestsOut"]
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_GROUP_REQUESTS_OUT"],
    array("group_id" => $arResult["Group"]["ID"]));
$arResult["Urls"]["UserRequestGroup"]
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER_REQUEST_GROUP"],
    array("group_id" => $arResult["Group"]["ID"]));
$arResult["Urls"]["UserLeaveGroup"]
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER_LEAVE_GROUP"],
    array("group_id" => $arResult["Group"]["ID"]));
$arResult["Urls"]["Subscribe"]
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_GROUP_SUBSCRIBE"],
    array("group_id" => $arResult["Group"]["ID"]));
$arResult["Urls"]["GroupsList"] = ComponentHelper::getWorkgroupSEFUrl();
$arResult["Urls"]["Copy"]
    = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_GROUP_COPY"],
    array("group_id" => $arResult["Group"]["ID"]));

$arResult["CanView"]["chat"] = (
    array_key_exists("chat", $arResult["ActiveFeatures"])
    && in_array($arResult["CurrentUserPerms"]["UserRole"],
        UserToGroupTable::getRolesMember())
);
$arResult["CanView"]["general"] = true;

$arResult["Title"]["chat"] = ((array_key_exists("chat",
        $arResult["ActiveFeatures"])
    && $arResult["ActiveFeatures"]["chat"] <> '')
    ? $arResult["ActiveFeatures"]["chat"] : GetMessage("SONET_UM_CHAT"));
...
if (
    ...
    $arResult["bShowRequestSentMessage"]
        = $arResult["CurrentUserPerms"]["InitiatedByType"];
    ...
    if (empty($arParams["PATH_TO_USER_REQUESTS"])) {
        $arParams["PATH_TO_USER_REQUESTS"] = ComponentHelper::getUserSEFUrl()
            .'user/#user_id#/requests/';
    }

    $arResult["Urls"]["UserRequests"]
        = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER_REQUESTS"],
...
$arResult['Group']['TypeCode'] = Workgroup::getTypeCodeByParams(array(
    'fields'   => array(
        ...
        'EXTERNAL' => (isset($arResult['Group']['IS_EXTRANET'])
        && $arResult['Group']['IS_EXTRANET'] == 'Y' ? 'Y' : 'N'),
    ),
    'fullMode' => true,
));
...
if ($USER->IsAuthorized()) {
    $res = WorkgroupFavoritesTable::getList(array(
        'filter' => array(
            'GROUP_ID' => $arResult["Group"]["ID"],
            'USER_ID'  => $USER->getId(),
        ),
