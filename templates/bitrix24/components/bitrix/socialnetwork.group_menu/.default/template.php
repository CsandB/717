...
Extension::load(["socialnetwork.common", "ui.icons.b24", "ui.buttons"]);
...
$APPLICATION->SetPageProperty("BodyClass",
    ($bodyClass ? $bodyClass." " : "")."profile-menu-mode");
        ...
        <script>
          BX.ready(function() {
            BX.SocialnetworkUICommon.showRecallJoinRequestPopup({
              ...
              PROJECT: <?=($arResult["Group"]["PROJECT"] == "Y" ? 'true'
                  : 'false')?>,
            });
          });
        </script><?
    } elseif ($arResult["bShowRequestSentMessage"]
...
<script>
  BX.ready(function() {
    BX.message({
      SGMPathToRequestUser: '<?=CUtil::JSUrlEscape(
          ! empty($arResult["Urls"]["Invite"])
              ...
              : $arResult["Urls"]["Edit"].(mb_strpos($arResult["Urls"]["Edit"],
                  "?") !== false ? "&" : '?')."tab=invite"
      )?>',
      ...
      SGMPathToEdit: '<?=CUtil::JSUrlEscape($arResult["Urls"]["Edit"]
    ...
    BX.BXSGM24.init({
      ...
      canInitiate: <?=($arResult["CurrentUserPerms"]["UserCanInitiate"]
      && ! $arResult["HideArchiveLinks"] ? 'true' : 'false')?>,
      canProcessRequestsIn: <?=($arResult["CurrentUserPerms"]["UserCanProcessRequestsIn"]
      && ! $arResult["HideArchiveLinks"] ? 'true' : 'false')?>,
      canModify: <?=($arResult["CurrentUserPerms"]["UserCanModifyGroup"]
      && ! $arResult["HideArchiveLinks"] ? 'true' : 'false')?>,
      userRole: '<?=$arResult["CurrentUserPerms"]["UserRole"]?>',
      userIsMember: <?=($arResult["CurrentUserPerms"]["UserIsMember"] ? 'true'
          : 'false')?>,
      userIsAutoMember: <?=(isset($arResult["CurrentUserPerms"]["UserIsAutoMember"])
      && $arResult["CurrentUserPerms"]["UserIsAutoMember"] ? 'true'
          : 'false')?>,
      editFeaturesAllowed: <?=(Workgroup::getEditFeaturesAvailability() ? 'true'
          : 'false')?>,
      urls: {
        group: '<?=CUtil::JSEscape(! empty($arResult["Urls"]["General"])
            ? $arResult["Urls"]["General"] : $arResult["Urls"]["View"])?>',
...
<div class="profile-menu profile-menu-group">
    <div class="profile-menu-inner">
        <div class="profile-menu-top">
            <a href="<?= $arResult["Urls"]["View"] ?>"
               class="ui-icon ui-icon-common-user-group profile-menu-avatar">
            ...
            <div class="profile-menu-info<?= ($arResult["Group"]["IS_EXTRANET"]
            == "Y" ? " profile-menu-group-info-extranet" : "") ?>">
                <a href="<?= $arResult["Urls"]["View"] ?>"
                   class="profile-menu-name"><?= $arResult["Group"]["NAME"] ?></a>
                <div class="profile-menu-type">
					<span class="profile-menu-type-name">
						<span class="profile-menu-type-name-item"><?= (is_array($arResult['Group']['Type'])
                            && ! empty($arResult['Group']['Type'])
                            && ! empty($arResult['Group']['Type']['NAME'])
                                ? (LANGUAGE_ID == 'de'
                                    ? $arResult['Group']['Type']['NAME']
                                    : mb_strtolower($arResult['Group']['Type']['NAME']))
                                : '') ?></span><?
                        if ($arResult["CurrentUserPerms"]["UserCanModifyGroup"]) {
                            ?><a
                            href="<?= htmlspecialcharsbx($arResult["Urls"]["Edit"]
                                .(mb_strpos($arResult["Urls"]["Edit"], "?")
                                !== false ? "&" : '?')."tab=edit") ?>"
                            class="profile-menu-type-icon"></a><?
                        }
                        ?></span>
                </div><?
                ...
                ?><span class="profile-menu-links"><?
                    ?><a href="<?= $arResult["Urls"]["Card"] ?>"
                         class="profile-menu-links-item"><?= Loc::getMessage($arResult["Group"]["PROJECT"]
                        == "Y" ? "SONET_SGM_T_LINKS_ABOUT_PROJECT"
                            : "SONET_SGM_T_LINKS_ABOUT") ?></a><?

                    ?><a href="<?= $arResult["Urls"]["GroupUsers"] ?>"
                         class="profile-menu-links-item"><?
                        if (intval($arResult['Group']['NUMBER_OF_MEMBERS'])
                            > 0
                        ) {
                            echo Loc::getMessage("SONET_SGM_T_MEMBERS2",
                                array('#NUM#' => intval($arResult['Group']['NUMBER_OF_MEMBERS'])));
                        } else {
                    ...
                    if (
                        ...
                        ?><a href="<?= $arResult["Urls"]["GroupRequests"] ?>"
                             class="profile-menu-links-count">
                        +<?= intval($arResult['Group']['NUMBER_OF_REQUESTS']) ?></a><?
                    }
                    ...
                    if (
                        ...
                        ?><a id="bx-group-menu-settings"
                             href="javascript:void(0);"
                             class="profile-menu-links-item"><?= Loc::getMessage("SONET_UM_ACTIONS_BUTTON") ?></a><?
                    }
                    ?></span><?

                if ($arResult["bUserCanRequestGroup"]) {
                    ?><span id="bx-group-menu-join-cont"
                    ...
                    if (
                        ...
                        <button class="ui-btn ui-btn-sm ui-btn-primary"
                                id="bx-group-menu-join"
                                bx-request-url="<?= $arResult["Urls"]["UserRequestGroup"] ?>"><?= Loc::getMessage('SONET_SGM_T_BUTTON_JOIN') ?></button><?
                    } else {
                        ?><a class="ui-btn ui-btn-sm ui-btn-primary"
                             href="<?= $arResult["Urls"]["UserRequestGroup"] ?>"><?= Loc::getMessage('SONET_SGM_T_BUTTON_JOIN') ?></a><?
                    }
                ...
                foreach ($arResult["CanView"] as $key => $val) {
                    ...
                    if ($key == 'general') {
                        $menuItems[] = array(
                            ...
                            "URL"       => ($arResult["Urls"]["General"]
                                ? $arResult["Urls"]["General"]
                                : $arResult["Urls"]["View"]),
                            "ID"        => "general",
                            "IS_ACTIVE" => in_array($arParams["PAGE_ID"],
                                array("group", "group_general")),
                        );
                    } else {
                        $item = array(
                            "TEXT"      => $arResult["Title"][$key],
                            "ID"        => $key,
                            "IS_ACTIVE" => ($arParams["PAGE_ID"] == "group_"
                                .$key),
                        );
