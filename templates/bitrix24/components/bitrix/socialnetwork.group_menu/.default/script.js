(function() {
  ...
  BX.BXSGM24.init = function(params) {
    ...
    this.editFeaturesAllowed = (typeof params.editFeaturesAllowed != 'undefined'
        ? !!params.editFeaturesAllowed
        : true);

    var f = BX.delegate(function(eventData) {
      if (BX.type.isNotEmptyString(eventData.code)) {
        if (BX.util.in_array(eventData.code,
            ['afterJoinRequestSend', 'afterEdit'])) {
    ...
    if (BX('bx-group-menu-join')) {
      BX.bind(BX('bx-group-menu-join'), 'click',
    ...
    if (BX('bx-group-menu-settings')) {
      ...
      BX.bind(BX('bx-group-menu-settings'), 'click',
          BX.delegate(function(event) {
