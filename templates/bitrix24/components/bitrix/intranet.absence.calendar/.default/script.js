var jsBXAC = {
  ...
  Init: function(arParams) {
    ...
    this.SETTINGS.NAME_TEMPLATE = arParams.NAME_TEMPLATE != ''
        ? arParams.NAME_TEMPLATE
        : this.SETTINGS.NAME_TEMPLATE;
    this.SETTINGS.SERVER_TIMEZONE_OFFSET = arParams.SERVER_TIMEZONE_OFFSET +
        (new Date()).getTimezoneOffset() * 60;
    this.SETTINGS.FIRST_DAY = null != arParams.FIRST_DAY
        ? arParams.FIRST_DAY
        : 1;
    this.SETTINGS.DAY_START = null != arParams.DAY_START
        ? arParams.DAY_START
        : 8;
    this.SETTINGS.DAY_FINISH = null != arParams.DAY_FINISH
        ? arParams.DAY_FINISH
        : 18;
    this.SETTINGS.DAY_SHOW_NONWORK = null != arParams.DAY_SHOW_NONWORK
        ? arParams.DAY_SHOW_NONWORK
        : false;
    ...
    this.SETTINGS.PAGE_NUMBER = arParams.PAGE_NUMBER > 0
        ? arParams.PAGE_NUMBER
        : 0;
  ...
  Show: function(MAIN_LAYOUT, VIEW) {
    ...
    if (null == this.VIEWS[this.CURRENT_VIEW]) return this.__showError(
        'ERR_VIEW_NOT_REGISTERED', this.CURRENT_VIEW);
  ...
  SetViewHandler: function(obHandler) {
    ...
    if (obHandler.ID == this.CURRENT_VIEW) {
      this.CURRENT_VIEW_HANDLER && this.CURRENT_VIEW_HANDLER.Unload &&
      this.CURRENT_VIEW_HANDLER.Unload();
      ...
      this.CURRENT_VIEW_HANDLER.SetSettings &&
      this.CURRENT_VIEW_HANDLER.SetSettings(this.SETTINGS);
  ...
  LoadData: function(ts_start, ts_finish) {
    ...
    document.getElementById('bx_goto_date').value = BX.calendar.ValueToString(
        ts_start, false, false);
    ...
    var url = this.LOADER
        + '?MODE=GET'
        + '&TS_START=' + (parseInt(ts_start.valueOf() / 1000) -
            this.SETTINGS.SERVER_TIMEZONE_OFFSET)
        + '&TS_FINISH=' + (parseInt(ts_finish.valueOf() / 1000) - 1 -
            this.SETTINGS.SERVER_TIMEZONE_OFFSET)
        + this.GetDataFilter()
  ...
  SetData: function(DATA, current_data_id, page_number, page_count) {
    ...
    for (var i = 0, dateTo; i < DATA.length; i++) {
      if (null != DATA[i].DATA) {
        for (var j = 0; j < DATA[i].DATA.length; j++) {
          DATA[i].DATA[j].DATE_ACTIVE_FROM = BX.parseDate(
              DATA[i].DATA[j].DATE_FROM);
          DATA[i].DATA[j].DATE_ACTIVE_TO = BX.parseDate(
              DATA[i].DATA[j].DATE_TO);

          if (DATA[i].DATA[j].DATE_ACTIVE_FROM >
              DATA[i].DATA[j].DATE_ACTIVE_TO) {
          ...
          if (DATA[i].DATA[j].DATE_ACTIVE_FROM.valueOf() != dateTo.valueOf()) {
            if (dateTo.getHours() + dateTo.getMinutes() + dateTo.getSeconds() >
                0) {
              continue;
            }
          }

          DATA[i].DATA[j].DATE_ACTIVE_TO.setDate(
              DATA[i].DATA[j].DATE_ACTIVE_TO.getDate() + 1);
          DATA[i].DATA[j].DATE_ACTIVE_TO.setSeconds(
              DATA[i].DATA[j].DATE_ACTIVE_TO.getSeconds() - 1);
        }
      }
    }
    ...
    this.CURRENT_VIEW_HANDLER.LoadData &&
    this.CURRENT_VIEW_HANDLER.LoadData(this.DATA);
  ...
  ShowLayout: function() {
    ...
    this.TOOLBAR.BXAddControl = function(n, o) {
      ...
      if (r.cells.length > 0)
        r.insertCell(
            r.cells.length - 1).className = 'bx-calendar-toolbar-delimiter';
      else
    ...
    (this.__ShowViewSwitcher()) &&
    (this.MAIN_LAYOUT.appendChild(this.CONTROLS.VIEWSWITCHER));
    ...
    if (null != this.SETTINGS.CONTROLS.DATEPICKER) {
      var obCalendarContainer = document.getElementById(
          'bx_calendar_control_datepicker');
    ...
    if (null != this.SETTINGS.CONTROLS.TYPEFILTER) {
      ...
      this.TOOLBAR.BXAddControl(this.MESSAGES.IAC_FILTER_TYPEFILTER,
          this.CONTROLS.TYPEFILTER);

      new JCCalendarFilter(function(a, b) {_this.SetDataFilter(a, b);},
          this.CONTROLS.TYPEFILTER, this.TYPES, this.MESSAGES,
          this.TYPE_BGCOLORS);
    }
    ...
    if (null != this.SETTINGS.CONTROLS.SHOW_ALL) {
      ...
      this.CONTROLS.SHOW_ALL.onclick = function() {
        _this.SetDataFilter('USERS_ALL', this.checked ? 'N' : 'Y');
      };

      this.TOOLBAR.BXAddControl(this.MESSAGES.IAC_FILTER_SHOW_ALL,
          this.CONTROLS.SHOW_ALL);
    }

    if (null != this.SETTINGS.CONTROLS.DEPARTMENT) {
      var obDepartmentsContainer = document.getElementById(
          'bx_calendar_conrol_departments');
      if (null != obDepartmentsContainer) {
        this.CONTROLS.DEPARTMENT = obDepartmentsContainer.firstChild;
        while (this.CONTROLS.DEPARTMENT && this.CONTROLS.DEPARTMENT.tagName !=
        'SELECT')
          this.CONTROLS.DEPARTMENT = this.CONTROLS.DEPARTMENT.nextSibling;

        this.CONTROLS.DEPARTMENT.parentNode.removeChild(
            this.CONTROLS.DEPARTMENT);

        this.CONTROLS.DEPARTMENT.onchange = function() {
          _this.SetDataFilter('DEPARTMENT', this.value);
        };

        this.TOOLBAR.BXAddControl(this.MESSAGES.IAC_FILTER_DEPARTMENT,
            this.CONTROLS.DEPARTMENT);
      }
    }
  },
  ...
  __ShowViewSwitcher: function() {
    ...
    if ((cnt = arViewList.length) > 1) {
      ...
      for (var i = 0; i < cnt; i++) {
        var obItem = this.CONTROLS.VIEWSWITCHER.appendChild(
  ...
  SetView: function(view) {
    ...
    if (null == this.VIEWS[view]) return this.__showError(
        'ERR_VIEW_NOT_REGISTERED', view);
    ...
    this.CURRENT_VIEW_HANDLER && this.CURRENT_VIEW_HANDLER.BeforeUnload &&
    this.CURRENT_VIEW_HANDLER.BeforeUnload();

    BX.ajax.insertToNode(this.LOADER + '?MODE=VIEW&VIEW=' + this.CURRENT_VIEW,
        this.CONTROLS.CALENDAR, false);
  },
  ...
  RegisterEntry: function(arEntry) {
    ...
    if (null != arEntry.VISUAL) {
      ...
      if (null == this._INFO_CACHE[arEntry.ENTRY_TYPE + ':' + arEntry.ID])
        this._INFO_CACHE[arEntry.ENTRY_TYPE + ':' +
        arEntry.ID] = new JCCalendarInfoWin(arEntry.ID, arEntry.ENTRY_TYPE,
            arEntry.USER_ID, this.LOADER, this.TYPE_BGCOLORS);

      arEntry.VISUAL.INFO = this._INFO_CACHE[arEntry.ENTRY_TYPE + ':' +
      arEntry.ID];
  ...
  UnRegisterEntry: function(arEntry) {
    if (null != arEntry.VISUAL) {
      if (null != this._INFO_CACHE && null !=
          this._INFO_CACHE[arEntry.ENTRY_TYPE + ':' + arEntry.ID]) {
  ...
  FormatName: function(FORMAT, arData) {
    ...
    var SECOND_NAME_SHORT = SECOND_NAME
        ? SECOND_NAME.substring(0, 1) + '.'
        : '';
    ...
    if (FORMAT.indexOf('#LAST_NAME#') >= 0 ||
        FORMAT.indexOf('#LAST_NAME_SHORT#') >= 0)
      res_check += arData['LAST_NAME'];
    if (FORMAT.indexOf('#SECOND_NAME#') >= 0 ||
        FORMAT.indexOf('#SECOND_NAME_SHORT#') >= 0)
  ...
  getEditUrl: function(INFO, bPublicEdit) {
    if (null != window.jsBXCalendarAdmin) {
      ...
      var url = '/bitrix/admin/iblock_element_edit.php'
          + '?type=' + jsBXCalendarAdmin.IBLOCK_TYPE + '&lang=' +
          jsBXCalendarAdmin.LANG;
  ...
  __savePosition: function() {
    window.location.hash =
        'AP:' + this.CURRENT_VIEW + '|' +
        this.__last_date_params.TS_START.valueOf() + '|' +
        this.__last_date_params.TS_FINISH.valueOf();
  },
...
function JCCalendarFilter(
    _callback, _parentNode, _arTypes, MESSAGES, typeBgColors) {
  var _this = this;
  ...
  this.LAYOUT.innerHTML = '<div class="bx-filter-title">' +
      ...
      '	<tr>' +
      '		<td class="bx-filter-title-text" id="bx_filter_title_text">' +
      MESSAGES.INTR_ABSC_TPL_FILTER_OFF +
      '</td><td width="0%"><a class="bx-filter-close" id="bx_filter_close"></a></td></tr>' +
      '</table>' +
      '</div>';
  ...
  var obLabel = document.createElement('SPAN');
  obLabel.className = 'bx-label';
  obLabel.innerHTML = this._parentNode.previousSibling.innerHTML;
  this._parentNode.previousSibling.parentNode.insertBefore(obLabel,
      this._parentNode.previousSibling);
  this._parentNode.previousSibling.parentNode.removeChild(
      this._parentNode.previousSibling);
  ...
  this.Run = function() {
    ...
    document.getElementById(
        'bx_filter_title_text').innerHTML = MESSAGES.INTR_ABSC_TPL_FILTER_OFF;
    ...
    for (var i = 0; i < _this.arTypes.length; i++) {
      if (!_this.arCurrentChecked[_this.arTypes[i].NAME]) {
        ...
        document.getElementById(
            'bx_filter_title_text').innerHTML = MESSAGES.INTR_ABSC_TPL_FILTER_ON;
...
function JCCalendarInfoWin(
    entry_id, entry_type, user_id, loader, typeBgColors) {
  this.ID = entry_id;
    ...
    if (null != data) {
      ...
      for (var i = 0; i < _this.INFO.USER.UF_DEPARTMENT.length; i++) {
        departments += (departments.length > 0 ? ', ' : '')
            + '<a href="' +
            jsBXAC.SETTINGS.DETAIL_URL_DEPARTMENT.replace(/#ID#/g,
                _this.INFO.USER.UF_DEPARTMENT[i].ID) + '">'
            + BX.util.htmlspecialchars(_this.INFO.USER.UF_DEPARTMENT[i].NAME) +
            '</a>';
      }
      ...
      if (_this.TYPE == 1) {
        if (null != window.jsBXCalendarAdmin) {
          strAdmin += '<div class="bx-calendar-info-admin">'
              +
              '<a href="javascript:void(0)" class="bx-calendar-delete" id="bx_calendar_entry_delete_' +
              _this.ID + '">'
              + window.jsBXCalendarAdmin.DELETE
              + '</a>'
              + '<a href="javascript:void(0)" onclick="' +
              GetAbsenceDialog(_this.ID) +
              '" class="bx-calendar-edit" id="bx_calendar_entry_edit_' +
              _this.ID + '">'
        ...
        var strDate = _this.INFO.ENTRY.DATE_ACTIVE_FROM + ' - ' +
            _this.INFO.ENTRY.DATE_ACTIVE_TO;
      } else {
        strAdmin += '<div class="bx-calendar-info-admin">'
            + '<a href="'
            + jsBXAC.SETTINGS.DETAIL_URL_PERSONAL.replace(/#USER_ID#/g,
                _this.USER_ID).replace(/#EVENT_ID#/g, _this.ID)
            + '" class="bx-calendar-personal">'
            + jsBXAC.MESSAGES.INTR_ABSC_TPL_PERSONAL_LINK_TITLE
            + '</a>'
            + '</div>';

        if ('NONE' == _this.INFO.ENTRY.PROPERTY_PERIOD_TYPE_VALUE || '' ==
            _this.INFO.ENTRY.PROPERTY_PERIOD_TYPE_VALUE) {
          var strDate = _this.INFO.ENTRY.DATE_ACTIVE_FROM + ' - ' +
              _this.INFO.ENTRY.DATE_ACTIVE_TO;
        } else {
          var strDate = jsBXAC.MESSAGES.INTR_ABSC_TPL_REPEATING_EVENT
              + ' (' + jsBXAC.MESSAGES['INTR_ABSC_TPL_REPEATING_EVENT_' +
              _this.INFO.ENTRY.PROPERTY_PERIOD_TYPE_VALUE] + ')';
        }
      }
      ...
      _this.DIV.innerHTML = '<div class="bx-calendar-info-header"><a class="bx-calendar-info-close" id="bx_top_close_' +
          _this.TYPE + '_' + _this.ID + '"></a></div>'
          + '<div class="bx-calendar-info-data">'
          + '<div class="bx-calendar-info-data-photo' +
          (_this.INFO.USER.PERSONAL_PHOTO ? '' : ' no-photo') + '">'
          +
          ...
          + '<a href="' +
          _this.INFO.USER.DETAIL_URL.replace(/#USER_ID#/g, _this.USER_ID) + '">'
          + BX.util.htmlspecialchars(
              jsBXAC.FormatName(jsBXAC.SETTINGS.NAME_TEMPLATE, _this.INFO.USER))
          + '</a>'
          + '</div>'
          ...
          + '</div>'
          + '<div class="bx-info-entry bx-calendar-color-' +
          _this.INFO.ENTRY.TYPE + '" style="background: ' +
          typeBgColors[_this.INFO.ENTRY.TYPE] + '">'
          +
          BX.util.htmlspecialchars(_this.INFO.ENTRY.PROPERTY_ABSENCE_TYPE_VALUE)
          + '</div>'
          + '<div class="bx-calendar-info-data-date">' + strDate + '</div>'
          + '<div class="bx-calendar-info-data-title">' +
          BX.util.htmlspecialchars(_this.INFO.ENTRY.NAME) + '</div>'
          + '<div class="bx-calendar-info-data-detail">'
          + BX.util.htmlspecialchars(_this.INFO.ENTRY.DETAIL_TEXT
              ? _this.INFO.ENTRY.DETAIL_TEXT
              : _this.INFO.ENTRY.PREVIEW_TEXT)
          + '</div>'
          + '</div>'
          + strAdmin
          + '</div>'
          +
          '<div class="bx-calendar-info-footer"><input type="button" value="' +
          jsBXAC.MESSAGES.INTR_ABSC_TPL_INFO_CLOSE + '" id="bx_bottom_close_' +
          _this.TYPE + '_' + _this.ID + '" /></div>';

      if (_this.TYPE == 1) {
        if (null != window.jsBXCalendarAdmin) {
          document.getElementById('bx_calendar_entry_delete_' +
              _this.ID).onclick = function() {return _this.deleteEntry();};
        }
      }
    }

    var left = parseInt(
        windowScroll.scrollLeft + windowSize.innerWidth / 2 - _this.width / 2);
    var top = parseInt(
        windowScroll.scrollTop + windowSize.innerHeight / 2 - _this.height / 2);

    _this.DIV.style.display = 'block';

    jsFloatDiv.Show(_this.DIV, left, top, 5, true, false);

    document.getElementById('bx_top_close_' + _this.TYPE + '_' +
        _this.ID).onclick = document.getElementById(
        'bx_bottom_close_' + _this.TYPE + '_' +
        _this.ID).onclick = function() {_this.Close();};
  };
}
...
JCCalendarInfoWin.prototype.Show = function(e) {
  ...
  if (null == this.DIV) {
    var url = this.LOADER + '?MODE=INFO&ID=' + this.ID + '&TYPE=' + this.TYPE
        + '&SITE_ID=' + jsBXAC.SETTINGS.SITE_ID + '&IBLOCK=' + (this.TYPE == 2
            ? jsBXAC.SETTINGS.CALENDAR_IBLOCK_ID
            : jsBXAC.SETTINGS.IBLOCK_ID) + '&sessid=' +
...
JCCalendarInfoWin.prototype.deleteEntry = function() {
  if (confirm(jsBXCalendarAdmin.DELETE_CONFIRM + ' "' +
      BX.util.htmlspecialcharsback(this.INFO.ENTRY.NAME) + '"?')) {
    BX.showWait();

    BX.ajax.get(
        '/bitrix/tools/intranet_absence.php?action=delete' + '&sessid=' +
        BX.message('bitrix_sessid') + '&absenceID=' + this.INFO.ENTRY.ID +
        '&js=1',
        BX.delegate(
            function() {
              BX.closeWait();
    ...
    BX.AbsenceCalendar.popup = new BX.PopupWindow('BXAbsence', null, {
                ...
                if (result == 'close') {
                  ...
                  var obErrors = BX.create('DIV', {
                    html: '<div class="webform-round-corners webform-error-block" style="margin-top:5px" id="error">\
                        ...
                        result.substring(6, result.length) + '</ul>\
												</div>\
												<div class="webform-corners-bottom"><div class="webform-left-corner"></div><div class="webform-right-corner"></div></div>\
											</div>',
                  });

                  if (BX.findChild(BX.AbsenceCalendar.popup.contentContainer,
                      {className: 'webform-error-block'}, true)) {
                    BX.AbsenceCalendar.popup.contentContainer.replaceChild(
                        obErrors,
                        BX.AbsenceCalendar.popup.contentContainer.firstChild);
                  } else {
                    BX.AbsenceCalendar.popup.contentContainer.insertBefore(
                        obErrors,
                        BX.AbsenceCalendar.popup.contentContainer.firstChild);
                  }
      ...
      events: {
        onAfterPopupShow: function() {
          this.setContent('<div style="width:450px;height:230px">' +
              BX.message('INTR_LOADING') + '</div>');
          BX.ajax.post(
...
BX.AbsenceCalendar.ShowForm = function(arParams) {
  ...
  BX.AbsenceCalendar.popup.params.zIndex = (BX.WindowManager
