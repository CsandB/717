...
$property_enums = CIBlockPropertyEnum::GetList(array(
    "DEF"  => "DESC",
    "SORT" => "ASC",
), array("IBLOCK_ID" => $arParams['IBLOCK_ID'], "CODE" => "ABSENCE_TYPE"));
while ($enum_fields = $property_enums->GetNext()) {
    $TYPES[] = array(
        'NAME'  => $enum_fields['XML_ID'],
        'TITLE' => Bitrix\Intranet\UserAbsence::getTypeCaption($enum_fields['XML_ID'],
            $enum_fields['VALUE']),
    );
...
<script type="text/javascript">
  function GetAbsenceDialog(abcenseID) {
      <?
      $arAbsenceParams["MESS"]["INTR_ABSENCE_TITLE"]
          = GetMessage("INTR_ABSENCE_TITLE_EDIT");
      $arAbsenceParams["ABSENCE_ELEMENT_ID"] = "#ABSENCE_ID#";
      ?>
    var dialog = "<?="BX.AbsenceCalendar.ShowForm("
    .CUtil::PhpToJSObject($arAbsenceParams).")"?>";
  ...
  jsBXAC.Init(
      {
        ...
        'DAY_SHOW_NONWORK': <?echo $arParams['DAY_SHOW_NONWORK'] == 'Y' ? 'true'
              : 'false'?>,
        ...
        'MONTHS': [<?for ($i = 1; $i < 13; $i++) {
            echo($i == 1 ? ''
                : ','), "'", CUtil::JSEscape(GetMessage('IAC_MONTH_'.$i)), "'";
        }?>],
        'MONTHS_R': [<?for ($i = 1; $i < 13; $i++) {
            echo($i == 1 ? ''
                : ','), "'", CUtil::JSEscape(GetMessage('IAC_MONTH_R_'
                .$i)), "'";
        }?>],
        'DAYS': [<?for ($i = 1; $i < 8; $i++) {
            echo($i == 1 ? '' : ','), "'", CUtil::JSEscape(GetMessage('IAC_DAY_'
                .$i)), "'";
        }?>],
        'DAYS_FULL': [<?for ($i = 1; $i < 8; $i++) {
            echo($i == 1 ? ''
                : ','), "'", CUtil::JSEscape(GetMessage('IAC_DAY_FULL_'
                .$i)), "'";
        }?>],
  ...
  jsBXAC.RegisterView({
    ID: 'day',
    NAME: '<?echo CUtil::JSEscape(GetMessage('IAC_VIEW_DAY'))?>',
    SORT: 100,
  });
  jsBXAC.RegisterView({
    ID: 'week',
    NAME: '<?echo CUtil::JSEscape(GetMessage('IAC_VIEW_WEEK'))?>',
    SORT: 200,
  });
  jsBXAC.RegisterView({
    ID: 'month',
    NAME: '<?echo CUtil::JSEscape(GetMessage('IAC_VIEW_MONTH'))?>',
    SORT: 300,
  });

  BX.ready(function() {
    jsBXAC.Show(document.getElementById('bx_calendar_layout'),
        '<?echo $arParams['VIEW_START']?>');
  });
...
<div id="bx_calendar_conrol_departments" style="display: none;"><?
    CIntranetUtils::ShowDepartmentFilter($arResult['UF_DEPARTMENT_field'],
        true);
    ?>
</div>
<div id="bx_calendar_control_datepicker" style="display: none"><input
            type="hidden" id="bx_goto_date" name="bx_goto_date"
            value="<? echo ConvertTimeStamp(); ?>"/><img
            src="/bitrix/js/main/core/images/calendar-icon.gif"
            class="calendar-icon"
            onclick="BX.calendar({node:this, field:'bx_goto_date', bTime: false, callback: jsBXAC.InsertDate});"
