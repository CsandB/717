...
if (
    ...
    && in_array("GROUP_ID", $arParams['EDITABLE_FIELDS'])
    || ($arResult["IS_BITRIX24"] && Integrator::isIntegrator($USER->GetID()))
) {
...
if (isset($arResult['User']['GROUP_ID'])) {
    $arResult["User"]["IS_ADMIN"] = in_array(1, $arResult['User']['GROUP_ID'])
        ? true : false;
}

$arResult["User"]["IS_INVITED"] = ! empty($arResult['User']["CONFIRM_CODE"]);

$arResult["IsMyProfile"] = ($arResult["User"]["ID"] == $USER->GetID()) ? true
    : false;
$arResult["User"]["IS_EXTRANET"] = (
    empty($arResult["User"]['UF_DEPARTMENT'][0])
    && Loader::includeModule('extranet')
    && Util::checkExternalAuthId($arResult["User"]['EXTERNAL_AUTH_ID'])
);
...
if ( ! empty($arRes)) {
    foreach ($arRes as $key => $val) {
        $arResult['USER_PROP'][$val["FIELD_NAME"]] = ($val["EDIT_FORM_LABEL"]
        <> '' ? htmlspecialcharsbx($val["EDIT_FORM_LABEL"])
            : $val["FIELD_NAME"]);
...
$arResult["User"]["PASSWORD_REQUIREMENTS"] = is_array($arPolicy)
&& isset($arPolicy["PASSWORD_REQUIREMENTS"])
...
if (Loader::includeModule("bitrix24")) {
    ...
    if ( ! $arResult['User']['IS_ADMIN']) {
        $arResult["ADMIN_LIMITS_ENABLED"] = COption::GetOptionString("bitrix24",
            "admin_limits_enabled", "N") == "Y" ? true : false;
