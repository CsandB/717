...
if ($arResult["User"]['EXTERNAL_AUTH_ID'] == 'email') {
    ...
    if (CModule::IncludeModule("socialservices")) {
        if ($bNetwork) {
            $arFields = array_merge(array('SOCSERV' => array('SOCSERVICES')),
                $arFields);
        } else {
...
if ($arResult["User"]["IS_EXTRANET"]
    && in_array("UF_DEPARTMENT", $arParams['EDITABLE_FIELDS'])
) {
...
if ($arResult['ERROR_MESSAGE']) {
    ?>
    <div class="content-edit-form-notice-error"><span
                class="content-edit-form-notice-text"><span
                    class="content-edit-form-notice-icon"></span><?= $arResult['ERROR_MESSAGE'] ?></span>
    </div>
    <?
}
?>

<form id="bx_user_profile_form" name="user_profile_edit" method="POST"
      action="<? echo POST_FORM_ACTION_URI; ?>" enctype="multipart/form-data"
      autocomplete="off">
    <? if (array_search("PERSONAL_GENDER", $arFields["PERSONAL"]) === false): ?>
        <input type="hidden" name="PERSONAL_GENDER"
               value="<?= $arResult["User"]["PERSONAL_GENDER"] ?>" /><? endif ?>
    <? echo bitrix_sessid_post() ?>
    <table id="content-edit-form-1" class="content-edit-form" cellspacing="0"
           cellpadding="0">
        ...
        foreach ($arFields as $GROUP_ID => $arGroupFields):
            ...
            if ( ! empty($arGroupFields)) {
                ?>
                <tr>
                    <td class="content-edit-form-header content-edit-form-header-first"
                        colspan="3">
            ...
            if ($GROUP_ID == "MAIN"):?>
                <? if (in_array('PASSWORD', $arParams['EDITABLE_FIELDS'])):?>
                    <tr>
                        ...
                        <td class="content-edit-form-event-link" colspan="2"><a
                                    href="#showPassword"
                    ...
                    <tr data-field-id="password" style="display:none">
                        ...
                        <td class="content-edit-form-field-input"><input
                ...
                <? if (in_array('EMAIL', $arParams['EDITABLE_FIELDS'])):?>
                    <tr>
                        <td class="content-edit-form-field-name"><?= $bNetwork
                                ? GetMessage("ISL_EMAIL_CONTACT")
                                : GetMessage("ISL_EMAIL"); ?></td>
                        <td class="content-edit-form-field-input">
            ...
            if ($FIELD == "FORUM_SHOW_NAME") {
                ?>
                <tr>
                <td><input type="hidden" name="<? echo $FIELD ?>" value="Y"/>
                </td></tr><?
                continue;
            }
            $value = $arResult['User'][$FIELD]; ?>
            <tr <? if ($arResult["IS_BITRIX24"]
            && (in_array($FIELD, array(
                ...
                || in_array($FIELD,
                    array("PERSONAL_MOBILE", "WORK_PHONE", "UF_PHONE_INNER")))
            && empty($value)
            || $FIELD == "GROUP_ID"
            && $arResult["User"]["IS_EXTRANET"]): ?>style="display:none"<? endif;
            ?> data-role="<?= $FIELD ?>"<?= ($FIELD == "TIME_ZONE"
                ? " data-field-id=\"time_zone\" style=\"display: "
                .($arResult["User"]["AUTO_TIME_ZONE"] <> "N" ? "none"
                    : "table-row").";\"" : "") ?>>
                <? if ( ! $bNetwork || $GROUP_ID !== "SOCSERV"):?>
                    <td class="content-edit-form-field-name"><?= $arResult['USER_PROP'][$FIELD]
                            ? $arResult['USER_PROP'][$FIELD]
                            : GetMessage('ISL_'.$FIELD); ?></td>
                <? endif; ?>
                <?
                switch ($FIELD) {
                    case "PASSWORD":
                    ...
                    case 'TIME_ZONE':
                        ?>
                        <td class="content-edit-form-field-input" colspan="2">
                        <select name="TIME_ZONE"<? if ($arResult["User"]["AUTO_TIME_ZONE"]
                            <> "N"
                        ) echo '  disabled="disabled"' ?>
                                class="content-edit-form-field-input-select">
                            <? if (is_array($arResult["TIME_ZONE_LIST"])
                                && ! empty($arResult["TIME_ZONE_LIST"])
                            ):?>
                                <? foreach (
                                    $arResult["TIME_ZONE_LIST"] as $tz =>
                                    $tz_name
                                ):?>
                                    <option value="<?= htmlspecialcharsbx($tz) ?>"<?= ($value
                    ...
                    case 'WORK_COUNTRY':
                        ?>
                        <td class="content-edit-form-field-input" colspan="2">
                        <? echo SelectBoxFromArray($FIELD, GetCountryArray(),
                        $value, GetMessage("ISL_COUNTRY_EMPTY"),
                        "class=\"content-edit-form-field-input-select\""); ?>
                        </td><?
                        break;
                    case 'SOCSERVICES':
                    ...
                    case "PERSONAL_GENDER":
                        ?>
                        <td class="content-edit-form-field-input" colspan="2">
                        <div><label><input type="radio" name="<? echo $FIELD ?>"
                                           value="M"<? echo $value == 'M'
                                    ? ' checked="checked"' : '' ?>
                                           class="content-edit-form-field-input-selector"/><span
                                        class="content-edit-form-field-input-selector-name"><? echo GetMessage('ISL_PERSONAL_GENDER_MALE') ?></span></label>
                        </div>
                        <div><label><input type="radio" name="<? echo $FIELD ?>"
                                           value="F"<? echo $value == 'F'
                                    ? ' checked="checked"' : '' ?>
                                           class="content-edit-form-field-input-selector"/><span
                                        class="content-edit-form-field-input-selector-name"><? echo GetMessage('ISL_PERSONAL_GENDER_FEMALE') ?></span></label>
                        </div>
                        </td><?
                        break;
                        ...
                        <td class="content-edit-form-field-input" colspan="2">
                            ...
                            $APPLICATION->IncludeComponent('bitrix:main.file.input',
                                '', array(
                                    ...
                                    'INPUT_NAME_UNSAVED' => $FIELD
                                        .'_ID_UNSAVED',
                                    'CONTROL_ID'         => $FIELD.'_IMAGE_ID',
                                    'INPUT_VALUE'        => $arResult["User"][$FIELD
                                    ."_FILE"]["ID"],
                                    'MULTIPLE'           => 'N',
                                    'ALLOW_UPLOAD'       => 'I',
                    ...
                    case 'PERSONAL_BIRTHDAY':
                        ...
                        <?
                        $daySelect
                            = '<select class="content-edit-form-field-input-select" id="PERSONAL_BIRTHDAY_DAY" name="PERSONAL_BIRTHDAY_DAY" onchange="onPersonalBirthdayChange(event)" onblur="onPersonalBirthdayChange(event)">';
                        $daySelect .= '<option value="">'
                            .GetMessage("ISL_BIRTHDAY_DAY").'</option>';
                        for ($i = 1; $i <= 31; $i++) {
                            $daySelect .= '<option'.($day == $i
                        ...
                        ?>
                        <select class="content-edit-form-field-input-select"
                                id="PERSONAL_BIRTHDAY_MONTH"
                                name="PERSONAL_BIRTHDAY_MONTH"
                                onchange="onPersonalBirthdayChange(event)"
                        ...
                        <input type="hidden" id="PERSONAL_BIRTHDAY_VALUE"
                               name="PERSONAL_BIRTHDAY" value="<?= $value ?>"/>

                        <script type="text/javascript">
                          function onPersonalBirthdayChange(event) {
                            var daySelect = BX('PERSONAL_BIRTHDAY_DAY', true);
                            var monthSelect = BX('PERSONAL_BIRTHDAY_MONTH',
                                true);
                            var yearTextbox = BX('PERSONAL_BIRTHDAY_YEAR',
                                true);
                            var birthdayHidden = BX('PERSONAL_BIRTHDAY_VALUE',
                                true);
                            ...
                            if (BX.type.isNumber(day) &&
                                BX.type.isNumber(month) &&
                                BX.type.isNumber(year) &&
                                (date = new Date(Date.UTC(year, month, day)))) {
                              birthdayHidden.value = BX.message('FORMAT_DATE').
                                  replace(/YYYY/ig, date.getUTCFullYear()).
                                  replace(/MM/ig, BX.util.str_pad_left(
                                      (date.getUTCMonth() + 1).toString(), 2,
                                      '0')).
                                  replace(/DD/ig, BX.util.str_pad_left(
                                      date.getUTCDate().toString(), 2, '0')).
                                  replace(/HH/ig, BX.util.str_pad_left(
                                      date.getUTCHours().toString(), 2, '0')).
                                  replace(/MI/ig, BX.util.str_pad_left(
                                      date.getUTCMinutes().toString(), 2, '0')).
                                  replace(/SS/ig, BX.util.str_pad_left(
                                      date.getUTCSeconds().toString(), 2, '0'));
                              ...
                              setTimeout(function() {
                                if (eventType == 'blur' &&
                                    BX.type.isNumber(day) &&
                                    BX.type.isNumber(month) &&
                                    !BX.type.isNumber(year) &&
                                    yearTextbox.getAttribute('data-focus') !=
                                    'true')
                        ...
                        <div class="content-edit-form-field-input-sub">
                            <label style="display: inline-block;">
                                <input
                                    ...
                                    <? if (
                                        ...
                                        && (
                                            ...
                                            || in_array($arResult["User"]["EXTERNAL_AUTH_ID"],
                                                array(
                                                    "email",
                                                    "replica",
                                                    "imconnector",
                                ...
                                <? if (
                                    ...
                                    <span id="group_admin_hint"
                                          data-text="<?= GetMessage("SOCNET_MOVE_RIGHTS_AUTH_HINT") ?>"
                                          class="user-profile-edit-help">?</span>
                                        ...
                                        <? else:?>
                                            ...
                                            <div>
                                                <a href='javascript:void'
                                                   onclick='BX.Helper.show("redirect=detail&code=5869717");'><?= GetMessage("SOCNET_RIGHTS_RESTR_MORE") ?></a>
                                            </div>
                                        <?endif ?>
                                    </div>
                                </div>
                            ...
                            <script>
                              BX.bind(BX('group_admin'), 'click',
                                  function(event) {
                                    if (BX('group_admin').checked) {
                                      event.preventDefault();
                                      BX.PopupWindowManager.create(
                                          'menu-custom-preset-delete-popup',
                                          null, {
                                            closeIcon: true,
                                            contentColor: 'white',
                                            ...
                                            content: '<div style=\'padding: 10px;\'>' +
                                                BX.message(
                                                    'SOCNET_MOVE_RIGHTS_CONFIRM') +
                                                '</div>',
                                            titleBar: BX.message(
                                                'SOCNET_MOVE_RIGHTS_CONFIRM_TITLE'),
                                            buttons: [
                    ...
                    case "UF_INTERESTS":
                        ...
                        <td class="content-edit-form-field-textarea"
                            colspan="2"><textarea name="<? echo $FIELD ?>"
                                                  class="content-edit-form-field-input-textarea"><?= $value ?></textarea>
                        </td><?
                        break;
                    case "UF_DEPARTMENT":
                        ?>
                        <td class="content-edit-form-field-input" colspan="2">
                            <select name="UF_DEPARTMENT[]" size="5"
                                    multiple="multiple">
                                <?
                                $rsDepartments
                                    = CIBlockSection::GetTreeList(array(
                                    "IBLOCK_ID" => intval(COption::GetOptionInt('intranet',
                                        'iblock_structure', false)),
                                ));
                                while ($arDepartment
                                    = $rsDepartments->GetNext()):
                                    ?>
                                    <option
                                    value="<? echo $arDepartment["ID"] ?>" <? if (is_array($value)
                                    && in_array($arDepartment["ID"], $value)
                                ) echo "selected" ?>><? echo str_repeat("&nbsp;.&nbsp;",
                                    $arDepartment["DEPTH_LEVEL"]) ?><? echo $arDepartment["NAME"] ?></option><?
                                endwhile;
                                ?>
                            </select>
                        </td>
                    ...
                    case "UF_PHONE_INNER":
                        ?>
                        <td class="content-edit-form-field-input" colspan="2">
                        <input type="text" name="<? echo $FIELD ?>"
                               value="<?= $value ?>"
                               class="content-edit-form-field-input-text"/>
                        </td><?
                        break;
                    ...
                    case "UF_XING":
                        ...
                        <td class="content-edit-form-field-input" colspan="2">
                            <input type="text" name="<? echo $FIELD ?>"
                                   placeholder="<?= $SocNetProfileExample[$FIELD] ?>"
                                   value="<?= $arResult["User"][$FIELD] ?>"
                                   class="content-edit-form-field-input-text"/>
                        </td>
                        <?
            ...
            <? if ($arResult["IS_BITRIX24"] && $FIELD == "PERSONAL_PHOTO"
            && in_array('UF_PHONE_INNER', $arFields['MAIN'])
            && ! ($arResult['User']["UF_PHONE_INNER"]
            ...
            <tr>
                <td class="content-edit-form-field-name"></td>
                <td class="content-edit-form-event-link" colspan="2"><a
                            href="#addPhone" onclick="ShowAddPnone(this)"
                            class="content-edit-form-event-link-tag"><span
        ...
        <? endif; ?>
            <? if ($arResult["IS_BITRIX24"] && $FIELD == "UF_SKYPE"
            && count($arSocialFields) > 0
            && $sonetLinkAddShow
        ):?>
            <tr>
                <td class="content-edit-form-field-name"></td>
                <td class="content-edit-form-event-link" colspan="2"><a
                            href="#addSocnet" onclick="ShowAddSocnet(this)"
                            class="content-edit-form-event-link-tag"><span
                                class="content-edit-form-event-link-icon content-edit-form-icons content-edit-form-icon-socnet"></span><span
        ...
        <tr>
            ...
            <td class="content-edit-form-buttons" colspan="2">
                ...
                <span class="webform-button"
                      onclick="location.href = '<? echo htmlspecialcharsbx(CUtil::addslashes(
                          $_REQUEST['backurl']
                              ? $_REQUEST['backurl']
                              : CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER"],
                              array("user_id" => $arParams["ID"]))
...
<script>
  ...
  function confirmDelete(event) {
    if (confirm("<?=$arResult["User"]["ACTIVE"] == "Y"
        ? GetMessage("SOCNET_CONFIRM_FIRE1")
        : GetMessage("SOCNET_CONFIRM_RECOVER1")?>"))
      return true;
    else
      event.preventDefault();
  }
  ...
  function bxUpeToggleHiddenField(tableId, fieldId, show) {
    var elements = BX.findChildren(BX(tableId),
        {attribute: {'data-field-id': '' + fieldId + ''}}, true);
    if (elements != null) {
      BX.fx.colorAnimate.addRule('animationRule', '#FFF', '#faeeb4',
          'background-color', 50, 1, true);
  ...
  function ShowAddSocnet(bindElement) {
    ...
    BX.PopupMenu.show('socnet_add', bindElement, [
          ...
          {
            text: "<?=GetMessage("ISL_UF_TWITTER")?>",
          ...
          {
            text: "<?=GetMessage("ISL_UF_FACEBOOK")?>",
            className: 'menu-popup-no-icon',
            onclick: function() {
          ...
          {
            text: "<?=GetMessage("ISL_UF_LINKEDIN")?>",
            className: 'menu-popup-no-icon',
            onclick: function() {
  ...
  function ShowAddPnone(bindElement) {
    ...
    BX.PopupMenu.show('phone_add', bindElement, [
          {
            text: "<?=GetMessage("ISL_PERSONAL_MOBILE")?>",
            className: 'menu-popup-no-icon',
            onclick: function() {
              form.querySelector(
                  '[data-role=\'PERSONAL_MOBILE\']').style.display = '';
              this.popupWindow.close();
            },
          },
          {
            text: "<?=GetMessage("ISL_WORK_PHONE")?>",
            className: 'menu-popup-no-icon',
            onclick: function() {
          ...
          {
            text: "<?=GetMessage("ISL_UF_PHONE_INNER")?>",
            className: 'menu-popup-no-icon',
            onclick: function() {
