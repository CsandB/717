(function() {
  ...
  BX.Bitrix24.SonetGroupFilter.prototype.init = function(params) {
    ...
    BX.addCustomEvent('BX.Main.Filter:beforeApply',
        BX.delegate(function(eventFilterId, values, ob, filterPromise) {
          if (
          ...
          BX.onCustomEvent(window, 'BX.SonetGroupList.Filter:beforeApply',
              [values, filterPromise]);
        }, this));

    BX.addCustomEvent('BX.Main.Filter:apply', BX.delegate(
        function(eventFilterId, values, ob, filterPromise, filterParams) {
          if (
              eventFilterId != filterId
              || (
                  this.actualSearchString.length > 0
          ...
          BX.onCustomEvent(window, 'BX.SonetGroupList.Filter:apply',
              [values, filterPromise, filterParams]);
        }, this));

    BX.addCustomEvent('BX.Filter.Search:input',
        BX.delegate(function(eventFilterId, searchString) {
          if (eventFilterId == filterId) {

            this.actualSearchString = (typeof searchString != 'undefined'
                ? BX.util.trim(searchString)
                : '');
            ...
            if (
              ...
              BX.onCustomEvent(window, 'BX.SonetGroupList.Filter:searchInput',
                  [searchString]);
            }
          }
        }, this));

    BX.addCustomEvent('BX.Main.Filter:blur',
        BX.delegate(function(filterObject) {
          if (
            ...
            var pagetitleContainer = BX.findParent(
                BX(filterId + '_filter_container'),
                {className: 'pagetitle-wrap'});
            if (pagetitleContainer) {
              BX.removeClass(pagetitleContainer,
                  'pagetitle-wrap-filter-opened');
            }
          }
        }, this));
  };
