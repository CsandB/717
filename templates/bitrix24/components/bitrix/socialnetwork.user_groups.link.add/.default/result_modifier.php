...
$arResult["Filter"] = array(
    ...
    array(
        'id'      => 'OWNER',
        'name'    => Loc::getMessage(ModuleManager::isModuleInstalled('intranet')
            ? 'SONET_C36_T_FILTER_OWNER_INTRANET' : 'SONET_C36_T_FILTER_OWNER'),
        'default' => true,
        'type'    => 'dest_selector',
        'params'  => array(
            'apiVersion'              => '3',
...
if (COption::GetOptionString("socialnetwork", "work_with_closed_groups", "N")
    != "Y"
) {
    $arResult["Filter"][] = array(
        'id'   => 'CLOSED',
...
$arResult["GROUP_PROPERTIES"]
    = $USER_FIELD_MANAGER->GetUserFields("SONET_GROUP", 0, LANGUAGE_ID);
...
foreach ($arResult["GROUP_PROPERTIES"] as $field => $arUserField) {
    ...
    $arUserField["EDIT_FORM_LABEL"] = ! empty($arUserField["EDIT_FORM_LABEL"])
        ? $arUserField["EDIT_FORM_LABEL"] : $arUserField["FIELD_NAME"];
    $arUserField["EDIT_FORM_LABEL"]
        = htmlspecialcharsEx($arUserField["EDIT_FORM_LABEL"]);
...
$arResult["ftMinTokenSize"] = (isset($config["ft_min_token_size"])
    ? $config["ft_min_token_size"] : CSQLWhere::FT_MIN_TOKEN_SIZE);
?>
