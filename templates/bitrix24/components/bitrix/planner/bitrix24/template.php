<? if ( ! defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) {
    ...
    <div class="timeman-container planner-container<?= (IsAmPmMode()
        ? " am-pm-mode" : "") ?>"><?
        ?>
        <div class="timeman-wrap planner-wrap"><?
            ?><span id="timeman-block" class="timeman-block"><?
                ...
                ?><span class="timeman-right-side"><?
                    ...
                    ?><span class="timeman-task-time" id="timeman-task-time"
                            style="display: none;"><i></i><span
                                id="timeman-task-timer"></span></span><?
                    ?></span><?
      ...
      (function() {
        ...
        BX.addCustomEvent(
            BXPLANNER, 'onPlannerDataRecieved', function(ob, DATA) {
              ...
              BX.adjust(BX('timeman-info', true),
                  {children: [NODE_EVENTS, NODE_TASKS]});

              if (!timer) {
                timer = BX.timer(
                    {container: BX('timeman-timer'), display: 'bitrix24_time'});
              }
            },
        );
        ...
        BX.addCustomEvent('onTaskTimerChange', function(params) {
          if (params.action === 'refresh_daemon_event') {
            ...
            var s = '';
            s += formatTime(parseInt(params.data.TIMER.RUN_TIME || 0) +
                parseInt(params.data.TASK.TIME_SPENT_IN_LOGS || 0), true);

            if (!!params.data.TASK.TIME_ESTIMATE &&
                (params.data.TASK.TIME_ESTIMATE > 0)) {
              s += ' / ' +
                  formatTime(parseInt(params.data.TASK.TIME_ESTIMATE || 0));
            }
              ...
              BXPLANNER.WND = new BX.PopupWindow('planner_main', this, {
                ...
                events: {
                  onPopupClose: function() {
                    BX.removeClass(BX('timeman-block', true),
                        'timeman-block-active');
                  },
                },
              });
            }
