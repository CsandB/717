...
<?
$requestUri = Application::getInstance()->getContext()->getRequest()
    ->getRequestUri();
    ...
    $items = array_merge($items, array(
        ...
        (
            ...
            "ON_CLICK"  => "BX.SidePanel.Instance.open('".$uri->getUri()
                ."', { width: 1000, loader: '".$this->getFolder()
                ."/images/slider/taskslist.min.svg', })",
            'SUB_LINK'  => array(
                'CLASS' => '',
                'URL'   => SITE_DIR."company/personal/user/"
                    .$arResult["User"]["ID"]."/tasks/task/edit/0/",
            ),
            'IS_ACTIVE' => (mb_strpos($requestUri, $arResult["Urls"]['tasks'])
                === 0),
        ),
    ));
}
...
if (
    ...
    $items = array_merge($items, array(
        ...
        (
            ...
            "ON_CLICK"  => "BX.SidePanel.Instance.open('".$uri->getUri()
                ."', { width: 1000, loader: '".$this->getFolder()
                ."/images/slider/calendar.min.svg' })",
            'IS_ACTIVE' => (mb_strpos($requestUri,
                    $arResult["Urls"]['calendar']) === 0),
        ),
    ));
}
...
if (
    ...
    $items = array_merge($items, array(
        ...
        (
            ...
            "ON_CLICK"  => "BX.SidePanel.Instance.open('".$uri->getUri()
                ."', { width: 1000, loader: '".$this->getFolder()
                ."/images/slider/disk.min.svg' })",
            'IS_ACTIVE' => (mb_strpos($requestUri, $arResult["Urls"]['files'])
                === 0),
        ),
    ));
}
...
if (
    ...
    $items = array_merge($items, array(
        ...
        (
            ...
            'IS_ACTIVE' => (mb_strpos($requestUri, $arResult["Urls"]['blog'])
                === 0),
        ),
    ));
}
...
if (
    ...
    $arResult['isExtranetSite']
        ? SITE_DIR."contacts/personal/user/".$arResult["User"]["ID"]
        ."/tasks/effective/"
        : SITE_DIR."company/personal/user/".$arResult["User"]["ID"]
        ."/tasks/effective/"
    );
    $items['effective_counter'] = array(
        "TEXT"             => GetMessage("SONET_UM_EFFICIENCY"),
        "ON_CLICK"         => "BX.SidePanel.Instance.open('".$efficiencyUrl
            ."', { width: 1000 })",
        "COUNTER"          => Counter::getInstance($arResult["User"]["ID"])
            ->get(Name::EFFECTIVE),
...
function checkEffectiveRights($viewedUser)
{
    ...
    return
        $currentUser == $viewedUser || User::isSuper($currentUser)
...
if (
    ...
    $items = array_merge($items, array(
        ...
        (
            ...
            "ON_CLICK" => "BX.SidePanel.Instance.open('".SITE_DIR
                ."timeman/timeman.php?USERS=U".$arResult["User"]["ID"]
                ."&apply_filter=Y', { width: 1000, loader: '".$this->getFolder()
                ."/images/slider/worktime.min.svg', })",
        ),
        array
        (
            "ID"       => "work_report",
            ...
            "ON_CLICK" => "BX.SidePanel.Instance.open('".SITE_DIR
                ."timeman/work_report.php', { width: 1000, loader: '"
                .$this->getFolder()."/images/slider/workreport.min.svg' })",
        ),
    ));
}
...
if (
    ...
    $items = array_merge($items, array(
        ...
        (
            ...
            "ON_CLICK" => "BX.SidePanel.Instance.open('"
                .$arResult["Urls"]['groups']."', { width: 1000, loader: '"
                .$this->getFolder()."/images/slider/grouplist.min.svg' })",
        ),
    ));
}
...
$APPLICATION->IncludeComponent(
    ...
    "",
    array(
        "ID"               => "socialnetwork_profile_menu_user_"
            .$arResult["User"]["ID"],
        "ITEMS"            => $items,
        "DISABLE_SETTINGS" => ! (
            $USER->isAuthorized()
