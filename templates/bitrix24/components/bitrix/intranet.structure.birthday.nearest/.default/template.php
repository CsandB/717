            ...
            <div class="sidebar-block-inner">
                ...
                <div class="filter-block">
                    ...
                    <div class="filter-field-buttons">
                        <input type="submit"
                               value="<?= GetMessage("INTR_ISBN_TPL_FILTER_SUBMIT") ?>"
                               class="filter-submit"/>&nbsp;&nbsp;<input
                                type="submit"
                                value="<?= GetMessage("INTR_ISBN_TPL_FILTER_CANCEL") ?>"
                                onclick="document.forms.bx_birthday_filter.department.selectedIndex=0;"
                                class="filter-submit"/>
                    </div>
                </div>
            </div>
...
<div class="bx-birthday-layout">
    ...
    foreach ($arResult['USERS'] as $arUser) {
        $birthday = FormatDateEx(
            ...
            false,
            $arParams['DATE_FORMAT'.($arParams['SHOW_YEAR'] == 'Y'
            || $arParams['SHOW_YEAR'] == 'M'
            && $arUser['PERSONAL_GENDER'] == 'M' ? '' : '_NO_YEAR')]
        );

        if ($arUser['IS_BIRTHDAY']) {
            $birthday .= ' - '.GetMessage('INTR_ISBN_TPL_TODAY');
        }
