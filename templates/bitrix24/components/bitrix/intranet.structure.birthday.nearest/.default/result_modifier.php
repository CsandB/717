...
if ($arParams['bCache']) {
    $cache_dir = '/'.SITE_ID.$this->__component->GetRelativePath().'/'
        .$this->GetName();
    $cache_id = $this->GetFile().'|'
        .$arParams['NUM_USERS'];//.'|'.$USER->GetGroups();
...
if ($arParams['bCache']
    && $obCache->InitCache($arParams['CACHE_TIME'], $cache_id, $cache_dir)
) {
...
if ($arParams['SHOW_FILTER'] == 'Y') {
    $arUserFields = $GLOBALS['USER_FIELD_MANAGER']->GetUserFields('USER', 0,
        LANGUAGE_ID);
...
foreach ($arResult['USERS'] as $key => $arUser) {
    ...
    $arUser['IS_ONLINE'] = $arResult['bUsersCached'] ? false
        : CIntranetUtils::IsOnline($arUser['LAST_ACTIVITY_DATE']);

    $arUser['IS_BIRTHDAY']
        = CIntranetUtils::IsToday($arUser['PERSONAL_BIRTHDAY']);

    if (array_key_exists('IS_ABSENT', $arCacheData[$arUser['ID']])) {
        $arUser['IS_ABSENT'] = $arCacheData[$arUser['ID']]['IS_ABSENT'];
    } else {
        $arUser['IS_ABSENT']
            = $arCacheData[$arUser['ID']]['IS_ABSENT']
    ...
    if (array_key_exists('IS_FEATURED', $arCacheData[$arUser['ID']])) {
        ...
        $arUser['IS_FEATURED']
            = $arCacheData[$arUser['ID']]['IS_FEATURED']
    ...
    if ($bLoadDepartments && is_array($arUser['UF_DEPARTMENT'])
        && count($arUser['UF_DEPARTMENT']) > 0
    ) {
        if (array_key_exists('UF_DEPARTMENT', $arCacheData[$arUser['ID']])) {
            $arUser['UF_DEPARTMENT']
                = $arCacheData[$arUser['ID']]['UF_DEPARTMENT'];
        } else {
            $arUser['UF_DEPARTMENT']
                = $arCacheData[$arUser['ID']]['UF_DEPARTMENT']
                = CIntranetUtils::GetDepartmentsData($arUser['UF_DEPARTMENT']);
        }
    }
...
if ($arResult['bUsersCached'] && $strUserIDs <> '') {
    $dbRes = CUser::GetList($by = 'id', $order = 'asc',
        array('ID' => $strUserIDs, 'LAST_ACTIVITY' => 120));
