  ...
  window.B24SGControl.prototype = {
    ...
    init: function(params) {
      ...
      if (BX('bx-group-join-submit')) {
        BX.bind(BX('bx-group-join-submit'), 'click',
            BX.delegate(this.sendJoinRequest, this));
      }

      BX.addCustomEvent('BX.Bitrix24.LeftMenuClass:onMenuItemAdded',
          BX.delegate(function() {
            this.favoritesValue = true;
          }, this));

      BX.addCustomEvent('BX.Bitrix24.LeftMenuClass:onMenuItemDeleted',
          BX.delegate(function() {
          ...
          BX.delegate(function(event) {
            if (event.getEventId() == 'sonetGroupEvent') {
              ...
              if (
                  ...
                  && BX.util.in_array(eventData.code, [
                    'afterModeratorAdd',
                    'afterModeratorRemove',
                    'afterOwnerSet',
                    'afterUserExclude'])
                  && typeof eventData.data != 'undefined'
                  && parseInt(eventData.data.groupId) > 0
    ...
    setSubscribe: function(event) {
      ...
      var action = (!BX.hasClass(BX('group_menu_subscribe_button'),
          'ui-btn-active') ? 'set' : 'unset');
    ...
    sendJoinRequest: function(event) {
      ...
      BX.ajax({
        ...
        data: {
          ...
          MESSAGE: (BX('bx-group-join-message') ? BX(
              'bx-group-join-message').value : ''),
          ajax_request: 'Y',
          save: 'Y',
          sessid: BX.bitrix_sessid(),
        },
        onsuccess: BX.delegate(function(responseData) {
          ...
          if (
            ...
            BX.addClass(BX('bx-group-join-form'),
                'sonet-group-user-request-form-invisible');
            ...
            BX.SocialnetworkUICommon.showError(responseData.ERROR_MESSAGE,
                BX('bx-group-join-error'));
          }
        }, this),
        onfailure: BX.delegate(function() {
          BX.SocialnetworkUICommon.showError(
              BX.message('SONET_C6_T_AJAX_ERROR'), BX('bx-group-join-error'));
          BX.SocialnetworkUICommon.hideButtonWait(BX('bx-group-join-submit'));
        }, this),
      });
    },
        ...
        onsuccess: function(data) {
          ...
          if (
            ...
            BX.onCustomEvent(window,
                'BX.Socialnetwork.WorkgroupFavorites:onSet', [
                  {
                    id: _this.groupId,
                    name: data.NAME,
                    url: data.URL,
                    extranet: (typeof data.EXTRANET != 'undefined'
                        ? data.EXTRANET
                        : 'N'),
                  }, _this.newValue]);
          }
    ...
    setNotifyButton: function(value, showHint) {
      ...
      if (button) {
        if (value) {
          ...
          BX.adjust(button,
              {attrs: {title: BX.message('SGMSubscribeButtonTitleOn')}});
          BX.addClass(button, 'ui-btn-active');
        } else {
          if (showHint) {
            this.showNotifyHint(button,
                BX.message('SGMSubscribeButtonHintOff'));
          }
          BX.adjust(button,
              {attrs: {title: BX.message('SGMSubscribeButtonTitleOff')}});
    ...
    showErrorPopup: function(errorText) {
      ...
      var errorPopup = new BX.PopupWindow('sgm-error' + Math.random(), window, {
        ...
        zIndex: 2,
        content: BX.create('DIV', {
          props: {'className': 'sonet-sgm-error-text-block'},
          html: errorText,
        }),
        closeByEsc: true,
        closeIcon: true,
