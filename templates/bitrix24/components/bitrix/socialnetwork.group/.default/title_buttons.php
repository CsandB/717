...
Loc::loadMessages(
    ...
    ."/bitrix/components/bitrix/socialnetwork_group/templates/.default/util_community.php"
);
...
<script>
  BX.ready(function() {
    B24SGControl.getInstance().init({
      ...
      groupOpened: <?=($arResult['Group']['OPENED'] == 'Y' ? 'true'
          : 'false')?>,
...
<div class="socialnetwork-group-title-buttons">
    <? if (in_array($arResult["CurrentUserPerms"]["UserRole"],
        ...
        if ($arResult["bChatActive"]) {
            ?>
            <button id="group_menu_chat_button"
                    class="ui-btn ui-btn-light-border ui-btn-icon-chat ui-btn-themes"
                    title="<?= GetMessage($arResult['Group']['PROJECT'] == 'Y'
                        ? "SONET_SGM_T_CHAT_TITLE_PROJECT"
                        : "SONET_SGM_T_CHAT_TITLE") ?>"
        ...
        <button id="group_menu_subscribe_button" class="ui-btn ui-btn-light-border ui-btn-icon-follow ui-btn-themes
                ...
                title="<?= GetMessage("SONET_SGM_T_NOTIFY_TITLE_"
                    .($arResult["bSubscribed"] ? "ON" : "OFF")) ?>"
