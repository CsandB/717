...
if (is_array($arResult["Moderators"]["List"])) {
    foreach ($arResult["Moderators"]["List"] as $key => $arModerator) {
        if (is_array($arModerator)) {
            ...
            $arResult["Moderators"]["List"][$key]["USER_PERSONAL_PHOTO_FILE"]["SRC"]
                = $arImage["src"];
        }
    }
}
...
if (is_array($arResult["Members"]["List"])) {
    foreach ($arResult["Members"]["List"] as $key => $arMember) {
        if (is_array($arMember)) {
            ...
            $arResult["Members"]["List"][$key]["USER_PERSONAL_PHOTO_FILE"]["SRC"]
                = $arImage["src"];
        }
    }
}
...
$arResult["Types"] = Workgroup::getTypes(array(
...
$arParams["PATH_TO_GROUP_TAG"] = $arParams["PATH_TO_GROUPS_LIST"]
    .(mb_strpos($arParams["PATH_TO_GROUPS_LIST"], '?') !== false ? '&' : '?')
    ."TAG=#tag#&apply_filter=Y";
...
if (
    ...
    $arResult["Group"]["KEYWORDS_LIST"] = explode(',',
        $arResult["Group"]["KEYWORDS"]);
