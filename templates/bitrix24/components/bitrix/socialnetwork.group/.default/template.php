...
if ($arResult["ErrorMessage"] <> '') {
    ?><span class='errortext'><?= $arResult["ErrorMessage"] ?></span><br/>
    <br/><?
}
...
if ($arResult["bShowRequestSentMessage"] == "U") {
    ...
    str_replace(
        "#user_id#",
        $USER->GetID(),
        COption::GetOptionString("socialnetwork", "user_request_page",
            (IsModuleInstalled("intranet"))
                ? "/company/personal/user/#user_id#/requests/"
                : "/club/user/#user_id#/requests/", SITE_ID)
    ),
    ...
    <div class="socialnetwork-group-description-wrapper">
        <table width="100%" cellspacing="0" id="bx_group_description"
               class="socialnetwork-group-description-table<? if ($arResult["bDescriptionOpen"]
                   != "Y"
               ):?> socialnetwork-group-description-hide-table<?endif ?>">
            <tr>
                <td valign="top">
                    <table width="100%" cellspacing="0"><?
                        if ($arResult["Group"]["CLOSED"] == "Y") {
                            ...
                            <tr>
                            <td colspan="2"
                                class="socialnetwork-group-description">
                        ...
                        if ($arResult["Group"]["DESCRIPTION"] <> '') {
                            ...
                            <td class="socialnetwork-group-description-left-col"><?= GetMessage("SONET_C6_DESCR") ?>
                                :
                            </td>
                        ...
                        if ($arResult["GroupProperties"]["SHOW"] == "Y") {
                            foreach (
                                $arResult["GroupProperties"]["DATA"] as
                                $fieldName => $arUserField
                            ) {
                                if (
                                    (
                                    ...
                                    ?>
                                    <tr class="ext-header-center-row">
                                    <td class="socialnetwork-group-description-left-col"><?= $arUserField["EDIT_FORM_LABEL"] ?>
                                        :
                                    </td>
                        ...
                        ?>
                        <tr>
                            <td class="socialnetwork-group-description-left-col">
                                <nobr><?= Loc::getMessage($arResult['Group']['PROJECT']
                                    == 'Y' ? "SONET_C6_TYPE_PROJECT"
                                        : "SONET_C6_TYPE") ?>:
                                </nobr>
                            </td>
                            <td class="socialnetwork-group-description"><?
                                ...
                                <br/><?
                                ?><?= ($arResult['Group']['Type']['DESCRIPTION']) ?>
                                <br/><?
                                ...
                                if ($arResult["bUserCanRequestGroup"]) {
                                    ...
                                    == 'Y' ? "SONET_C6_ACT_JOIN_PROJECT"
                                        : "SONET_C6_ACT_JOIN")
                                        : Loc::getMessage($arResult['Group']['PROJECT']
                                    == 'Y' ? "SONET_C6_ACT_JOIN2_PROJECT"
                                        : "SONET_C6_ACT_JOIN2")
                                    );

                                    ?>
                                    <div id="bx-group-join-form"
                                         class="sonet-group-user-request-form"><?
                                    ?>
                                    <div id="bx-group-join-error"
                                         class="sonet-ui-form-error-block-invisible ui-alert ui-alert-danger"></div><?

                                    if ($arResult['Group']['OPENED'] != 'Y') {
                                        ?>
                                        <div class="sonet-group-user-request-join-message-cont">
                                            <textarea
                                                    class="sonet-group-user-request-join-message-text"
                                                    id="bx-group-join-message"></textarea>
                                        </div><?
                                    }
                                    ...
                                    <button class="ui-btn ui-btn-success"
                                            id="bx-group-join-submit"
                                            bx-request-url="<?= $arResult["Urls"]["UserRequestGroup"] ?>"><?= $joinButtonTitle ?></button><?
                                    ?><a class="ui-btn ui-btn-light-border"
                                         href="<?= $arResult["Urls"]["GroupsList"] ?>"><?= Loc::getMessage("SONET_C6_ACT_RETURN_TO_LIST") ?></a><?
                                    ?></span>
...
$APPLICATION->SetPageProperty("BodyClass",
    ($bodyClass ? $bodyClass." " : "")."pagetitle-menu-visible");
