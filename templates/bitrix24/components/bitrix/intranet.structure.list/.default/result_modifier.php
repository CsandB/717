...
$arResult['USER_PROP'] = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("USER",
    0, LANGUAGE_ID);
...
if (isset($arResult['USERS']) && is_array($arResult['USERS'])) {
    foreach ($arResult['USERS'] as $i => $arUser) {
        ...
        if (CModule::IncludeModule('socialnetwork')
            ...
            $arUser["CurrentUserPerms"]
                = CSocNetUserPerms::InitUserPerms($GLOBALS["USER"]->GetID(),
                $arUser["ID"], CSocNetUser::IsCurrentUserModuleAdmin());

            if (
                ($GLOBALS["USER"]->GetID() != $arUser["ID"])
                && (IsModuleInstalled("im")
                    || $arUser["CurrentUserPerms"]["Operations"]["message"])
            ) {
        ...
        $arUser["Urls"]
            = array(
            "VideoCall" => CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_VIDEO_CALL"],
                array(
                    "user_id" => $arUser["ID"],
                    "USER_ID" => $arUser["ID"],
                    "ID"      => $arUser["ID"],
