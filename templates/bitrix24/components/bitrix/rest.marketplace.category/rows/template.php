        ...
        foreach ($arResult["ITEMS"] as $app) {
            ...
            <li class="mp-apps-item">
                <a href="javascript:void(0)"
                   class="mp-apps-icon mp-apps-icon--grey">
                    <?
                    if ($app["ICON"]):
                        ?>
                        <img src="<?= htmlspecialcharsbx($app["ICON"]) ?>"
                             alt="">
                    <?php
                    else:
                        ?>
                ...
                <div class="mp-apps-info-container">
                    ...
                    <? if ($app["INSTALLED"] == "Y"):?>
                        <a href="<?= $app["URL"] ?>"
                           ...
                           onclick="BX.Rest.Markeplace.CategoryRows.setCurrentApp('<?= CUtil::JSEscape($app['CODE']) ?>')"><?= GetMessage("MARKETPLACE_CATEGORY_ROWS_DEINSTALL") ?></a>
                    <? else:?>
                        <a href="<?= $app["URL"] ?>"
                           class="ui-btn ui-btn-success"
                           data-id="btn-<?= $app['CODE'] ?>"
                           onclick="BX.Rest.Markeplace.CategoryRows.setCurrentApp('<?= CUtil::JSEscape($app['CODE']) ?>')"><?= GetMessage("MARKETPLACE_CATEGORY_ROWS_INSTALL") ?></a>
                    <?endif ?>
                </div>
            </li>
            <?
