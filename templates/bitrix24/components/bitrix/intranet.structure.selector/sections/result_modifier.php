...
$arResult["CURRENT_SECTION"]
    =
$current_section = intval($arResult['FILTER_VALUES'][$arParams['FILTER_NAME']
.'_UF_DEPARTMENT']);
if ($current_section === 0) {
    $iblockID = COption::GetOptionInt("intranet", "iblock_structure");
    $db_up_department = CIBlockSection::GetList(array(),
        array("SECTION_ID" => 0, "IBLOCK_ID" => $iblockID));
    if ($ar_up_department = $db_up_department->Fetch()) {
        $arResult["CURRENT_SECTION"]
            = $current_section = $ar_up_department["ID"];
    }
...
if ($arParams['SHOW_SECTION_INFO'] == 'Y' && $current_section) {
    $dbRes = CIBlockSection::GetList(array(),
        array("IBLOCK_ID" => $iblockID, "ID" => $current_section), false,
        array("UF_HEAD"));
    if ($arResult['SECTION_DATA'] = $dbRes->GetNext()) {
        if ($arResult['SECTION_DATA']['DETAIL_PICTURE']) {
            $arResult['SECTION_DATA']['DETAIL_PICTURE']
                = CFile::ShowImage($arResult['SECTION_DATA']['DETAIL_PICTURE']);
        } elseif ($arResult['SECTION_DATA']['PICTURE']) {
            $arResult['SECTION_DATA']['DETAIL_PICTURE']
                = CFile::ShowImage($arResult['SECTION_DATA']['PICTURE']);
        }
        //uf_head:
        $dbUser = CUser::GetList($b = "", $o = "", array(
            "ID"     => intval($arResult['SECTION_DATA']["UF_HEAD"]),
            "ACTIVE" => "Y",
        ), array(
            'FIELDS' => array(
                "ID",
            ...
            'SELECT' => array("UF_SKYPE"),
        ));
        if ($arUser = $dbUser->GetNext()) {
            $arResult['SECTION_DATA']["UF_HEAD"] = $arUser;
            $arResult['SECTION_DATA']["UF_HEAD"]["FORMATTED_NAME"]
                = CUser::FormatName($arParams['NAME_TEMPLATE'], $arUser, true,
                false);

            if (intval($arResult['SECTION_DATA']["UF_HEAD"]["PERSONAL_PHOTO"])
                > 0
            ) {
                $imageFile
                    = CFile::GetFileArray($arResult['SECTION_DATA']["UF_HEAD"]["PERSONAL_PHOTO"]);
                if ($imageFile !== false) {
            ...
            if ( ! empty($arUser["CONFIRM_CODE"])) {
                $arResult['SECTION_DATA']["UF_HEAD"]["ACTIVITY_STATUS"]
                    = "inactive";
            } else {
                $arResult['SECTION_DATA']["UF_HEAD"]["ACTIVITY_STATUS"]
                    = "active";
        ...
        // head department:
        $resSec = CIBlockSection::GetList(array(), array(
            "IBLOCK_ID" => $iblockID,
            "ID"        => $arResult['SECTION_DATA']["IBLOCK_SECTION_ID"],
        ), false, array("ID", "NAME", "UF_HEAD", "SECTION_PAGE_URL"));
        if ($arSection = $resSec->GetNext()) {
            $arResult['SECTION_DATA']["IBLOCK_SECTION_NAME"]
                = $arSection["NAME"];
            $arResult['SECTION_DATA']["IBLOCK_SECTION_UF_HEAD"]
                = $arSection["UF_HEAD"];
            $dbUser = CUser::GetList($b = "", $o = "",
                array("ID" => intval($arSection["UF_HEAD"])));
            if ($arUser = $dbUser->Fetch()) {
                $arResult['SECTION_DATA']["IBLOCK_SECTION_UF_HEAD_NAME"]
                    =
                $name = CUser::FormatName($arParams['NAME_TEMPLATE'], $arUser);
            }
        }
...
while ($arRes = $rsEnum->GetNext()) {
    ...
    if ($current_section && ! $bChainFinished) {
        $arResult['SECTIONS_CHAIN'][$arRes['DEPTH_LEVEL'] - 1]
            = array($arRes['ID'], $arRes['NAME']);
        if ($current_section == $arRes['ID']) {
            $bChainFinished = true;
            if (true || $arParams['SET_TITLE'] == 'Y') {
                $GLOBALS['APPLICATION']->SetTitle($arRes['NAME']);
            }
            $arResult['SECTIONS_CHAIN']
                = array_slice($arResult['SECTIONS_CHAIN'], 0,
                $arRes['DEPTH_LEVEL']);
        }
    }
}
...
$IBLOCK_PERMISSION = CIBlock::GetPermission(COption::GetOptionInt('intranet',
    'iblock_structure'));
