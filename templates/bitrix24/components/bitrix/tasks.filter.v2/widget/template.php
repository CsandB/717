...
<div class="sidebar-widget sidebar-widget-tasks">
    <div class="sidebar-widget-top">
        ...
        <?
        $path
            = \Bitrix\Tasks\Integration\Socialnetwork\UI\Task::getActionPath();
        $url = Task::makeActionUrl($path);
        ?>
    ...
    <div class="sidebar-widget-item-wrap">
        ...
        if ($arResult['USE_ROLE_FILTER'] === 'Y') {
            ...
            foreach ($arResult["PRESETS_LIST"] as $key => $filter) {
                ...
                if ($filter["#DEPTH"] > 1) {
                    $levelOffset
                        = 'task-filter-item-sublevel task-filter-item-sublevel_'
                        .($filter["#DEPTH"] - 1);
                }
                ?>

                <a class="task-item <?= $levelOffset ?>"
                   href="<?= ($arParams["PATH_TO_TASKS"]
                       ."?F_FILTER_SWITCH_PRESET=".$key) ?>">
