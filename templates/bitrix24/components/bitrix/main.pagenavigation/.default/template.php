    ...
    if ($arResult["REVERSED_PAGES"] === true) {
        ...
        if ($arResult["CURRENT_PAGE"] < $arResult["PAGE_COUNT"]) {
            ...
            if ($arResult["PAGE_COUNT"] == ($arResult["CURRENT_PAGE"] + 1)) {
                ...
                $sPrevHref
                    = htmlspecialcharsbx($component->replaceUrlTemplate($arResult["CURRENT_PAGE"]
                    + 1));
            }
        } else {
            $bPrevDisabled = true;
        ...
        <div class="navigation-pages">
            ...
            $bPoints = false;
            do {
                $NavRecordGroupPrint = $arResult["PAGE_COUNT"]
                    - $arResult["START_PAGE"] + 1;
                if ($arResult["START_PAGE"] <= 2
                    || $arResult["PAGE_COUNT"] - $arResult["START_PAGE"] <= 1
                    || abs($arResult['START_PAGE'] - $arResult["CURRENT_PAGE"])
                    <= 2
                ) {
                    if ($arResult["START_PAGE"] == $arResult["CURRENT_PAGE"]):
                        ?><span
                            class="navigation-current-page"><?= $NavRecordGroupPrint ?></span><?
                    elseif ($arResult["START_PAGE"] == $arResult["PAGE_COUNT"]):
                        ?><a data-slider-ignore-autobinding="true"
        ...
        <div class="navigation-arrows">
            <<?
            if ( ! $bPrevDisabled):?>a href="<?= $sPrevHref; ?>" data-slider-ignore-autobinding="true" id="<?= $ClientID ?>_previous_page"<? else: ?>span<? endif ?>
                    class="navigation-button<?
                    if ($bPrevDisabled):?> navigation-disabled<? endif ?>"><span
                        class="navigation-text"><span
                            class="navigation-ctrl-before">Ctrl</span><span
                            class="navigation-text-cont"><?= GetMessage(
                            "mp_nav_prev"
                        ) ?></span></span><?
                if ( ! $bPrevDisabled): ?></a><?
            else: ?></span><?
            endif ?><<?
            if ( ! $bNextDisabled):?>a href="<?= $sNextHref; ?>" data-slider-ignore-autobinding="true" id="<?= $ClientID ?>_next_page"<? else: ?>span<? endif ?>
                    class="navigation-button<?
                    if ($bNextDisabled):?> navigation-disabled<? endif ?>"><span
                        class="navigation-text"><span
                            class="navigation-text-cont"><?= GetMessage(
                            "mp_nav_next"
                        ) ?></span><span
                            class="navigation-ctrl-after">Ctrl</span></span><?
                if ( ! $bNextDisabled): ?></a><?
        else:?></span><?
        endif ?>
        </div>
        ...
        <div class="navigation-pages">
            ...
            do {
                if ($arResult["START_PAGE"] <= 2
                    || $arResult["END_PAGE"] - $arResult["START_PAGE"] <= 1
                    || abs($arResult['START_PAGE'] - $arResult["CURRENT_PAGE"])
                    <= 2
                ) {
                    if ($arResult["START_PAGE"] == $arResult["CURRENT_PAGE"]):
                        ?><span
                            class="navigation-current-page"><?= $arResult["START_PAGE"] ?></span><?
