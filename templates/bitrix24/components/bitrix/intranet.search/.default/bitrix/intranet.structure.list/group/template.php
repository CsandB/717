...
if ( ! is_array($arResult['USERS'])
    ...
    foreach ($arResult['DEPARTMENTS'] as $arDept) {
        $arDeptsChain[$arDept['DEPTH_LEVEL']] = array(
            "NAME" => $arDept['NAME'],
            "ID"   => $arDept['ID'],
        );//'<a class="breadcrumbs-item" href="'.$arParams['STRUCTURE_PAGE'].'?set_filter_'.$arParams['STRUCTURE_FILTER'].'=Y&'.$arParams['STRUCTURE_FILTER'].'_UF_DEPARTMENT='.$arDept['ID'].'">'.htmlspecialcharsbx($arDept['NAME']).'<i></i></a>';
        if (count($arDept['USERS']) > 0) {
            ?>
            <div class="breadcrumbs" style="padding:10px 0 0 0">
                <?
                $ar_dep_breadcrumb = array_slice($arDeptsChain, 0,
                    $arDept['DEPTH_LEVEL']);
                $ar_dep_breadcrumb_size = sizeof($ar_dep_breadcrumb) - 1;
                foreach ($ar_dep_breadcrumb as $key => $val) {
                    ?><a class="breadcrumbs-item"
                         href="<?= $arParams['STRUCTURE_PAGE'].'?set_filter_'
                         .$arParams['STRUCTURE_FILTER'].'=Y&'
                         .$arParams['STRUCTURE_FILTER'].'_UF_DEPARTMENT='
                         .$val['ID'] ?>"><?= htmlspecialcharsbx($val['NAME']) ?><? if ($key
                        != $ar_dep_breadcrumb_size
                    ):?><i></i><?endif; ?></a><?
                }
