...
$APPLICATION->IncludeComponent("bitrix:main.user.link",
    ...
    array(
        "AJAX_ONLY"                   => "Y",
        "PATH_TO_SONET_USER_PROFILE"  => COption::GetOptionString('intranet',
    ...
    <table class="bx-users-table data-table" style="border: 0px" width="100%">
        ...
        <tr>
            ...
            foreach ($arParams['USER_PROPERTY'] as $key):
                ?>
                <td><?= $arResult['USER_PROP'][$key]
                        ? $arResult['USER_PROP'][$key]
        ...
        foreach ($arResult['DEPARTMENTS'] as $arDept) {
            $arDeptsChain[$arDept['DEPTH_LEVEL']]
                = '<a style="font-size:15px; color:#000;" href="'
                .$arParams['STRUCTURE_PAGE'].'?set_filter_'
                .$arParams['STRUCTURE_FILTER'].'=Y&'
                .$arParams['STRUCTURE_FILTER'].'_UF_DEPARTMENT='.$arDept['ID']
                .'">'.htmlspecialcharsbx($arDept['NAME']).'</a>';
            ...
            <tr>
                <td colspan="<?= count($arParams['USER_PROPERTY']) ?>">
                    ...
                    <div class="users-departments-chain"
                         style="margin-bottom:4px;"><?= isset($arDept['DEPTH_LEVEL'])
                            ? implode('&nbsp;-&nbsp;',
                                array_slice($arDeptsChain, 0,
                                    $arDept['DEPTH_LEVEL']))
                            : GetMessage('ISL_DEPARTMENT_NOT_FOUND') ?></div>
                </td>
            </tr>
                            ...
                            switch ($key) {
                                    ...
                                    if (true):
                                        ...
                                        <div class="bx-user-name">
                                            ...
                                            if ($bShowLink):
                                                ...
                                                bx-tooltip-user-id="<?= $arUser["ID"] ?>"><?= $arUser["NAME_FORMATTED"] ?></a><?
                                            else:
                                                ?><?= $arUser["NAME_FORMATTED"] ?><?
                                            endif;
                                            ?>
                                ...
                                case 'EMAIL':
                                    $result = '<a href="mailto:'
                                        .urlencode($arUser[$key]).'">'
                                        .htmlspecialcharsbx($arUser[$key])
                                        .'</a>';
                                    break;
                                ...
                                case 'UF_PHONE_INNER':
                                    $result = $arUser[$key] ? '<a href="callto:'
                                        .urlencode($arUser[$key]).'">'
                                        .htmlspecialcharsbx($arUser[$key])
                                        .'</a>' : '';
                                    break;
                                ...
                                case 'UF_SKYPE':
                                    $result = $arUser[$key] ? '<a href="callto:'
                                        .urlencode($arUser[$key]).'">'
                                        .htmlspecialcharsEx($arUser[$key])
                                        .'</a>' : '';
                                    break;

                                default:
                                    if (mb_substr($key, 0, 3) == 'UF_'
                                        && is_array($arResult['USER_PROPERTIES'][$key])
                                    ) {
                                        ob_start();
                                        $arResult['USER_PROPERTIES'][$key]['VALUE']
                                            = $arUser[$key];
