            ...
            <div class="sidebar-block-inner">
                ...
                <div class="filter-block">
                    ...
                    <div class="filter-field-buttons">
                        <input type="submit"
                               value="<?= GetMessage("INTR_ISE_TPL_FILTER_SUBMIT") ?>"
                               class="filter-submit"/>&nbsp;&nbsp;<input
                                type="submit"
                                value="<?= GetMessage("INTR_ISE_TPL_FILTER_CANCEL") ?>"
                                onclick="document.forms.bx_events_filter.department.selectedIndex=0;"
                                class="filter-submit"/>
                    </div>
                </div>
            </div>
    ...
    <div class="bx-events-layout">
        ...
        foreach ($arResult['ENTRIES'] as $arEntry) {
            ...
            $arUser['UF_DEPARTMENT']
                = array($arEntry['PROPERTY_DEPARTMENT_VALUE'] => $arResult['DEPARTMENTS'][$arEntry['PROPERTY_DEPARTMENT_VALUE']]);
            $arUser['WORK_POSITION'] = $arEntry['PROPERTY_POST_VALUE'];

            $arUser['SUBTITLE'] = FormatDateEx($arEntry['DATE_ACTIVE_FROM'],
                    false, $arParams['DATE_FORMAT'])
                .' - '
                .ToLower(
                    $arEntry['PROPERTY_STATE_VALUE']
                        ? $arEntry['PROPERTY_STATE_VALUE']
                        : ($arEntry['PREVIEW_TEXT'] ? $arEntry['PREVIEW_TEXT']
                        : $arEntry['NAME'])
                );
