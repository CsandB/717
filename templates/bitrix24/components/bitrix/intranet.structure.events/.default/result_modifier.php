...
if ( ! empty($arRes)) {
    foreach ($arRes as $key => $val) {
        ...
        <> '' ? $val["EDIT_FORM_LABEL"] : $val["FIELD_NAME"]);
    }
}
...
if ($arParams['bAdmin']):
    ...
    __IncludeLang(dirname(__FILE__).'/lang/'.LANGUAGE_ID.'/'
        .basename(__FILE__));

    $INTRANET_TOOLBAR->AddButton(array(
        'ONCLICK' => $APPLICATION->GetPopupLink(array(
            'URL'    => "/bitrix/admin/iblock_element_edit.php?type="
                .$arParams['IBLOCK_TYPE']."&lang=".LANGUAGE_ID."&IBLOCK_ID="
                .$arParams['IBLOCK_ID']."&bxpublic=Y&from_module=iblock",
            'PARAMS' => array(
                'height' => 500,
                'width'  => 700,
                'resize' => false,
    ...
    $INTRANET_TOOLBAR->AddButton(array(
        'HREF'  => "/bitrix/admin/iblock_element_admin.php?type="
            .$arParams['IBLOCK_TYPE']."&lang=".LANGUAGE_ID."&IBLOCK_ID="
            .$arParams['IBLOCK_ID'],
