;(function(window) {
  ...
  function animation(message, main_block) {
    ...
    var corner = BX.create('div',
        {props: {className: 'anim-corner'}, children: [corner_gradient]}),
        corner_wrap = BX.create('div',
            {props: {className: 'anim-corner-wrap'}, children: [corner]}),
        distort_shadow = BX.create('div', {
          props: {className: 'block-distort-shadow-wrap'},
          children: [
            BX.create('div', {props: {className: 'block-distort-shadow'}})],
        }),
        distort = BX.create('div', {
          props: {className: 'block-distort'},
          children: [message, corner_wrap],
        }),
        main_wrap = BX.create('div', {
          props: {className: 'main-mes-wrap'},
          children: [distort, distort_shadow],
        });
      ...
      complete: function() {
        ...
        var easing_2 = new BX.easing({
          ...
          step: function(state) {
            ...
            distort.style[vendor('transform')] = 'rotate(' +
                state.distort_rotate + 'deg)';

            corner_wrap.style[vendor('transform')] = 'rotate(' +
                state.corner_rotate + 'deg)';
            corner_wrap.style.bottom = state.corner_bottom + 'px';

            corner.style.height = state.corner_height + 'px';

            distort_shadow.style[vendor('transform')] = 'rotate(' +
                state.shadow_rotate + 'deg)';

            message.style[vendor('transform')] = 'rotate(' +
                state.message_rotate + 'deg)';

            corner_gradient.style.height = state.corner_height + 'px';

            corner_gradient.style.backgroundImage = getPrefix() +
                'linear-gradient(' + state.gradient_rotate +
                'deg, #ece297 42%, #e5d38e 57%, #f6e9a3 78%)';
          ...
          complete: function() {
            ...
            var easing_3 = new BX.easing({
              ...
              step: function(state) {
                ...
                corner.style[vendor('transform')] = 'skew(' +
                    state.corner_skew + 'deg, 0deg)';
                ...
                distort_shadow.style[vendor('transform')] = 'rotate(' +
                    state.shadow_rotate + 'deg) skew(' + state.shadow_skew +
                    'deg, 0)';
                ...
                corner_gradient.style.backgroundImage = getPrefix() +
                    'linear-gradient(' + state.gradient_rotate +
                    'deg, #ece297 42%, #e5d38e ' + state.gradient_percent +
                    '%, #f6e9a3 78%)';
              },
              complete: function() {
  ...
  top.BSBBW = function(params) {
    ...
    BX.addCustomEvent(this.controller, 'onDataAppeared',
        BX.delegate(this.onDataAppeared, this));
    BX.addCustomEvent(this.controller, 'onDataRanOut',
        BX.delegate(this.onDataRanOut, this));
    BX.addCustomEvent(this.controller, 'onReachedLimit',
        BX.delegate(this.onReachedLimit, this));
    BX.addCustomEvent(this.controller, 'onRequestSend',
        BX.delegate(this.showWait, this));
    BX.addCustomEvent(this.controller, 'onResponseCame',
        BX.delegate(this.hideWait, this));
    BX.addCustomEvent(this.controller, 'onResponseFailed',
        BX.delegate(this.hideWait, this));
    BX.addCustomEvent(window, 'onImUpdateCounter',
        BX.delegate(this.onImUpdateCounter, this));
    ...
    BX.addCustomEvent(window, 'onSonetLogCounterClear', BX.delegate(
        function() {this.onImUpdateCounter({'BLOG_POST_IMPORTANT': 0});},
        this));
    BX.addCustomEvent(window, 'onImportantPostRead',
        BX.delegate(this.onImportantPostRead, this));
  };

  top.BSBBW.prototype = {
    init: function(params) {
      this.page_settings = params['page_settings'];
      this.page_settings['NavRecordCount'] = parseInt(
      ...
      if (!this.inited) {
        BX.bind(this.nodes['right'], 'click',
            BX.delegate(function() {this.onShiftPage('right');}, this));
        BX.bind(this.nodes['left'], 'click',
            BX.delegate(function() {this.onShiftPage('left');}, this));
        BX.adjust(this.nodes['btn'], {
          attrs: {url: this.url},
          events: {click: BX.delegate(this.onClickToRead, this)},
        });
      }
      return true;
    },
    show: function() {
      ...
      var btn = BX.findChild(this.nodes['text'],
          {'className': 'sidebar-imp-mess-text'}, true);
      var avatar = BX.findChild(this.nodes['text'],
          {attribute: {'data-bx-author-avatar': true}}, true);
      if (!!btn) {
        BX.adjust(btn, {
          attrs: {url: this.url},
          events: {click: BX.delegate(this.onClickToRead, this)},
      ...
      btn = BX.findChild(this.nodes['leaf'],
          {'className': 'sidebar-imp-mess-text'}, true);
      avatar = BX.findChild(this.nodes['leaf'],
          {attribute: {'data-bx-author-avatar': true}}, true);
      ...
      if (!result || !result.data || result.data.length <= 0)
        ...
        for (var ii in data) {
          if (typeof data[ii] == 'object' &&
              !this.data_id['id' + data[ii]['id']]) {
            dataNew.push(data[ii]);
          }
        }
        result.page_settings['NavRecordCount'] = parseInt(
            result.page_settings['NavRecordCount']);
        this.page_settings['NavRecordCount'] = parseInt(
            this.page_settings['NavRecordCount']);
        if (this.data.length > 0 &&
            dataNew.length == (result.page_settings['NavRecordCount'] -
                this.page_settings['NavRecordCount'])) {
          var d = dataNew.pop();
          while (!!d) {
    ...
    onClickToRead: function(send) {
      ...
      for (ii in this.options)
        options.push({
          post_id: data['id'],
          name: this.options[ii]['name'],
          value: this.options[ii]['value'],
        });
      var request = this.post_info;
    ...
    onShiftPage: function(status) {
      ...
      this.active = setTimeout(
          BX.delegate(function() {this.active = false;}, this), 120000);
    ...
    onDataAppeared: function() {
      if (!!this.data && this.data.length > 0 &&
          this.controller.style.display == 'none') {
        var height = (!!this.bodyAnimationheight
        ...
        (this.bodyAnimation = new BX.easing({
          ...
          complete: BX.delegate(function() {
            BX.adjust(this.controller,
                {style: {display: 'block', height: 'auto', opacity: 'auto'}});
          }, this),
        })).animate();
      }
    },
    onReachedLimit: function() {
      ...
      this.page_settings['NavPageNomer'] = parseInt(
          this.page_settings['NavPageNomer']);
      this.page_settings['NavPageCount'] = parseInt(
          this.page_settings['NavPageCount']);
      ...
      if (this.page_settings['NavPageCount'] <= 1)
        ...
        if (this.page_settings['NavPageNomer'] > 1)
          this.page_settings['iNumPage'] = parseInt(
              this.page_settings['NavPageNomer']) - 1;
        else
          needToUnbind = true;
      } else if (this.page_settings['NavPageNomer'] <
          this.page_settings['NavPageCount'])
        this.page_settings['iNumPage'] = parseInt(
            this.page_settings['NavPageNomer']) + 1;
      else
      ...
      if (needToUnbind === true) {
        BX.removeCustomEvent(this.controller, 'onReachedLimit',
            BX.delegate(this.onReachedLimit, this));
        return true;
      }
