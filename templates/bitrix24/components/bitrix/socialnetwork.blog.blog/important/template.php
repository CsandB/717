...
foreach ($arResult["POST"] as $id => $res) {
    $res = array(
        "id"                  => $res["ID"],
        "post_text"           => TruncateText(($res["MICRO"] != "Y"
            ? $res["TITLE"]." ".$res["CLEAR_TEXT"] : $res["CLEAR_TEXT"]),
            $arParams["MESSAGE_LENGTH"]),
        "post_url"            => $res["urlToPost"],
        "author_name"         => $res["AUTHOR_NAME"],
        "author_avatar_style" => (! empty($res["AUTHOR_AVATAR"]["src"])
            ? "url('".$res["AUTHOR_AVATAR"]["src"]."')" : ""),
        "author_avatar"       => (! empty($res["AUTHOR_AVATAR"]["src"])
            ? "style=\"background:url('".$res["AUTHOR_AVATAR"]["src"]
...
?>
    <div class="sidebar-widget sidebar-imp-messages"
         id="blog-<?= $uid ?>"<? if (empty($arRes["data"])) { ?> style="display:none;"<? } ?>>
            ...
            <div class="sidebar-imp-mess-tmp">
                <div class="sidebar-imp-mess" id="message-block-<?= $uid ?>">
                    <div class="sidebar-imp-mess-wrap"
                         id="blog-leaf-<?= $uid ?>">
                        <div class="user-avatar sidebar-user-avatar user-default-avatar"<? if ($res["author_avatar"]
                            !== ""
                        ) { ?> <?= $res["author_avatar"] ?><? } ?>></div>
                        <a href="<?= $res["author_url"] ?>"
                           class="sidebar-imp-mess-title"><?= $res["author_name"] ?></a>
                        <a href="<?= $res["post_url"] ?>"
                           class="sidebar-imp-mess-text"><?= $res["post_text"] ?></a>
                    </div>
                    <div class="sidebar-imp-mess-wrap"
                         id="blog-text-<?= $uid ?>">
                        <div class="user-avatar sidebar-user-avatar user-default-avatar"<? if ($res["author_avatar"]
                    ...
                    <div id="blog-<?= $uid ?>-template"
                         class="sidebar-imp-mess-templates"
                         style="display:none;">
                        <div class="user-avatar sidebar-user-avatar user-default-avatar"
                             data-bx-author-avatar="true"></div>
                    ...
                    <div class="sidebar-imp-mess-bottom">
                        <span id="blog-<?= $uid ?>-btn"
                              class="sidebar-imp-mess-btn"><?= $btnTitle ?></span>
                        <div class="sidebar-imp-mess-nav-block">
                            <span class="sidebar-imp-mess-nav-arrow-l"
                                  id="blog-<?= $uid ?>-right"></span>
                            <span class="sidebar-imp-mess-nav-arrow-r"
                                  id="blog-<?= $uid ?>-left"></span>
                            <span id="blog-<?= $uid ?>-current"
                                  class="sidebar-imp-mess-nav-current-page">1</span><?
                            ?>
                            <span class="sidebar-imp-mess-nav-separator">/</span><?
                            ?><span id="blog-<?= $uid ?>-total"
                                    class="sidebar-imp-mess-nav-total-page"><?= $arResult["NAV_RESULT"]->NavRecordCount ?></span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        if (!!<?=$controller?> && !<?=$controller?>.loaded) {
          ...
          new BSBBW({
            ...
            'post_info': {
              ...
              'filter': <?=CUtil::PhpToJSObject($filter)?>,
              'avatar_size': <?=intval($arParams["AVATAR_SIZE"])?>,
            },
